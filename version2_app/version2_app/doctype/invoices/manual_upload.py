...
def manual_upload(data):
	try:
		...
		for each in output:
			# print(str(each['BILL_GENERATION_DATE']))
			...
			if each['TRANSACTION_DESCRIPTION'] not in payment_Types:
				# each['BILL_NO'] = str(each['BILL_NO']).strip()
				# print(each['BILL_NO']),len(each['BILL_NO'])
				each['BILL_GENERATION_DATE'] = str(each['BILL_GENERATION_DATE'])
				# each['BILL_GENERATION_DATE_CHAR'] = str(each['BILL_GENERATION_DATE_CHAR'])
				# companyData.invoice_item_date_format
