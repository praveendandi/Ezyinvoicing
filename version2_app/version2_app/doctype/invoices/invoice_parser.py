...
from version2_app.version2_app.doctype.invoices.invoices import calulate_items,Error_Insert_invoice,gsp_api_data,check_token_is_valid,get_tax_payer_details,insert_invoice,check_company_exist,check_invoice_exists
from version2_app.version2_app.doctype.payment_types.payment_types import GetPaymentTypes
...
def file_parsing(filepath):
	try:
		...
		if len(gstNumber) < 15 and len(gstNumber)>0:
			...
			# logger.error("Error:  *******The given gst number is not a vaild one**********")
				...
				if checkTokenIsValidResponse['success'] == True:
					...
					if getTaxPayerDetailsResponse['success'] == True:
						...
						if calulateItemsApiResponse['success'] == True:
							...
							if reupload == False:
								insertInvoiceApiResponse = insert_invoice({"guest_data":guest,"company_code":company_code['code'],"taxpayer":getTaxPayerDetailsResponse['data'],"items_data":calulateItemsApiResponse['data'],"total_invoice_amount":total_invoice_amount,"invoice_number":guest['invoice_number'],"amened":amened})
								if insertInvoiceApiResponse['success']== True:
									# logger.info(f"Invoice Created,   {insertInvoiceApiResponse}")
									...
									errorInvoice = Error_Insert_invoice(error_data)
									# logger.error(f"insertInvoiceApi fialed:,   {insertInvoiceApiResponse['message']}")
									return {"success":False,"message":insertInvoiceApiResponse['message']}
							else:
								insertInvoiceApiResponse = Reinitiate_invoice({"guest_data":guest,"company_code":company_code['code'],"taxpayer":getTaxPayerDetailsResponse['data'],"items_data":calulateItemsApiResponse['data'],"total_invoice_amount":total_invoice_amount,"invoice_number":guest['invoice_number'],"amened":amened})
								if insertInvoiceApiResponse['success']== True:
									# logger.info(f"Invoice Created,   {insertInvoiceApiResponse}")
									return {"success":True,"message":"Invoice Created"}
							...
							error_data['amened'] = amened
							errorInvoice = Error_Insert_invoice(error_data)
							# logger.error(f"calulateItemsApi fialed:,   {calulateItemsApiResponse['message']}")
						...
						error_data['amened'] = amened
						errorInvoice = Error_Insert_invoice(error_data)
						#logger.error(f"Tax payer details fialed:,   {getTaxPayerDetailsResponse['message']}")
					...
					error_data['amened'] = amened
					errorInvoice = Error_Insert_invoice(error_data)
					#logger.error(f"Check Token Valid:,   {checkTokenIsValidResponse['message']}")
				...
				if calulateItemsApiResponse['success'] == True:
					...
					if reupload == False:
						...
						if insertInvoiceApiResponse['success']== True:
							...
							error_data['amened'] = amened
							errorInvoice = Error_Insert_invoice(error_data)
							#logger.error(f"B2C insertInvoiceApi fialed:,   {insertInvoiceApiResponse['message']}")
							return {"success":False,"message":insertInvoiceApiResponse['message']}
					else:
						...
						if insertInvoiceApiResponse['success']== True:
							...
							error_data['amened'] = amened
							errorInvoice = Error_Insert_invoice(error_data)
							#logger.error(f"B2C re insertInvoiceApi fialed:,   {insertInvoiceApiResponse['message']}")
							return {"success":False,"message":insertInvoiceApiResponse['message']}
					...
					error_data['amened'] = amened
					errorInvoice = Error_Insert_invoice(error_data)
					#logger.error(f"B2C calulateItemsApi fialed:,   {calulateItemsApiResponse['message']}")
					return {"success":False,"message":calulateItemsApiResponse['message']}		
		else:
			error_data['error_message'] = gspApiDataResponse['message']
			error_data['amened'] = amened
			errorInvoice = Error_Insert_invoice(error_data)
			#logger.error(f"gspApiData fialed:,   {gspApiDataResponse['message']}")
			return {"success":False,"message":gspApiDataResponse['message']}
	except Exception as e:
		frappe.log_error(frappe.get_traceback())
		#logger.error(f"file_parsing,   {str(e)}")
