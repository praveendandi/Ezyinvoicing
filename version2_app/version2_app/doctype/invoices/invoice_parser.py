...
def file_parsing(filepath):
	...
	for index, i in enumerate(data):
		...
		if "S.Charge" in i or "S. Charge" in i or "Cash" in i:
			i = "  "
		if 'XX/XX' in i:
			i = " "
		if i !=" ":
	...
	for i in original_data:
		...
		if len(check_date) > 0 and "CGST" not in i and "SGST" not in i and "CESS" not in i and "Vat" not in i and "VAT" not in i and "Cess" not in i and "Allow " not in i:
			item = dict()
			for index, j in enumerate(i.split(' ')):
				...
				if index == 1:
					item['name'] = j
				if j=="Ram" or j== "Room#" or j=="707" or j=="CHECK":
					j=' '	
				if index == 2:
					if len(i.split(" "))>3:
