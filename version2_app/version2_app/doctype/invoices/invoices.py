			...
			if calulationType.calculation_by == "Description":
				...
				if sac_code_based_gst_rates.taxble == "Yes":
					if sac_code_based_gst_rates.net == "No":
						...
						gst_percentage = (int(sac_code_based_gst_rates.cgst) + int(sac_code_based_gst_rates.sgst))
						...
						final_item['gst_rate'] = int(sac_code_based_gst_rates.cgst)+int(sac_code_based_gst_rates.sgst)+int(sac_code_based_gst_rates.igst)
					final_item['other_charges'] = 0
					final_item['item_value'] = item['item_value']
					...
					if item['sac_code'] != "996311" and sac_code_based_gst_rates.taxble == "No":
						...
						final_item['taxable'] = sac_code_based_gst_rates.taxble
						final_item['type'] = "Non-Gst"
				if sac_code_based_gst_rates.cess == "Yes":
					if sac_code_based_gst_rates.state_cess == "Yes":
						final_item['cess'] = sac_code_based_gst_rates.state_cess_rate
						final_item["cess_amount"] = (item["item_value"]*(sac_code_based_gst_rates.state_cess_rate/100))
					if sac_code_based_gst_rates.central_cess == "Yes":
					...
					final_item['cess'] = 0
					final_item["cess_amount"] = 0
				if sac_code_based_gst_rates.vat == "Yes":
				...
				final_item['item_value_after_gst'] = final_item['item_value_after_gst']+final_item['cess_amount']+final_item['vat_amount']
				print("====",final_item)
			else:
				sac_code_based_gst_rates = frappe.get_doc(
					'SAC HSN CODES', item['sac_code'])
