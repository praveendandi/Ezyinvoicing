...
def GstDataImportToLicensing():
    try:
        ...
        if company.proxy == 1:
            ...
            insertCompnay = requests.post(company.licensing_host+"/api/resource/Properties",headers=headers,json=inputData,proxies=proxies,verify=False)
        else:
        ...
        if len(all_data)>0:
            for each in all_data:
                ...
                headers = {'Content-Type': 'application/json'}
                if company.proxy == 1:
                    json_response = requests.post(company.licensing_host+"/api/resource/TaxPayerDetail",headers=headers,json=each,proxies=proxies,verify=False)
                else:
                    if company.skip_ssl_verify == 1:
