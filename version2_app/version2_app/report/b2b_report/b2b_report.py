...
def execute(filters=None):
	try:
		...
		columns = ['Receiver GSTIN / UIN','Receiver Name','Invoice Type (R / SEZ / DE)','Invoice Number','Guest Name','Invoice Date','Checkout Date','Invoice Value','Place Of Supply','Reverse Charge','E-Commerce GSTIN','SAC / HSN CODE','Applicable % of Tax Rate','Tax Rate',"Taxable Value","Non Taxable Value",'Integrated Tax Amount','Central Tax Amount','State / UT Tax Amount','State Cess Amount','Central Cess Amount',"Irn Generated","Irn Number","Ack No","Ack Date","Other Charges"]
		fields = ['invoice_number','guest_name', 'invoice_date','checkout_date','gst_number','trade_name','place_of_supply','invoice_category','total_invoice_amount','sez',"irn_generated","irn_number","ack_no","ack_date","other_charges"]
		...
		grouped_df = items_df.groupby(["invoice_number","taxable","gst_rate","sac_code"],as_index=False).sum().round(2)
		...
		mergedDf.rename(columns={"other_charges":"Other Charges",'invoice_number': 'Invoice Number','guest_name':'Guest Name', 'invoice_date': 'Invoice Date','checkout_date':'Checkout Date','gst_number':'Receiver GSTIN / UIN','trade_name':'Receiver Name','total_invoice_amount':'Invoice Value','place_of_supply':'Place Of Supply','sac_code':'SAC / HSN CODE','gst_rate':'Tax Rate','tax_value':'Taxable Value','non_tax_value':'Non Taxable Value','igst_amount':'Integrated Tax Amount','cgst_amount':'Central Tax Amount','sgst_amount':'State / UT Tax Amount','state_cess_amount':'State Cess Amount','cess_amount':"Central Cess Amount","irn_generated":"Irn Generated","irn_number":"Irn Number","ack_no":"Ack No","ack_date":"Ack Date"}, inplace=True)
