...
def execute(filters=None):
	try:
		...
		doc = frappe.db.get_list('Invoices', filters={'invoice_date': ['Between',(filters['from_date'],filters['to_date'])],'irn_generated':['=','Success'],'invoice_category':['!=','Credit Invoice'], 'un_billed_invoice': 0},fields=fields,as_list=True)
		if len(doc) == 0:
			data = []
			columns = []
			return columns,data
