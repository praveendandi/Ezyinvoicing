...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RBRC-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i or "Total" in i:
            ...
            if total_invoice_amount == "":
                if "Balance  INR" in i or "Balance" in i and "INR" in i:
                    ind = raw_data.index(i)
                    total_ind = raw_data[ind-2]
                    total_invoice_amount=total_ind.split()
                    total_invoice_amount = float(total_invoice_amount[-2].replace(",", ""))
            if "Departure" in i:
                date_time_obj=re.findall("([0-9]{2}\/[0-9]{2}\/[0-9]{2})+",i)[0]
                # date_time_obj = (i.strip().split(":")[1]).strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj, '%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room  :" in i or "Room :" in i or "Room No  " in i:
                room = i.split(":")
                roomNumber = room[-1].strip()
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST ID" in i:
                gstNumber = (i.split('Original')[0]).replace("GST ID","").replace("Original Bill Date","").strip()
            if ("Invoice No :" in i or "Invoice No  :" in i) and "Invoice Date" not in i:
                invoiceNumber = i.split(":")[-1].strip()
            # if "Company Name" in i:
            #     ind = raw_data.index(i)
            #     inv_no = raw_data[ind-1]
            #     invoiceNumber=inv_no.split()
            #     invoiceNumber = invoiceNumber[-1].strip()
            #     print(invoiceNumber,"/////////////")
            if "Guest Name" in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in spliti[-1]:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "[Add: udf.]" not in each["name"]  and "[NA Pkg. Trx]" not in each['name'] and "[NA P.Room]" not in each["name"] and "Room#" not in each["name"]: 
        ...
        company_code = {"code":"RBRC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RBRC-01","invoice_date":date_time_obj}
