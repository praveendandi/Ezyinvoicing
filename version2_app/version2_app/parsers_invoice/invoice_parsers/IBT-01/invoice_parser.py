...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBT-01")
        ...
        for each in items:
            if ("CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]) or "Allowance Room Charges" in each["name"]:
        ...
        company_code = {"code":"IBT-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBT-01","invoice_date":date_time_obj}
