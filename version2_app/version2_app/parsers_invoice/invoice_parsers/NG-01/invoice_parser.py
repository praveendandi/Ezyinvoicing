...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("NG-01")
        ...
        for i in raw_data:
            ...
            if "Company GSTIN  " in i or "Company GSTIN  :" in i or "GST ID   " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace("PrintedBy/On","").strip()
                gstNumber = gstNumber.replace("ConfirmationNo","").replace("Membership","").strip()
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    ...
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Accommodation" or each["name"]=="Group Rate Code-CS":
                        if ind+1 < len(items):
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] =guest['name'].replace("Membership","").replace("Departure","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"NG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"NG-01","invoice_date":date_time_obj}
