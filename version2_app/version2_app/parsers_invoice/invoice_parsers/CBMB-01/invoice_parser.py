...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CBMB-01")
        ...
        for i in raw_data:
            ...
            if "Confirmation No" in i or "Confirmation No " in i or "Confirmation No  " in i:
                ...
                conf_number = confirmation_number[1].replace(" ", "").replace("Timeprinted","")
            if "Total : INR" in i or "Total  :  INR" in i or "Total :" in i and ("Tax:" not in i):
            ...
            if "Departure :" in i or "Depart" in i or "Depart " in i:
                depatureDateIndex = i.index('Depart')
                date_time_obj = ':'.join(i[depatureDateIndex:].split(':')[1:10])[1:10]
                date_time_obj = date_time_obj.strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            # if "Bill Origination Date" in i or "Bill Origination Date:" in i:
            #     date_time_obj = i.split("Original Bill")[0].split(":")[-1].strip()#(i.split(":")[1]).strip()
            #     date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No :" in i or "Room No  :" in i:
                room = i.split(":")
                roomNumber = room[1].replace("Rate","").strip()
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST IN" in i or "GST IN     " in i or "GST IN      " in i or "GST IN " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("Page","").replace("BillNo","").strip()
            if "Bill No" in i or "Bill No " in i:
                invoiceNumber = i.split(":")[-1].lstrip("-")
                invoiceNumber = ''.join(filter(lambda i: i.isdigit(), invoiceNumber)).lstrip("0")
            ...
            if "Room No" in i or "Room No" in i or "Room No  :" in i:
                guestDeatils.append(i)
                # guestDeatils.append(raw_data[raw_data.index(i)+1])
                # guestDeatils.append(raw_data[raw_data.index(i)+2])
            if "Membership" in i:
                Membership = i.split(":")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "Tax:" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for index, i in enumerate(guestDeatils):
            # if "Charged To" in i:
            #     guest['name'] = i.split(':')[1].replace("FSSAI No.","")
            #     guest['name'] = guest['name'].strip()
            if "Room No  :" in i or "Room No :" in i:
                guest['name'] = i.split(':')[0].replace("Room No","")
                guest['name'] = guest['name'].strip()  
            if index == 0:
                guest['address1'] = i
            if index == 1:
        ...
        company_code = {"code":"CBMB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMB-01","invoice_date":date_time_obj}
