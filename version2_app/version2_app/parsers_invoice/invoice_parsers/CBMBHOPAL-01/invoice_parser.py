...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CBMBHOPAL-01")
        ...
        for i in raw_data:
            ...
            if "Bill Generation" in i:
                date_time_obj = i.split(":")[-1].strip()#(i.split(":")[1]).strip()
                # print(date_time_obj,"++++++++++++")
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No" in i:
                room = i.split(":")
                roomNumber = room[1].replace("Page","").strip()
            ...
            if "Room No :" in i:
                guestDeatils.append(raw_data[raw_data.index(i)])
        ...
        for index, i in enumerate(guestDeatils):
            ...
            if index == 1:
                guest['address1'] = i.split('Rate :')[0].strip()
            if index == 1:
                guest['address2'] = i.split('Rate :')[0].strip()
        ...
        guest['company_code'] = "CBMBHOPAL-01"
        ...
        company_code = {"code":"CBMBHOPAL-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMBHOPAL-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "A000-"+invoiceNumber
