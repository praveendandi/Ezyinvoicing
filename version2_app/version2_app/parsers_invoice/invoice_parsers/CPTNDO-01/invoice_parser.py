...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CPTNDO-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  :" in i and "Page :" not in i:
            ...
            if "GST ID" in i and "Hotel GST ID" not in i:
                ...
                gstNumber = gstNumber.replace("TAXTYPE","")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "Page :" not in i:
                ...
                if item['name'] == "Accommodation - Package":
                    # item['name'] = "Accommodation"
                    item['item_value'] = item['item_value'] - 400    
                    
                    newItem = copy.deepcopy(item)
                    newItem['name'] = "Edesia Food B/Fast Package"
                    newItem['item_value'] = 400
                    newItem['sort_order'] = itemsort+2
                    itemsort+=1
        ...
        company_code = {"code":"CPTNDO-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CPTNDO-01","invoice_date":date_time_obj}
        ...
        if gspApiDataResponse['success'] == True:
            if guest['invoice_type'] == 'B2B':
                ...
                calulateItemsApiResponse = calulate_items({'items':guest['items'],"invoice_number":guest['invoice_number'],"company_code":company_code['code'],"invoice_item_date_format":companyCheckResponse['data'].invoice_item_date_format,"sez":0,"state_code": ""})
                if calulateItemsApiResponse['success'] == True:
                    guest['invoice_file'] = filepath
                    if reupload == False:
