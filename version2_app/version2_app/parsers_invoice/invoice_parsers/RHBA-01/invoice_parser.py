...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RHBA-01")
        ...
        for i in raw_data:
            ...
            if "Bill  No. " in i or "Bill  No." in i:
                date_time_obj = (i.split(":")[0][:11]).strip()
            ...
            if "GST ID" in i or "GST ID  " in i:
                ...
                gstNumber = gstNumber.replace("Confirmation No.","").replace("Confirmation No","").strip()
            if "Bill  No. " in i or "Bill  No." in i:
            ...
            if len(check_date) > 0 and "." in split_str:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if val != "":
                        ...
                        # item_value = val[-2]
                        item_value = re.findall(r"([-0-9]+[,.\]+[0-9]+)",val[-2])[0]
                        item['item_value'] = float(item_value.replace(',', ''))
                    if index == 1:
        ...
        guest['invoice_number'] = invoiceNumber.replace(' ', '').lstrip("  ")
        ...
        company_code = {"code":"RHBA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RHBA-01","invoice_date":date_time_obj}
