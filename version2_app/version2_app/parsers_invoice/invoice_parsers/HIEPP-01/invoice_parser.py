...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HIEPP-01")
        ...
        for i in raw_data:
            ...
            if "Cashier" in i and "Signature" not in i:
                p = i.split(" ")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Page 1 of","")
                guest['name'] = guest['name'].replace("Page 1 of","")
            if index == 1:
                guest['address1'] = i
            if index == 2:
        ...
        company_code = {"code":"HIEPP-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIEPP-01","invoice_date":date_time_obj}
