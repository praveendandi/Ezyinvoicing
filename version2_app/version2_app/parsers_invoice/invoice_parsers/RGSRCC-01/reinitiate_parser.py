...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("RGSRCC-01")
        ...
        for i in raw_data:
            # print(i,"++++++++++++++++")
            if "Bill  No. " in i:
                date_time_obj = (i.split(":")[0]).replace("Bill  No.","").strip()
        ...
        company_code = {"code":"RGSRCC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RGSRCC-01","invoice_date":date_time_obj}
