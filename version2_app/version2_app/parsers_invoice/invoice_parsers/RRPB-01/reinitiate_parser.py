...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("RRPB-01")
        ...
        for i in raw_data:
            ...
            #     print_by = i.split(":")
            #     print_by = print_by[1].replace("Cashier","")
            if "Printed By / On" in i:
                p = i.split(":")
                new = p[1].replace(" ","")
        ...
        company_code = {"code":"RRPB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RRPB-01","invoice_date":date_time_obj}
