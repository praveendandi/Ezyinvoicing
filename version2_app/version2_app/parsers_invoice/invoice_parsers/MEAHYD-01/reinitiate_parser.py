...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("MEAHYD-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i:
                ...
                if "-" in total_invoice[-2]:
                    invoice_category = "Credit Invoice"
            if ("Departure" in i or "Departure   " in i) and "retention" not in i:
                date_time_obj = (i.split(":")[-1]).strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. :" in i or "Room  :" in i or "Room :" in i:
                room = i.split(":")
                roomNumber = room[-1].replace('"',"")
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "Company GSTIN" in i or "GST ID" in i and "Property Tax ID" not in i:
                gstNumber = i.split(' ')[-1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.rstrip("c").strip()
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            item["name"] = ((i[starting_index:ending_index]).split("[")[0].strip()).replace("  "," ")
                        else:
                            ending_index = i.find(item_value)
                            item["name"] = ((i[starting_index:ending_index]).strip()).replace("  "," ")
                            # item["name"] = ((i[starting_index:ending_index]).split("at")[0].strip()).replace("  "," ")
                    if len(val)>1:		
                        if 'SAC' in j:
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Room No." not in each['name'] and "Allow " not in each['name'] and "CHECK#" not in each['name'] and "[Add:" not in each['name']:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[0].replace("Room","").replace('"','').strip()
            if index == 1:
                guest['address1'] = i.split(':')[0].replace("Arrival","").strip()
            if index == 1:
                guest['address2'] = i.split(':')[0].replace("Arrival","").strip()
        ...
        company_code = {"code":"MEAHYD-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"MEAHYD-01","invoice_date":date_time_obj}
