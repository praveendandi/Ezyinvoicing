    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Room No." in i or "Room No" in i or "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
            if "GST ID" in i or "GSTIN ID  " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace('/', '').strip()
                # gstNumber = gstNumber.rstrip('TAXINVOICE')
                gstNumber = gstNumber.replace("(cid","")
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        print(starting_index,"KKKKKKKKKK")
                        if "~" in i:
                            ending_index = i.find("~")
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "March Getaway Sale" or each["name"] == "Package Rate" or each['name'] == "Bed and Breakfast" or each['name'] == "Room Only":
                        if ind+1 < len(items):
