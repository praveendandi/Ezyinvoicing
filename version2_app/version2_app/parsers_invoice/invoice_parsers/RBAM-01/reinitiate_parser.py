...
def reinitiateInvoice(data):
    try:
        ...
        for i in raw_data:
            # print(i,"**************")
            # if "CREDIT_INVOICE" in i or "COPY OF INVOICE CREDIT_INVOICE" in i:
            ...
            if "Total" in i and "INR" in i and "Auth" not in i:
            ...
            if "GST ID" in i or "GST ID  :" in i:
                ...
                # gstNumber = gstNumber.replace("ConfirmatiwonNo.","")
                # gstNumber = gstNumber.replace("Membership","").strip(".")
                gstNumber = gstNumber.replace("TAXINVOICE"," ").replace("CREDIT","")
                gstNumber = gstNumber.replace("CREDITINVOICE","").replace("CREDIT","")
                gstNumber= gstNumber.replace("TAX_INVOICE40199","")
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "VAT" not in each["name"] and  "Vat" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Accommodation":
                        if ind+1 < len(items):
