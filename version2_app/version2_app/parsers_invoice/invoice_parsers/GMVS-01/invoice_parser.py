...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("GMVS-01")
        ...
        for i in raw_data:
            # print(i,"+++++++++++")
            # if "Departure" in i:
            #     date_time_obj = (i.split(":")[1]).strip().split(" ")[0].strip()
            #     # print(date_time_obj,"+++++++++")
            ...
            if "Cust GST ID" in i or "Cust GST ID " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.", "").replace("Membership","").replace("ConfirmationNo","").strip()
            if "No." in i and "Bill" in i:
            ...
            if i in "Date Description Reference Debit Credit" or i.strip() in 'Date  Description  Reference  Debit  Credit':
                entered = True
            if 'CGST 6%=' in i:
                entered = False
            if 'Billing' in i:
        ...
        for index, i in enumerate(guestDeatils):
            ...
            if index==2:
                guest['address1'] = i
            if "Room No." in i:
                guest['address2'] = guestDeatils[guestDeatils.index(i)+1]
        ...
        company_code = {"code":"GMVS-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"GMVS-01","invoice_date":date_time_obj}
