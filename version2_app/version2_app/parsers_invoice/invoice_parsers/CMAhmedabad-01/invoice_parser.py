...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CMAhmedabad-01")
        ...
        for i in raw_data:
            ...
            if "Confirmation No :" in i or "Confirmation No  :" in i:
                ...
                conf_number = confirmation_number[-3].replace(" ", "").replace(":", "")
            ...
            if "Depart :" in i:
                ...
                date_obj = ':'.join(i[depatureDateIndex:].split(':')[1:])[1:]
                date_time_obj = re.findall("([0-9]{2}\-[0-9]{2}\-[0-9]{2})+",date_obj)[-1]
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No " in i or "Room  :" in i:
                room = i.split(":")
                roomNumber = room[1].replace("Page # ","")
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if ("GST ID " in i or "GSTIN                " in i or "GST IN        " in i or "GSTIN " in  i) and "Property GST ID  " not in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace("Page","")
                gstNumber = gstNumber.replace("Cashier","").strip()
            if "GSTIN                " not in i:
                if "Property GST ID  :" in i:
                    ind = raw_data.index(i)
                    gstNumber = raw_data[ind+1]
                    gstNumber = gstNumber.replace("Cashier","").strip()
            if "Invoice# :" in i or "Invoice No :" in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Room No","").replace("Room","").strip()
            # if "Room No :" in i or "Room  :" in i:
            #     guest['name'] = i.split('Room No')[0].strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        guest['company_code'] = "CMAhmedabad-01"
        ...
        company_code = {"code":"CMAhmedabad-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CMAhmedabad-01","invoice_date":date_time_obj}
