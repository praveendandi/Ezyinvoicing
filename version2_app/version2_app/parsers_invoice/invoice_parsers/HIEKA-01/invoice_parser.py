...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HIEKA-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "Total Value of Supply INR" not in i and "Total  Value  of Supply  INR" not in i:
                total_invoice = i.split(" ")
                total_invoice_amount = float(total_invoice[-2].replace(",", ""))
            if "Invoice Date" in i or "Invoice  Date" in i or "Invoice  Date" in i or "Invoice  Date" in i:
                # date_time_obj = (i.split(" ")[-1]).strip()
                date_time_obj=re.findall("\d{4}-\d{2}-\d{2}",i)[-1]
            ...
            if "GSTIN / UIN" in i or "GSTIN  /  UIN:" in i:
                ...
                gstNumber = (gstNumber.split("Departure")[0]).strip()
            # if "GSTIN / UIN" in i or "GSTIN  /  UIN:" in i:
            #     i = i.replace("GST ","")
            #     gstNumber = (i.split(':')[1]).split(" ")[1]
            # #     gstNumber = (gstNumber.replace("Arrival","")).strip()
            #     gstNumber = (gstNumber.split()).strip()
            #     print(gstNumber,"<<<<<<<<<<<<<<<<<<<<<")
                if len(gstNumber) == 15:
                    gstNumber = gstNumber
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        test_val=re.sub("[0-9]{6}|[0-9]{8}|[0-9]{4}|\#","", item["name"])
                        if test_val:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0: 
                ...
                guest['name'] = re.sub('[0-9]|[-:]',' ',guest['name']).replace('Departure','').strip()
                guest['name'] = guest['name'].replace("Page     of","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"HIEKA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIEKA-01","invoice_date":date_time_obj}
