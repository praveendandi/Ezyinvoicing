...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISNM-01")
        ...
        for i in raw_data:
            ...
            if "Guest Tax ID " in i or "Guest Tax ID  :" in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace("ConfirmationNo","").strip()
            ...
            if len(check_date) > 0 and "." in split_str:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 0 and len(val)>1:
                        item['date'] = j
                    if len(val)>1 and "Invoice" not in val:
                        item_value = val[-1]
        ...
        company_code = {"code":"IBISNM-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBISNM-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "INM-"+invoiceNumber
