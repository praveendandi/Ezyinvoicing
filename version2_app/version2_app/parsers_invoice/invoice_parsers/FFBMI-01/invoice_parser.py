...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FFBMI-01")
        ...
        for i in raw_data:
            ...
            if "Bill  No. :" in i:
                last_ind = raw_data.index(i)
                dateobj = raw_data[:last_ind]
                result = re.findall("([0-9]{2}\-[0-9]{2}\-[0-9]{2})+",str(dateobj))
                # result = ''.join(data)
                date_time_obj = datetime.datetime.strptime(result[0],'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if date_time_obj == "":
                if "Departure :" in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Room No.","").replace("Page","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"FFBMI-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FFBMI-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "FFI-"+invoiceNumber
