...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FMORB-01")
        ...
        for i in raw_data:
            ...
            # if "CREDIT TAX INVOICE" in i or "CREDIT TAX INVOICE" in i:
            ...
            if ("TAX INVOICE" in i or "CREDIT TAX INVOICE" in i  or "CREDIT TAX INVOICE" in i ) and ("Original Bill No." not in i and "COPY OF INVOICE" not in i):
                ind = raw_data.index(i)
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str and "IGST" not in i and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "VAT" not in i and "FAIRFIELD BY MARRIOTT GOA ANJUNA" not in i and " www.fairfield.marriott.com" not in i:
                item = dict()
                item_value = ""
        ...
        company_code = {"code":"FMORB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMORB-01","invoice_date":date_time_obj}
...
#     # invoiceNumber = invoiceNumber.lstrip("2")
#     invoiceNumber = invoiceNumber
#     return invoiceNumber    


def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "2222-"+invoiceNumber
    return invoiceNumber