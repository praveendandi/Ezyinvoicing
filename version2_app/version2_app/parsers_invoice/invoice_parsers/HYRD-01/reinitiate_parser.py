...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("HYRD-01")
        ...
        for i in raw_data:
            ...
            if "Printed By / On" in i:
                p = i.split(":")
                new = p[1].replace(" ","")
                print_by=re.sub(r'[0-9]{2}\-[A-Z]+\-\d+','',new)
        ...
        itemsort = 0
        for i in data:
            pattern = re.compile("^([0-9]{2}\-[a-zA-Z0-9]{3}\-[0-9]{2})+")
        ...
        company_code = {"code":"HYRD-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HYRD-01","invoice_date":date_time_obj}
