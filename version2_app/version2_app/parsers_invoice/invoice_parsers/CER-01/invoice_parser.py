...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CER-01")
        ...
        for i in raw_data:
            ...
            if "GST No  " in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace("PrintedBy/On","")
                gst_num= ' '.join(re.findall(r"[^\W]+", gstNumber))
                gstNumber = gst_num.replace("Printed By / On","").strip()
        ...
        company_code = {"code":"CER-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CER-01","invoice_date":date_time_obj}
