...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i and "received" not in i:
            ...
            if date_time_obj == "":
                if "Departure :" in i:
                    ...
                    date_time_obj = ':'.join(i[depatureDateIndex:].split(':')[1:])[1:].replace("i","")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        item_value = val[-1]
                        item['item_value'] = item_value.replace('n','')
                        item['item_value'] = float(item['item_value'].replace(',', ''))
                        # item['item_value'] = float(item['item_value'].replace('n',''))
                    # else:
                    # 	item_value = val[-2]
        ...
        for ind, each in enumerate(items):
            ...
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                    if each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "March Getaway Sale":
                        if ind+1 < len(items):
