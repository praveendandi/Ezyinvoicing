...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("SJB-01")
        ...
        for i in raw_data:
            ...
            if "GST ID   " in i or "GST ID  " in i:
                ...
                gstNumber = gstNumber.replace("Arrival","")
        ...
        for i in data:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        # item_value = val[-2]
                        item_value = re.findall(r"([-0-9]+[,.\]+[0-9]+)",val[-2])[0].strip()
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            remove_special_characters = re.sub(r'([0-9]|\.|\,|\-)+','',item["name"])
                            item["name"] = ''.join(remove_special_characters).strip()
                        else:
                            ending_index = i.find("~")
                            item["name"] = ((i[starting_index:ending_index]).strip()).replace(" "," ")
                            remove_special_characters = re.sub(r'([0-9]|\.|\,|\-)+','',item["name"])
        ...
        for ind, each in enumerate(items):
            if  "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Central" not in each["name"] and "State" not in each["name"]:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Departure","")
        ...
        company_code = {"code":"SJB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"SJB-01","invoice_date":date_time_obj}
