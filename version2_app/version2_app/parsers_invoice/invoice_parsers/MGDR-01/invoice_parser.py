...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            # print(i,"+++++++++++++++++")
            if "Confirmation No. " in i or "aConfirmation No. " in i or "Confirmation No " in i:
            ...
            # if "COPY OF INVOICE" in i or "INVOICE" in i:
            #     dateind = raw_data.index(i)
            #     date_time_obj = raw_data[dateind].strip().split(" ")[-1]
            ...
            if "Bill  No. :" in i:
                ...
                date_time_obj = dateobj.replace("COPY OF INVOICE","").replace("INVOICE","").strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if date_time_obj == "":
                if "Departure :" in i:
            ...
            if "Room No." in i or "Room No. :" in i:
                room = i.split(":")
                roomNumber = room[-1]
            if "GST ID" in i or "GST ID  " in i or "Cust GST ID " in i and "Membership" not in i:
                ...
                gstNumber = gstNumber.replace("Membership","").replace("Memlbership","").strip()
            ...
            if "Guest Name" in i:
                guestDeatils.append(i)
            if "Membership" in i or "Membership " in i:
                Membership = i.split(":")
