...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("MHY-01")
		...
		for i in raw_data:
			...
			if "Total in INR" in i and "Totale" not in i and "PAYMENT" not in i:
			...
			if "Departure" in i:
				...
				date_time_obj = datetime.datetime.strptime(date_time_obj, '%d-%b-%y %H:%M:%S').strftime("%d-%b-%y %H:%M:%S")
			# if "Departure" in i:
			...
			if "Room     :" in i:
				room = i.split(":")
				roomNumber = room[-1].strip().replace("PX", "")
		...
		for i in data:
			...
			if len(check_date) > 0:
				...
				for index, j in enumerate(i.split(' ')):
					...
					# 	item_value = val[-2]
					# 	item['item_value'] = float(item_value.replace(',', ''))
					if index == 1 and len(val)>1 and "Pandey" not in i:
						starting_index = i.index(j)
						if "~" in i:
							item_start_index=i.find("~")
							item_desc = i[item_start_index:]
							ending_index = item_desc.find(item_value)
							ending_index = item_start_index+ending_index
							item["name"] = ((i[item_start_index+1:ending_index]).strip()).replace("  "," ")
						else:
		...
		company_code = {"code":"MHY-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"MHY-01","invoice_date":date_time_obj}
