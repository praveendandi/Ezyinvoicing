...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RCP-01")
        ...
        for i in raw_data:
            if "Bill  No." in i:
                date_obj = i.split(":")[0].replace("Bill  No.","").strip()
                inv_date = datetime.datetime.strptime(date_obj, '%m/%d/%y')
            ...
            if "Bill  No." in i:
                ...
                invoiceNumber = invoiceNumber.rstrip("-")
            if "Bill To" in i:
            ...
            if "Membership" in i:
                ...
                print(membership,"MERSvvvvvvvvvvvvvvvv")
            if "Printed By / On" in i:
                p = i.split(":")
                if len(p) > 2:
                    print_by = p[2].strip().split(" ")[0]
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "Page" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        company_code = {"code":"RCP-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RCP-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = invoiceNumber.replace("7392","")
    invoiceNumber = "7392-" + invoiceNumber
