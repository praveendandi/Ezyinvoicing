...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i:
                ...
                if match:
                    ...
                    if total_invoice_amount == "":
                        ...
                        if total_invoice_amount>-1 and total_invoice_amount<1:
            ...
            if "Company GSTIN  :" in i or "GST ID   :" in i or "GST ID   " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
        ...
        for i in original_data:
            ...
            if len(check_date) > 0 and  "." in split_str and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "VAT" not in i:
                item = dict()
                item_value = ""
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest["name"] = guest["name"].replace("Rate","").replace("Membership","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
