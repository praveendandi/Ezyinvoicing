...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("CBMG-01")
		...
		for i in raw_data:
			...
			if ("GST ID " in i or "GST ID   :" in i) and "Hotel GST ID"  not in i:
		...
		for index, i in enumerate(guestDeatils):
			print(i, index)
			if index == 0:
				guest['name'] = i.split(':')[0].replace('Room No','').strip()
			if index == 1:
		...
		company_code = {"code":"CBMG-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMG-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "9318"+invoiceNumber
