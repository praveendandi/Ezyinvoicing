...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("HIGC-01")
        ...
        for i in raw_data:
            ...
            if "Bill  No. " in i:
                date_time_obj = i.split(":")[0].replace("Bill  No. ","").replace("  ","").strip()
        ...
        company_code = {"code":"HIGC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIGC-01","invoice_date":date_time_obj}
