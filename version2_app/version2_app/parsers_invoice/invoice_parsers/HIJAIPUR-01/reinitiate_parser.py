...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("HIJAIPUR-01")
        ...
        for i in raw_data:
            ...
            #     date_time_obj=raw_data[index_val].replace("Original Bill  :","")
            #     date_time_obj = datetime.datetime.strptime(date_time_obj.strip(),'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
        ...
        guest['company_code'] = "HIJAIPUR-01"
        ...
        company_code = {"code":"HIJAIPUR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIJAIPUR-01","invoice_date":date_time_obj}
