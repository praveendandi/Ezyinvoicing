...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("FBMB-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "Total Net Amount(INR)" not in i:
            ...
            #     gstNumber = gstNumber.replace("Confirmation No.","")
            #     gstNumber=gstNumber.strip()
            #     dateind = raw_data.index(i)
            #     date_time_obj = raw_data[dateind+1].strip()
            #     date_time_obj=date_time_obj.split(":")[-1].strip()
            ...
            if "GST ID  :" in i:    
                ...
                try:
                    date_time_obj = datetime.datetime.strptime(dateobj, '%d-%m-%y')
                except ValueError:
                    date_time_obj = datetime.datetime.strptime(dateobj,'%d-%b-%y') 
                date_time_obj = date_time_obj.strftime('%d-%b-%y %H:%M:%S')
            if "GST ID  " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber=gstNumber.strip()
            ...
            if "GST ID  " in i:
                ind = raw_data.index(i)
                guest_name = raw_data[ind+2]
                guestDeatils.append(guest_name)
            if "Membership" in i:
                Membership = i.split(":")
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Room charges" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "Bed and Breakfast" or each["name"] == "Accommodation charges" or each["name"] == "Accommodation":
                        if ind+1 < len(items):
        ...
        company_code = {"code":"FBMB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FBMB-01","invoice_date":date_time_obj}
