...
def reinitiateInvoice(data):
    ...
    companyCheckResponse = check_company_exist("MARRIOTINDORE-01")
    ...
    for i in raw_data:
        print(i,"+++++++++++++++++")
        if "TAX BILL" in i:
            nextline = raw_data.index(i)
            date_time_obj = raw_data[nextline+1].split(":")[0].strip()
        ...
        if  "GST ID" in i or "GST ID  " in i and ("Pan No" not in i and "Hotel" not in i):
    ...
    guest['company_code'] = "MARRIOTINDORE-01"
    ...
    company_code = {"code":"MARRIOTINDORE-01"}
    error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"MARRIOTINDORE-01","invoice_date":date_time_obj}
