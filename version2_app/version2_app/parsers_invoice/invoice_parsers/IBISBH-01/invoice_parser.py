...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISBH-01")
        ...
        for i in raw_data:
            ...
            # if ("Total  INR" in i  or "Total          INR" in i) and ("Total in words:" not in i):
            ...
            if "Room No. :" in i or "Room No." in i or "Room No" in i:
                if ":" in i:
                    room = i.split(":")
                    roomNumber = room[-1]
                    roomNumber = re.sub(r'[^a-zA-Z0-9\s]',"", roomNumber)
                else:
                    room = i.split(" ")
                    roomNumber = room[1]
                    roomNumber = re.sub(r'[^a-zA-Z0-9\s]',"", roomNumber)
            if "Guest Tax ID :" in i or "Guest Tax ID  " in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace("Membership","")
            if "Bill No. " in i:
        ...
        company_code = {"code":"IBISBH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBISBH-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "ICCC-"+invoiceNumber
