...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("SSBWHI-01")
        ...
        for i in raw_data:
            # print(i,"//////////////////////")
            if "CREDIT TAX INVOICE" in i and total_invoice_amount == "":
            ...
            if "Total amount:" in i:
                ...
                if "-" not in str(total_invoice_amount) and invoice_category == "Credit Invoice":
                    invoice_category = "Tax Invoice"
            if "Departure date :" in i and date_time_obj == "":
            ...
            if "Room no. :" in i:
                room = i.split(":")
                roomNumber = room[-1].strip().replace(" ","")
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "Invoice no." in i:
                ind = raw_data.index(i)
                gstNumber = raw_data[ind-1]
                gstNumber = gstNumber.replace("CREDIT TAX INVOICE","")
                gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.replace("/Page","").strip()
            if "Invoice no." in i:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            item["name"] = result.replace("-","").strip()
                        else:
        ...
        if companyCheckResponse['data'].pms_information_invoice_for_payment_qr == "Yes" and informationInvoice == "Yes":
            if informationInvoice == "Yes":
                ...
                socket = frappe.publish_realtime("custom_socket", {'message':'Information Invoice Created','data':information_data,"company":"SSBWHI-01"})
            ...
            if informationInvoice == "No":
                ...
                company_code = {"code":"SSBWHI-01"}
                error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"SSBWHI-01","invoice_date":date_time_obj}
