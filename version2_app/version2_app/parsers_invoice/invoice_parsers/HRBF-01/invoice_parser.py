...
def file_parsing(filepath):
    ...
    try:
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST 18%" != each["name"] and "IGST 12%" != each["name"]:
        ...
        if len(gstNumber) < 15 and len(gstNumber)>0:
            ...
            return {"success":False,"message":"Invalid GST "+gstNumber}
