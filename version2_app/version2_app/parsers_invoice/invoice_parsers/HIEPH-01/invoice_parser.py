...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HIEPH-01")
        ...
        for i in raw_data:
            ...
            if date_time_obj == "":
                if "Departure" in i:
                    ...
                    depatureDate= "".join(re.findall("[0-9]{4}\-[0-9]{2}\-[0-9]{2}",depatureDateIndex))
                    date_time_obj = datetime.datetime.strptime(depatureDate,'%Y-%m-%d').strftime('%d-%b-%y %H:%M:%S')
            ...
            # if "Room No." in i:
            #     guestDeatils.append(i)
            if "GUEST NAME:" in i or "GUESTNAME" in i or "GUEST NAME:" in i:
                guestDeatils.append(i)
            if "Membership" in i:
                Membership = i.split(":")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "IGST" not in i and ("CGST/SGST" in i or ("CGST" not in i and "SGST" not in i)):
                item = dict()
                item_value = ""
                dt = i.strip()
                items_split = dt.split(" ")
                if len(items_split) > 2 and "." in items_split[-1]:
                    item['date'] = items_split[0]
                    item['item_value'] = float(items_split[-1].replace(",",""))
                    if len(items_split) == 3:
                        item["name"] = items_split[1].strip()
                    else:
                        item["name"] = (' '.join(items_split[1:-2])).strip()
                    if "#" in item["name"] and "##" not in item["name"]:
                    ...
                    #     item["name"] = item["name"][:name_index]
                    if "Transfer Debit" in item["name"] or "TransferDebit" in item["name"]:
                        if "(Amt" in i:
                            name_index = item["name"].rindex("(Amt")
                            item["name"] = item["name"][:name_index]
                    if "Transfer Credit" in item["name"] or "TransferCredit" in item["name"]:
                        item["name"] = "Transfer Credit"
                    if "Miscellaneouswith CGST/SGST@12%" in item["name"]:
                        item["name"] = "Miscellaneouswith CGST/SGST@12%"
                    if "Miscellaneousw/oServiceTax" in item["name"]:
                        item["name"] = "Miscellaneousw/oServiceTax"
        ...
        for each in items:
            if "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and ("CGST/SGST" in each["name"] or ("CGST" not in each["name"] and "SGST" not in each["name"])):
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                if "Page" in i:
                    guest_index = i.index("Page")
                    guest['name'] = i[0:guest_index]
                # if "Room No.":
                #     guest_index = i.index("Room No.")
                #     guest['name'] = i[0:guest_index]
                if "Page 1 of" in i:
                    guest['name'] = i.split('Page 1 of')[0] 
                if "Page 1 of" in i:
                    guest['name'] = i.split('Page 1 of')[0]  
                # print(guest,i)    
        ...
        company_code = {"code":"HIEPH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIEPH-01","invoice_date":date_time_obj}
