...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("CBMKA-01")
		...
		for i in raw_data:
			...
			if "Total Amount with Taxes" in i:
			...
			if "INVOICE on" in i or "INVOICE on" in i:
				invoice_date = i.index('INVOICE on')
				date_time_obj = (i[invoice_date:])
				date_time_obj = re.search(r'\b\d{2}-\d{2}-\d{2}\b', date_time_obj)
				date_time_obj = date_time_obj.group(0)
				inv_date = datetime.datetime.strptime(date_time_obj, '%d-%m-%y')
			...
			if date_time_obj == "":
				if "Departure :" in i:
					...
					inv_date = datetime.datetime.strptime(date_time_obj, '%d-%m-%y %H:%M')
					date_time_obj = inv_date.strftime("%d-%b-%y %H:%M:%S")
			if "Room No. :" in i:
				if ":" in i:
					room = i.split(":")
				...
				# # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
			if "GST NO" in i:
				gstNumber = i.split('Invoice No')[0].replace(" ","").replace("GSTNO","")
		...
		for index, i in enumerate(guestDeatils):
			if "Confirmation No" in i:
				guest_name_val= i.split("Confirmation")
				guest['name']  = guest_name_val[0].strip()
			if "Arrival" in i:
				guest['address1'] = i.split("Arrival")[0].strip()
			if index == 1:
				guest['address2'] = i
		...
		company_code = {"code":"CBMKA-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMKA-01","invoice_date":date_time_obj}
