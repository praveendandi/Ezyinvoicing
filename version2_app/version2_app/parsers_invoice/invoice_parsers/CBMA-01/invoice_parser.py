...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CBMA-01")
        ...
        for i in raw_data:
            ...
            if ("Total" in i and "INR" in i) and ("BILL" not in i):
            ...
            if "Room  :" in i or "Room :" in i or "Room No :" in i:
                ...
                roomNumber = room[1].replace("Page#","").replace("Page #","").strip()
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "Guest Name" in i:
                guestDeatils.append(i)
            if "GST ID " in i or "GST ID" in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("OriginalBill","").strip()
            if "Invoice# :" in i:
            ...
            #     guestDeatils.append(i)
            if "Loyalty No" in i:
                Membership = i.split("Loyalty No")[-1]
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "BILL" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        company_code = {"code":"CBMA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMA-01","invoice_date":date_time_obj}
