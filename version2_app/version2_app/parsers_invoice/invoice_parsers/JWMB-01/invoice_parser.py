...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("JWMB-01")
		...
		for i in raw_data:
			...
			# if "TAX INVOICE" in i or "CREDIT INVOICE" in i:
			...
			# if "Room  :" in i:
			# 	date_time =i.split(" ")[0]
			# 	if len(date_time)>0 and date_time != 'Room':
			# 		date_time_obj = datetime.datetime.strptime(date_time,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
			# if "Departure" in i or "Departure   " in i:
			# 	date_time = (i.split(":")[-1]).strip()
			# 	if len(date_time)>0:
			# 		date_time_obj = datetime.datetime.strptime(date_time,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
		...
		for each in items:
			if each["name"] != "":
				if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "UTGST" not in each["name"] and "[Add: udf.]" not in each["name"] and "[" not in each["name"]:
		...
		company_code = {"code":"JWMB-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JWMB-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "BLRJW"+invoiceNumber
