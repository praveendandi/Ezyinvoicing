...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("CMNM-01")
		...
		for i in raw_data:
			...
			if "Bill  No. :" in i:
				date_time_obj = (i.split(" ")[0]).strip()
				date_time_obj = datetime.datetime.strptime(date_time_obj,'%m/%d/%y').strftime('%d-%b-%y %H:%M:%S')
			if "Room No. :" in i:
				room = i.split(":")[1]
				roomNumber = re.findall(r'\d+', room)[0] if len(re.findall(r'\d+', room))> 0 else 0
			if "GST ID  :" in i and "Hotel GST ID"  not in i:
				gstNumber = i.split(':')[1].replace(' ', '')
				gstNumber = gstNumber.replace("Page","").replace("ConfirmationNo","").replace("ConfirmationNo.","")
			if "Bill  No. :" in i:
			...
			if len(check_date) > 0 and "." in split_str:
				...
				for index, j in enumerate(i.split(' ')):
					...
					if len(val)>1:
						...
						item_value = re.findall(r"([-0-9]+[,.\]+[0-9]+\d)",remove_characters)[-1]
		...
		for each in items:
			if each["name"] != "":
				if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "[" not in each["name"]:
		...
		for index, i in enumerate(guestDeatils):
			print(i, index)
			if index == 0:
				guest['name'] = i.split(':')[1].replace('Room No','')
				guest['name'] = guest['name'].replace("Departure","").strip()
			if index == 1:
				guest['address1'] = i.replace("Loyalty No :","")
			if index == 1:
				guest['address2'] = i.replace("Loyalty No :","")
		...
		company_code = {"code":"CMNM-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CMNM-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = invoiceNumber.replace("CYNM","")
    invoiceNumber = "CYNM-"+invoiceNumber
