...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("TABG-01")
        ...
        for i in raw_data:
            ...
            #     # print(total_invoice_amount,"totttttttttt")
            if "GRAND TOTAL" in i:
                total_invoice = i.split()
                total_invoice_amount = float(total_invoice[-1].replace(",", ""))
            ...
            if "Bill Date  :" in i:
                ...
                # depatureDate= "".join(re.findall("[0-9]{2}\/[0-9]{2}\/[0-9]{4}",depatureDateIndex))
            ...
            if "GST NO     :" in i:
                ...
                gstNumber = gstNumber.replace("Arrv.Date","").strip()
            if "Bill Number:" in i:
        ...
        itemsort = 0
        for i in data:
            if "Arrv.Date  " not in i and  "Depr.Date  " not in i and "Bill Date  " not in i:
                i = i.replace("  "," ")
                i = i.strip()
                ...
                if len(check_date) > 0:
                    ...
                    for index, j in enumerate(i.split(' ')):
                            ...
                            if item_value.replace(".","").replace(',', '').isdigit():
                                ...
                                for k in item_values:
                                    if k.replace(".","").replace(',', '').isdigit():
                                        item_value = k
                                        item['item_value'] = float(k.replace(',', ''))
                                        break       
                        # else: 
                        # 	item_value = val[-2]
                        # 	item['item_value'] = float(item_value.replace(',', ''))
                        if index == 2 and len(val)>1:
                            ...
                            if test_val:
                                clean_text = ' '.join(re.findall(r"[^\W]+", test_val))
                                item["name"]=clean_text.strip()
                            else:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0: 
                ...
                guest['name'] = re.sub('[0-9]|[-:,]',' ',guest['name']).replace("Bill Number","").strip()
                # guest['name'] = i.replace("Departure","").strip(" ")
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"TABG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TABG-01","invoice_date":date_time_obj}
