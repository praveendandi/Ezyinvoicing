...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TAU-01")
        ...
        for i in raw_data:
            ...
            if "GST ID   " in i or "GST ID   :" in i or "GST ID   " in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.", "").replace("PrintedBy/On","").replace("MembershipNo","").strip() 
            if "Invoice No. " in i:
        ...
        company_code = {"code":"TAU-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TAU-01","invoice_date":date_time_obj}
