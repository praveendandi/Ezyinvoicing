...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RBCC-01")
        ...
        for i in raw_data:
            ...
            if "GST ID" in i or "GST ID  " in i or "GST ID   " in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace('ConfirmationNo','')
                gstNumber = gstNumber.replace("-","").replace("/","")
                gstNumber = gstNumber.strip()
            if "Bill  No. :" in i or "Bill  No." in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "").replace("-","").lstrip("0")
            if "Bill To" in i:
        ...
        for i in data:
            if "Page :" not in i:
                ...
                check_date=[]
            before_index=data.index(i)-1
            if len(check_date) > 0 and "Hotel GST" not in data[before_index]:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Room Only" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "March Getaway Sale":
                        if ind+1 < len(items):
        ...
        company_code = {"code":"RBCC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RBCC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "RBFL"+invoiceNumber
