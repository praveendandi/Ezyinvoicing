...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HICOCHIN-01")
        ...
        for i in raw_data:
            ...
            #     original_bill = i.index('Original Bill')
            #     date_time_obj = ':'.join(i[original_bill:].split(':')[1:])[1:]
            #     inv_date = datetime.datetime.strptime(date_time_obj, '%d-%m-%y %H:%M:%S')
            #     date_time_obj = inv_date.strftime("%d-%m-%y %H:%M:%S").replace("TAX INVOICE213837","").strip()
            if "Original Bill  :" in i:
                original_bill = i.split(" ")
                date_time_obj = original_bill[-1]
                date_time_obj = datetime.datetime.strptime(date_time_obj.strip(),'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
                # date_time_obj = date_time_obj.strftime("%d-%m-%y %H:%M:%S").replace("TAX INVOICE213837","").strip()
            # if "Departure :" in i:
            ...
            #         date_time_obj=(raw_data[index_val]).replace("Original Bill  :","").replace("TAX INVOICE213837","")
            #         date_time_obj = datetime.datetime.strptime(date_time_obj.strip(),'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            #     else:
            #         index_val = raw_data.index(i)+1
            #         date_time_obj=(raw_data[index_val]).replace("Original Bill  :","").replace("TAX INVOICE213837","")
            ...
            if "GST ID" in i:
                ...
                gstNumber = gstNumber.replace("TAXINVOICE","").replace("CREDIT","").strip()
            if "Bill  No." in i:
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Accommodation Charge":
                        if ind+1 < len(items):
        ...
        guest['company_code'] = "HICOCHIN-01"
        ...
        company_code = {"code":"HICOCHIN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HICOCHIN-01","invoice_date":date_time_obj}
