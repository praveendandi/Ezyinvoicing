...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("HIENDIA-01")
        ...
        for i in raw_data:
            ...
            #     date_time_obj= i.split(" ")[-1]
                # index_val = raw_data.index(i).split(" ")[-1]
            ...
            # if "Invoice Date" in i:
            #     date_time_obj = (i.split(":")[1]).strip()
            #     date_time_obj = date_time_obj.replace("T","").strip()
            #     date_obj = re.findall("([0-9]{4}\-[0-9]{2}\-[0-9]{2})+", date_time_obj)
            #     date_time_obj = datetime.datetime.strptime(date_obj[0],'%Y-%m-%d').strftime('%d-%b-%y %H:%M:%S')
            #     print(date_time_obj,"inv dateeeeeeeeee")
            if "GSTIN / UIN :" in i:
                room = i.split("Room No.")
                roomNumber = room[-1].strip()
            if roomNumber == "":
                if "Room No." in i and "Room No. 21" not in i and "Room No.10" not in i:
                    room = i.split(".")
                    roomNumber = room[-1].strip()
            ...
            if "GSTIN / UIN:" in i or "GSTIN / UIN :" in i:
                ...
                if len(gstNumber) == 15:
                    ...
                    gstNumber = ""
            if "Invoice No." in i or "Bill to Invoice No." in i:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        test_val=re.findall("[0-9]{6}|[0-9]{8}|[0-9]{4}", item["name"])
                        if test_val:
                            item["name"]=item["name"].replace(test_val[-1],"").replace("<","").replace(">","")
                        else:
        ...
        for index, i in enumerate(guestDeatils):
            ...
            if index == 0:
                if "Conf. No." in i:
                    ...
                    ind = i.index("Cashier")
                elif "Print Date" in i:
                    ind = i.index("Print Date")
                else:
                    ind = i.index("Folio No.")
                guest['name'] = i[:ind].replace("Guest Name : ","").strip()
            if index == 3:
                ind = i.index("Arrival")
                guest['address1'] = i[:ind].replace("Address :","").strip()
            if index == 3:
                ind = i.index("Arrival")
                guest['address1'] = i[:ind].replace("Address :","").strip()
        ...
        company_code = {"code":"HIENDIA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIENDIA-01","invoice_date":date_time_obj}
