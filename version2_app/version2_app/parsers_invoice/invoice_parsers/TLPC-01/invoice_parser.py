...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TLPC-01")
        ...
        for i in raw_data:
            ...
            if "FSSAI License No " in i or "FSSAI License No:" in i or "FSSAI License No" in i:
            ...
            if "GSTIN ID  " in i or "GSTIN ID :" in i or "GSTIN ID " in i or "GSTIN ID  " in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace("/","")
                gstNumber = gstNumber.replace("ConfirmationNo","")
                gstNumber = gstNumber.replace("ConfirmationNo. ","").replace(".","").strip()
        ...
        company_code = {"code":"TLPC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TLPC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "TLPC"+invoiceNumber
