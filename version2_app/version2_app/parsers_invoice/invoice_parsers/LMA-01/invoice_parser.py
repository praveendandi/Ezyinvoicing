...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("LMA-01")
        ...
        for i in raw_data:
            ...
            if "GST ID  :" in i and "Hotel GST ID  " not in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","").strip()
                gstNumber = gstNumber.replace("HotelGSTID","").strip()
        ...
        company_code = {"code":"LMA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LMA-01","invoice_date":date_time_obj}
