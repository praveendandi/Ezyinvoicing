...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FPSCOMR-01")
        ...
        for i in raw_data:
            # print(i,"++++++++++++++")
            if "INFORMATION INVOICE" in i or "INFORMATION INVOICE" in i:
            ...
            if "GST ID  " in i or "GST ID  :" in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace("/","").replace("Bill Date","")
                gstNumber = gstNumber.replace("TAXINVOICE","").replace("HotelGSTID","")
                gstNumber = gstNumber.replace("INVOICE","").strip()
            if "Bill  No. " in i or "Bill  No. " in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
                invoiceNumber = invoiceNumber.lstrip("0").strip()
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "Bed and Breakfast" or each["name"] == "Accommodation Charges" or each["name"] == "Accommodation":
                        if ind+1 < len(items):
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("..","").replace("Departure","")
                guest['name'] = guest['name'].replace("Printed By / On","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"FPSCOMR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FPSCOMR-01","invoice_date":date_time_obj}
