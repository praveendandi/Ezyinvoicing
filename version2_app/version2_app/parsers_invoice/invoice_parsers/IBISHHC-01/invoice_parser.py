...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISHHC-01")
        ...
        for i in raw_data:
            # print(i,"++++++++++++++++++++")
            i = i.strip()
            if "Conf. No. :" in i or "Confirmation No. :" in i or "Confirmation No " in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0 and  "." in split_str and "Aparna" not in i and "BANJARA" not in i:
                item = dict()
                item_value = ""
                for index, j in enumerate(i.split(' ')):
                    val = dt.split(" ")
                    if index == 0 and len(val)>1 and "Room No." not in i:
                        item['date'] = j
                    if len(val)>1 and "Room No." not in i:
                        item_value = val[-1]
                        item['item_value'] = float(item_value.replace(',', ''))
                    if index == 1 and len(val)>1 and "Room No." not in i:
        ...
        company_code = {"code":"IBISHHC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBISHHC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "IHHC-"+invoiceNumber
