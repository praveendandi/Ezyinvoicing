...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("SBC-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i:
                ...
                if "-" in total_invoice[-2]:
                    invoice_category = "Credit Invoice"	
            if "Invoice Generation Date :" in i:
            ...
            if "Bill  No. " in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "").replace("-","").replace(".","")
        ...
        company_code = {"code":"SBC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"SBC-01","invoice_date":date_time_obj}
