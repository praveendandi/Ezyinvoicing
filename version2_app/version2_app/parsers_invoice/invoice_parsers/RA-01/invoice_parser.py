...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RA-01")
        ...
        for i in raw_data:
            ...
            if "GST ID  " in i and gstNumber =="" and "GST No." not in i :
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            # item["name"] = item["name"].replace("SUKRITI #427=>NSICON 2022 #9027","")
                            # item["name"] = item["name"].strip()  
                            # txt = "IRD Food - Dinner SUKRITI #427=>NSICON 2022 #9027"
                            # if "#" in item["name"]:
                            #     value1 = item["name"][:item["name"].find("#")]
                            #     item["name"] = value1.strip()
                        else:
                            ending_index = i.find(item_value)
                            item["name"] = ((i[starting_index:ending_index]).strip()).replace("  "," ")
                            result = re.sub(r'[0-9]+', '', item["name"])
                            item["name"] = result.replace("X","").replace("/","").strip()
                            if "#" in item["name"]:
                                value1 = item["name"][:item["name"].find("#")]
                                item["name"] = value1.replace("SUKRITI","").strip()
                    if len(val)>1:		
                        if 'SAC' in j:
        ...
        company_code = {"code":"RA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RA-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = invoiceNumber.replace("2019","")
