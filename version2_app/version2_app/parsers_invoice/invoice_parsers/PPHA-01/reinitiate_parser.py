...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("PPHA-01")
        ...
        for i in raw_data:
            ...
            if "Reservation #" in i or "Reservation No" in i:
                if "Reservation #" in i:
                    ...
                    confirmation_number = i.split(":")
                    conf_number = confirmation_number[1].replace(" ", "")
                    conf_number = conf_number.replace("BillNo","")
            if "Net Amount" in i:
            ...
            if "Bill Date" in i or "Bill Date & Time" in i:
                ...
                invoiceNumber = (str(date_time)+"-"+invoiceNumber).lstrip("0")
            if "Room No" in i:
                room = i.split(" ")
                roomNumber = room[-1]
            if "Company GST No." in i or "Guest GSTN #" in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("Plan","")
                gstNumber = gstNumber.replace("GSTNBill#","")
            if ("Bill Number :" in i or "Bill Number  :" in i or "Bill No :" in i)  and "GSTN Bill No :" not in i: 
        ...
        company_code = {"code":"PPHA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"PPHA-01","invoice_date":date_time_obj}
