...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RANO-01")
        ...
        for i in raw_data:
            ...
            if "Hotel GST ID  :" in i:
                ...
                date_time_obj = datetime.datetime.strptime(raw_data[ind+1], '%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
        ...
        for i in data:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    # if "~" in i:
                    #     # if "Room Charge" not in i or "TCK Package Foods" not in i:
                    #     start_index = i.find("~")
                ...
                item_ignore_count=0
                # print(item)
                if item !={} and 'name' in item.keys():
                    pattern_date = re.compile("^([0-9]{2}\/[0-9]{2}\/[0-9]{2})+")
                    item_date = re.findall(pattern_date, item["name"])
                    if len(item_date) > 0:
                        if item_ignore_count is 0:
                            item_ignore_count=1
                            pattern_date = re.compile("^([0-9]{2}\/[0-9]{2}\/[0-9]{2})+")
                            check_date = re.findall(pattern_date, items[-1]["refer"])
                            if len(check_date)==0:
                                items.pop(-1)
                        item["refer"]=item["name"]
                        item["name"]=item["name"].replace(item_date[0],"").strip()
                    else:
                        item["refer"]=item["name"]
                    if len(item)>1:
                        items.append(item)
        ...
        company_code = {"code":"RANO-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RANO-01","invoice_date":date_time_obj}
