...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TWPR-01")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Departure","")
        ...
        if companyCheckResponse['data'].pms_information_invoice_for_payment_qr == "Yes" and informationInvoice == "Yes":
            if informationInvoice == "Yes":
                ...
                socket = frappe.publish_realtime("custom_socket", {'message':'Information Invoice Created','data':information_data,"company":"TWPR-01"})
            ...
            if informationInvoice == "No":
                ...
                company_code = {"code":"TWPR-01"}
                error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TWPR-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "JAIPU-"+invoiceNumber
