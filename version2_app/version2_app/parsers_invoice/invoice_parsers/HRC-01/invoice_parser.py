...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HRC-01")
        ...
        for i in raw_data:
            ...
            if "GST ID" in i and "PAN No" not in i and "Property" not in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("COPYOFINVOICE","")
                print("------------------------",gstNumber)
            if "Bill  No." in i:
        ...
        company_code = {"code":"HRC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HRC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "HRC"+invoiceNumber
