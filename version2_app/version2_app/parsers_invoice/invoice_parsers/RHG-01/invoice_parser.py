...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RHG-01")
        ...
        for i in raw_data:
            # print(i,"**************")
            if "Confirmation No " in i or "Confirmation No. " in i or "Conlfirmation No " in i:
            ...
            if "GSTIN ID  " in i:
                ...
                gstNumber = gstNumber.replace("-","").replace("/","").replace(".","")
                gstNumber = gstNumber.replace("ConfirmationNo","").replace("ConlfirmationNo","")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] =guest['name'].replace('Departure','').replace("Arrival","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"RHG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RHG-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = ""+invoiceNumber.lstrip("0001")
