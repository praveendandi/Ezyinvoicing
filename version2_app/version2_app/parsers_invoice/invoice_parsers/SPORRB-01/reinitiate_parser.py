...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("SPORRB-01")
        ...
        for i in raw_data:
            # print(i,"++++++++++++",entered,i)
            if "Reg No" in i:
                confirmation_number = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
                confirmation_number = ''.join(filter(lambda i: i.isdigit(), confirmation_number))
                conf_number = confirmation_number.replace(" ", "")
            if "Net Amount:" in i:
                total_invoice = i.split(":")[-1].strip()
                total_invoice_amount = float((total_invoice.split(" ")[-1]).replace(",", ""))
                if "-" in total_invoice.split(" ")[-1]:
                    invoice_category = "Credit Invoice"
            # if "Bill Date :" in i or "Bill Date" in i:
            #     date_time_obj = (i.split(":")[-1]).strip()
            #     date_time_obj = datetime.datetime.strptime(date_time_obj,'%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if "Bill Date" in i or "Bill Date & Time" in i or "BillDate" in i:
                date_time_obj = i.split(':')[-1].strip()
                if "/" in date_time_obj:
                    date_time_obj = datetime.datetime.strptime(date_time_obj, '%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
                else:
                    date_time_obj = datetime.datetime.strptime(date_time_obj, '%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
                # date_time = datetime.datetime.strptime(date_time_obj, '%d-%b-%y %H:%M:%S').strftime('%d%m%y')
                # invoiceNumber = str(date_time)+invoiceNumber
                # invoiceNumber = invoiceNumber.lstrip("0")
            if "Room No" in i or "Room No  :" in i or "Room No " in i:
                room = i.split(":")
                roomNo=re.sub("[a-zA-Z]","",room[-1])
                roomNumber = roomNo.strip()
            # if "Room No" in i or "Room No  " in i:
            #     room = i.split(":")
            #     roomNo=re.match("\d+",room[-1])
            #     if roomNo:
            #         roomNumber=roomNo.group()
            #     else:
            #         roomNumber=0
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GSTN Number" in i:
                gstNumber = i.split(':')[1].replace('~', '').strip()
                gstNumber = (gstNumber.replace("Room Tariff","").strip()).split("Plan")[0].strip()
            # if "Bill Number" in i or "Bill Number." in i:
            #     invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
            if "Bill Number" in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
                invoiceNumber = invoiceNumber.lstrip("0")
                invoiceNumber = invoiceNumber.replace("/","").strip()
            if "GSTN Bill No" in i:
                number_invoice = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
            if "Bill To" in i:
        ...
        if invoiceNumber != reupload_inv_number:
            if invoiceNumber != number_invoice:
                return {"success":False,"message":"Incorrect Invoice Attempted"}
            else:
                invoiceNumber = number_invoice
            ...
            if (len(check_date) > 0 and "~" in i):
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        ...
                        if "~" in i:
                            result = re.search("~(.*)~", i)
                            result_name= re.sub(r'[0-9]+', '', result.group(1))
                            item['name'] = result_name.strip()
                        item_value = val[-1]
        ...
        for each in items:
            if 'Integrated GST' not in each["name"] and "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Central GST" not in each["name"] and "State GST" not in each["name"]:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Invoice No","").strip()
                guest['name'] = re.sub('[0-9]',' ',guest['name']).strip()
                guest["name"] = guest["name"].replace(".","").replace("Bill Number","")
                guest["name"] = guest["name"].replace("Invoice No","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"SPORRB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"SPORRB-01","invoice_date":date_time_obj}
        ...
        error_data['pincode'] = " "
        error_data['total_invoice_amount'] = total_invoice_amount if total_invoice_amount != 0.00 else 0
