...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("TRND-01")
        ...
        for i in raw_data:
            ...
            if "FSSAI No. :" in i:
                ...
                match = re.search(date_pattern, date_obj)
                if match:
                    date_str = match.group(0)
                    date_str = date_str  
                    date_time_obj = datetime.datetime.strptime(date_str,'%dth %B %Y').strftime('%d-%b-%y %H:%M:%S')
                else:
                    print("No date found.")
            if date_time_obj == "":
                if "Departure :" in i:
            ...
            if "GST ID" in i or "Company GST ID :" in i and "PROPERTY GST ID :" not in i :
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("TAXINVOICE","").replace("ConfirmationNo","")
                gstNumber = gstNumber.replace("CREDITTAXINVOICE","").replace("ConfirmationNo.","")
        ...
        company_code = {"code":"TRND-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TRND-01","invoice_date":date_time_obj}
