    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Original Bill" in i and "Original Bill  date" not in i:
                date_time_obj = i.split(":")
                date_time_obj = date_time_obj[-1].replace(" ", "")
            ...
            if "COPY OF INVOICE" in i and date_time_obj == "":
                dateind = raw_data.index(i)
                date_time_obj = raw_data[dateind-1].strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if "TAX INVOICE" in i and "CREDIT" not in i:
                dateind = raw_data.index(i)
                date_time_obj = raw_data[dateind+1].strip()
                if "Original Bill " in date_time_obj:
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and ("IGST" not in each["name"] or ("Misc" in each["name"])):
