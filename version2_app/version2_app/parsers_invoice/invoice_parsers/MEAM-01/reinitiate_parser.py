...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("MEAM-01")
        ...
        for i in raw_data:
            ...
            if "GST ID  " in i or "GST ID " in i and "Property Tax ID" not in i:
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Room No." not in each['name'] and "Allow " not in each['name']:
        ...
        company_code = {"code":"MEAM-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"MEAM-01","invoice_date":date_time_obj}
