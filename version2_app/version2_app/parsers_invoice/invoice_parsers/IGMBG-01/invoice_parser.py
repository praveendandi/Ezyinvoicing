...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("IGMBG-01")
		...
		for i in raw_data:
			...
			if "GST ID" in i or "GST ID  " in i:
				...
				gstNumber = gstNumber.replace("PageNo.","").strip()
		...
		for i in data:
			...
			if len(check_date) > 0 and "." in split_str and "Room No" not in i:
				item = dict()
				item_value = ""
		...
		for each in items:
			if each["name"] != "":
				if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Room No." not in each['name']:
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				...
				if "Membership No." in guest['name']:
					guest['name'] = guest['name'].replace("Membership No.","").strip()
			if index == 1:
				guest['address1'] = i
			if index == 2:
				guest['address2'] = i
		...
		company_code = {"code":"IGMBG-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IGMBG-01","invoice_date":date_time_obj}
