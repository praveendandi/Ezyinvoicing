...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("FMPK-01")
        ...
        for i in raw_data:
            ...
            if "TAX INVOICE" in i or "CREDIT TAX INVOICE" in i:
            ...
            if "Invoice No" in i:
                ...
                if "-" in invoiceNumber:
                    invoicenum = invoiceNumber.split("-")[-1]
                    invoiceNumber = "PNQFI-"+invoicenum
                # if "OMPEH-" in invoiceNumber:
                    # invoiceNumber = invoiceNumber.lstrip("OMPEH-")
                    # invoiceNumber = invoiceNumber.replace("OMPEH-","")
                    # invoiceNumber = "PNQFI-" + invoiceNumber.strip()
                    # print(invoiceNumber,"////////////..........")
            if "Bill To" in i:
            ...
            if "Company Name" in i:
                guestDeatils.append(i)
            if "GST No." in i or "Guest Name         " in i:
                guestDeatils.append(i)
            if "Membership" in i:
                Membership = i.split(":")
        ...
        if invoiceNumber != reupload_inv_number:
            invoice_number = reupload_inv_number.lstrip("OMPEH-")
            other_invoice_number = invoiceNumber.lstrip("PNQFI-")
            if invoice_number == other_invoice_number:
                invoiceNumber = reupload_inv_number    
        check_invoice = check_invoice_exists(invoiceNumber)
        # if invoiceNumber != reupload_inv_number:
        #     invoice_number = reupload_inv_number.replace("OMPEH-","")
        #     other_invoice_number = "PNQFI-" + invoice_number
        #     print(other_invoice_number,"??????????????")
        #     other_invoice_number = other_invoice_number.strip()
        #     print(other_invoice_number,"///////////")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "IGST" not in i and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "VAT" not in i and "Fairfield by Marriott Pune Kharadi" not in i and " (A Unit Of Argon Hotels Pvt Ltd)" not in i and "Room#" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Invoice No","")
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"FMPK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMPK-01","invoice_date":date_time_obj}
