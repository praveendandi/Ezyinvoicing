...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Confirmation No." in i or "Confirmation Number:" in i or "Confirmation Number :" in i:
                confirmation_number = i.split(":")
                conf_number = confirmation_number[-4].replace(" ", "")
                conf_number= conf_number.replace("GuestName","")
            if "Total      in INR" in i or "Total" in i and "in INR" in i:
            ...
            if "Room No. :" in i:
                ...
                # print(room,i)
                # roomNumber = room[-1]
                roomNumber = room[-2].replace("Room Type","").strip()
            ...
            if 'Billing' in i:
                entered = False
            if 'Total' in i and "in INR" in i:
                entered = False
            if entered == True:
                data.append(i)
            if "Page :" in i:
                raw_value = raw_data[raw_data.index(i)-1]
                guestDeatils.append(raw_value)
        ...
        for i in original_data:
            ...
            if len(check_date) > 0 and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "VAT" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest["name"] = i.replace("Arrival","")
            # if guest["name"] == "":
            #     if "Guest Name :" in i:
            #         guest_name = (i.split('Guest Name :')[-1]).strip()
            #         dte, tokens = parse(guest_name, fuzzy_with_tokens=True)
            #         guest["name"] = (''.join(tokens)).strip()
            if index == 1:
                find = i.index("Place of Supply")
                guest['address1'] = i[:find].strip()
            if index == 2:
                find = i.index("Room No.")
                guest['address2'] = i[:find].strip()
