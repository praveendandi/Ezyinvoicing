...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("RAUD-01")
        ...
        for ind, each in enumerate(items):
            ...
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charges" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "Bed and Breakfast" or each["name"] == "Accomodation Charges":
                        if ind+1 < len(items):
        ...
        company_code = {"code":"RAUD-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RAUD-01","invoice_date":date_time_obj}
