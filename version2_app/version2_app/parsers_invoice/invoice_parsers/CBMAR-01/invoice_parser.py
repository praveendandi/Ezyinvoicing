...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CBMAR-01")
        ...
        for i in raw_data:
            ...
            if "Bill  No. :" in i:
                date_inv = (i.split(" ")[0]).strip()
                date_time_obj = datetime.datetime.strptime(date_inv,'%d-%b-%Y').strftime('%d-%b-%y %H:%M:%S')
            if date_time_obj == "":
                if "Departure" in i:
                    date_inv = (i.split(":")[-1]).strip()
                    date_time_obj = datetime.datetime.strptime(date_inv,'%d-%b-%y').strftime('%d-%b-%y %H:%M:%S')
            # if "COPY OF INVOICE" in i:
            #     dateind = raw_data.index(i)
            #     date_time_obj = raw_data[dateind+1].strip().split(" ")[0]
            #     if "COPY OF INVOICE" in i:
            #         date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%b-%Y').strftime('%d-%b-%y %H:%M:%S')
            #         print(date_time_obj,"invvvvvvv date")
            #     else:
            #         date_time_obj = datetime.datetime.strptime(date_time_obj, '%d-%b-%Y').strftime('%d-%b-%y %H:%M:%S')
            #         print(date_time_obj,"invvvvvvv date")
            if "Bill  No. :" in i:
                invoiceNumber = i.split(":")[-1].lstrip("0")
        ...
        company_code = {"code":"CBMAR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMAR-01","invoice_date":date_time_obj}
