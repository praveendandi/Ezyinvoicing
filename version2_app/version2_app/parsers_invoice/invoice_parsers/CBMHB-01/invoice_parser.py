...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("CBMHB-01")
		...
		for i in raw_data:
			...
			if "Bill No  :" in i or "Bill  No." in i:
				invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "").replace("0","")
			...
			if ("GST ID   :" in i or "GST ID " in i)and "PAN ." not in i:
				gstNumber = i.split(':')[1].replace('Invoicedate', '')
				gstNumber = gstNumber.replace("Invoice date","").strip(' ')
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				guest['name'] = i.split(':')[1].replace('Check-Out (BD)','').strip()
				guest['name']=guest['name'].replace('Check-In (BD)','').strip()
				guest['name']=guest['name'].replace('Check Out By','').strip()
			if index == 1:
				guest['address1'] = i
			if index == 1:
				guest['address2'] = i
		...
		company_code = {"code":"CBMHB-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMHB-01","invoice_date":date_time_obj}
