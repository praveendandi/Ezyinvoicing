...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("CBMS-01")
		...
		for i in raw_data:
			...
			if "Total :" in i or "Total  :" in i or "Total  :" in i:
				total_invoice = i.split(" ")
				if total_invoice[-2].replace(",", ""):
					total_invoice_amount = float(total_invoice[-2].replace(",", ""))
					if "-" in total_invoice[-2]:
						invoice_category = "Credit Invoice"
				else:
					total_invoice_amount = float(total_invoice[-3].replace(",", ""))
					if "-" in total_invoice[-3]:
						invoice_category = "Credit Invoice"
			# if "Departure :" in i:
		...
		for i in data:
			...
			if len(check_date) > 0 and "FSSAI:" not in i:
				item = dict()
				item_value = ""
				dt = i.strip()
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				...
				guest['name'] = guest['name'].replace('Original Bill','').strip()
			if index == 1:
				guest['address1'] = i
			if index == 2:
				guest['address2'] = i
		...
		company_code = {"code":"CBMS-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMS-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "IXBCY-"+invoiceNumber
