...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("MEANM-01")
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Room No." not in each['name'] and "Allow " not in each['name'] and "CHECK#" not in each['name']:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Departure","").strip()
            if index == 1:
                print(i,"/////")
        ...
        company_code = {"code":"MEANM-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"MEANM-01","invoice_date":date_time_obj}
