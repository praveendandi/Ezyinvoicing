...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("JMR-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  :" in i:
                ...
                date_time_obj = datetime.datetime.strptime(date_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            # if "Departure :" in i:
            ...
            if "GST ID  " in i and "HOTEL" not in i:
                # if "GST NO." not in i:
            ...
            if "Bill  No. " in i:
                invoiceNumber = (i.split(':')[-1]).replace(" ", "").lstrip("0")
                if "/" in invoiceNumber:
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and ("IGST" not in each["name"] or "Debit Note- IGST"  in each["name"]):
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Guests","").strip()
        ...
        company_code = {"code":"JMR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JMR-01","invoice_date":date_time_obj}
        ...
        # error_data['invoice_number'] = invoiceNumber+"-1"
        # gstNumber = "12345"
