...
def file_parsing(filepath):
    try:
        ...
        companyCheckResponse = check_company_exist("PPHK-01")
        ...
        for i in raw_data:
            ...
            if "Room No" in i:
                room = i.split(" ")
                roomNumber = room[-1]
            if "GSTN Number" in i or "Billing Instructions" in i or "Guest GSTN #" in i:
                if "Guest GSTN #" in i:
                    gstNumber = i.split(':')[1].replace('GSTN Bill #', '').strip()
                else:
                    gstNumber = i.split(':')[-1].replace(' ', '')
            if ("Bill Number :" in i or "Bill No" in i) and "GSTN" not in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest["name"] = guest["name"].replace("BillNumber","")
            if index == 1:
                guest['address1'] = ((i.split(':')[1]).replace("Bill Date","")).strip()
                guest["address1"] = guest["address1"].replace("BillDate","")
            if index == 2:
        ...
        company_code = {"code":"PPHK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"PPHK-01","invoice_date":date_time_obj}
