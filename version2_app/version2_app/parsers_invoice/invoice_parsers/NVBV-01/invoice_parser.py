...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "C. GST NO" in i or "C GST ID  " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace("ConfirmationNo","")
                gstNumber = gstNumber.replace("Membership","").replace("-","").replace(".","").strip()
            if "Bill  No. " in i:
        ...
        for each in items:
            ...
            elif "Allowance Service Charge" in each["name"]:
                if each["name"] not in payment_Types:
                    total_items.append(each)
        ...
        company_code = {"code":"NVBV-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"NVBV-01","invoice_date":date_time_obj}
