...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HIEMIA-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  date" in i:
                ...
                print(date_time_obj,"------------------------------")
                date_obj = re.search("^([0-9]{2}\-[0-9]{2}\-[0-9]{2})+", date_time_obj)
                date_time_obj = datetime.datetime.strptime(date_obj.group(1),'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No." in i:
                room = i.split(":")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "Client GST ID" in i:
        ...
        company_code = {"code":"HIEMIA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIEMIA-01","invoice_date":date_time_obj}
