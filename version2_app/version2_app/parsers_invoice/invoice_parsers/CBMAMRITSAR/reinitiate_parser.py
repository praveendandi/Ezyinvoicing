...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("CBMAMRITSAR")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[0].replace('Room','').strip()
            if index == 1:
                guest['address1'] = i
            if index == 1:
                guest['address2'] = i
        ...
        guest['company_code'] = "CBMAMRITSAR"
        ...
        company_code = {"code":"CBMAMRITSAR"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMAMRITSAR","invoice_date":date_time_obj}
