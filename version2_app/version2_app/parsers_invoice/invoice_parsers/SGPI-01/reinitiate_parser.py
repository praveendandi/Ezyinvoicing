...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("SGPI-01")
        ...
        for i in raw_data:
            # print(i,'000000000000000000000000000000')
            # if raw_data.index(i)==1 and "TAX INVOICE" not in i:
            ...
            if "GST ID" in i or "GST ID  " in i and "Pan No" not in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("PrintedonDate","").strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace('Printed By','').strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"SGPI-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"SGPI-01","invoice_date":date_time_obj}
