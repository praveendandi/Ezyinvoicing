...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("NKHR-01")
        ...
        for i in raw_data:
            ...
            if "Departure :" in i:
                ...
                if checkout_date == '':
                    ...
                    checkout_date = datetime.datetime.strptime(checkout_date,'%d-%m-%y %H:%M').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. " in i or "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "Company GSTIN  " in i:
            ...
            if i in "Date Description Reference Debit Credit" or i in "Date Description Reference Debit Credit":
                entered = True
            if 'CGST 6%=' in i:
                entered = False
            if 'Billing' in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest["name"] = guest["name"].replace("Arrival","").replace("Membership","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"NKHR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"NKHR-01","invoice_date":date_time_obj,"checkout_date":checkout_date if checkout_date != '' else None}
        error_data['invoice_file'] = filepath
        error_data['guest_name'] = guest['name']
        error_data['gst_number'] = gstNumber.strip()
        ...
        # error_data["checkout_date"] = checkout_date if checkout_date != "" else None
        # gstNumber = "12345"
