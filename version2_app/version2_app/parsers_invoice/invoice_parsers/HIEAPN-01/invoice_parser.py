...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HIEAPN-01")
        ...
        for i in raw_data:
            ...
            if "Room No." in i:
                ...
                room = i.split(" ")
                roomNumber = room[-1]
                roomNumber = roomNumber.replace("No.","")
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GSTIN / UIN:" in i or "GSTIN/UIN:" in i:
                i = i.replace("GST ","")
                gstNumber = (i.split(':')[1]).split(" ")[1].strip()
            ...
            if "Invoice No." in i:
                ...
                invoiceNumber = invoiceNumber.replace("O","0")
                ...
                if len(items_split)> 4 and "." in items_split[-1]:
                    ...
                    if "Transfer Debit" in item["name"]:
                        if "(Amt" in i:
                            ...
                            item["name"] = item["name"][:name_index]
                    if "Miscellaneous with CGST / SGST@12%" in item["name"]:
                        item["name"] = "Miscellaneous with CGST / SGST@12%"
        ...
        for each in items:
            print(each,"000000000000000000000")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                if "Page" in guest["name"]:
                    ind = guest["name"].index("Page")
                    guest["name"] = guest["name"][0:ind]
                # print(guest,i)
        ...
        company_code = {"code":"HIEAPN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIEAPN-01","invoice_date":date_time_obj}
