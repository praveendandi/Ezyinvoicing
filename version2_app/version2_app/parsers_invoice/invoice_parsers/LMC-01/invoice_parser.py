...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("LMC-01")
        ...
        for i in raw_data:
            if "CREDIT TAX INVOICE" in i or "TAX_INVOICE" in i or "TAX INVOICE" in i:
                ind = raw_data.index(i)
                print(raw_data[ind+1]),"jkjsgdjhsagdjhasgdjhadsgjhadsg"
                # year_date_obj = raw_data[ind+1].split("/")[-1]
                # ddmm_date_obj = raw_data[ind+1].split("/")[:2]
                # ddmm_date_obj.append('20'+year_date_obj)
                # date_obj = '/'.join(ddmm_date_obj)
                # inv_date = datetime.datetime.strptime(date_obj, '%d/%m/%Y')
                inv_date = datetime.datetime.strptime(raw_data[ind+1],"%d/%m/%y")
            ...
            if "GST ID  :" in i and "Pan No" not in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.replace("PrintedBy/On","")
            if "Bill  No." in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        item_value = val[-1]
                        item_value = item_value.lstrip(ascii_letters)
                        item["item_value"] = item_value.replace('B', '')
                        item["item_value"] = item_value.replace('Sa', '')
                        if "H" in item['item_value']:
                            item["item_value"] = item_value.replace('H', '')
                        if "Shamm" in item['item_value']:
                            item["item_value"] = item_value.replace('Shamm', '')
                        if "G" in item['item_value']:
                            item["item_value"] = item_value.replace('G', '')
                        if "#" in item['item_value']:
                            item["item_value"] = item_value.replace('#', '')
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace('Rate','').strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"LMC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LMC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = invoiceNumber.replace("9999","")
