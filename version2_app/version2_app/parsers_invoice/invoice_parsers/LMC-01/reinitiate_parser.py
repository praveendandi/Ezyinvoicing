    ...
    try:
        ...
        for i in raw_data:
            ...
            if "CREDIT TAX INVOICE" in i or "TAX_INVOICE" in i or "TAX INVOICE" in i:
                ...
                date_obj = raw_data[ind+1].split(" ")[-1]
                inv_date = datetime.datetime.strptime(date_obj, '%d/%m/%y')
                date_time_obj = inv_date.strftime("%d-%b-%y %H:%M:%S")
            # if "TAX INVOICE" in i or "Original Bill  :" in i and "CREDIT TAX INVOICE" not in i and "COPY OF INVOICE" not in i:
            #     print(i,"///////////")
            #     ind = raw_data.index(i)
            #     date_obj = raw_data[ind+1]
            #     inv_date = datetime.datetime.strptime(date_obj, '%d-%m-%y')
            #     date_time_obj = inv_date.strftime("%d-%m-%y %H:%M:%S")
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        ...
                        # item['item_value'] = float(item['item_value'].replace('INR', ''))
                    # else:
                    # 	item_value = val[-2]
