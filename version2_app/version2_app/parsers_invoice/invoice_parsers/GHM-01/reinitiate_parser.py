...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("GHM-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i or "Total          INR" in i:
            ...
            if ("TAX INVOICE" in i or "CREDIT INVOICE" in i)and "Original Bill" not in i:
                # if "Original Bill" not in i:
            ...
            if "Departure :" in i:
                ...
                if checkout_date == '':
                    ...
                    checkout_date = datetime.datetime.strptime(checkout_date,'%d-%b-%y %H:%M:%S').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST ID" in i and "Pan No" not in i and "Property GST ID" not in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                # gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.replace("COPY OF INVOICE","").strip()
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            item["name"]= item["name"].replace("~[Add: 18%.","")
                    ...
                    if "~" in i:
                        ...
                        item_ref = i[start_index:end_index].replace("~","")
                        if item_ref[:2].isalpha():
                            item["item_reference"] = item_ref[:8]
                        else:
                            item["item_reference"] = item_ref[:6]
                        item["item_reference"] = item["item_reference"].replace("[NA Room]","").replace("[NA Pk","")
                        item["item_reference"] = item["item_reference"].replace("~[Add: 18%.","")
                        item_dt =  datetime.datetime.strptime(item['date'],'%d.%m.%y').strftime('%Y-%m-%d')
                        item["check_number"] = item["item_reference"].replace("[NA Pkg. Trx]","")
                        item["reference_check_number"] = item_dt + '-' + item["check_number"]
                    else:
                        item["item_reference"] = ""
                itemsort+=1
                if item !={}:
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"]:
        ...
        company_code = {"code":"GHM-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"GHM-01","invoice_date":date_time_obj,"checkout_date":checkout_date if checkout_date != '' else None}
