...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TWHH-01")
        ...
        for i in raw_data:
            ...
            if "Depart/Inv Date" in i or "Depart/Inv Date " in i:
                date_time_obj = (i[i.rindex("Depart/Inv Date"):]).replace('Depart/Inv Date :',"")
                date_time_obj = date_time_obj.strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%b-%y %H:%M:%S').strftime('%d-%b-%y %H:%M:%S')
                checkout_date = datetime.datetime.strptime(date_time_obj,'%d-%b-%y %H:%M:%S').strftime('%d-%b-%y %H:%M:%S')
            ...
            if "GST ID" in i or "GST ID  " in i:
                ...
                gstNumber = gstNumber.replace("Depart/InvDate","").replace("Cashier","").strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.replace("Cashier","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"TWHH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TWHH-01","invoice_date":date_time_obj,"checkout_date":checkout_date if checkout_date != '' else None}
