...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TRGR-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i:
                ...
                if "-" in total_invoice[-2]:
                    invoice_category = "Credit Invoice"
            if "Bill  No. :" in i:
                date_obj = (i.split("Bill  No.")[0]).strip()
                date_match = re.search(r"(\d{2})th\s+(\w+)\s+(\d{4})", date_obj)
                if date_match:
                    extracted_date = date_match.group(0)  # Get the entire matched date string
                    date_time_obj = datetime.datetime.strptime(extracted_date, "%dth %B %Y").strftime('%d-%b-%y %H:%M:%S')
                else:
                    date_time_obj = ""
            if date_time_obj == "":
            ...
            if "GST ID" in i or "GST ID  :" in i:
                ...
                gstNumber = gstNumber.replace("GSTNO.","").strip()
        ...
        company_code = {"code":"TRGR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TRGR-01","invoice_date":date_time_obj}
