...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("HCJND-01")
        ...
        for i in raw_data:
            ...
            if "Room No." in i or "Room No" in i or "Room No. " in i or "Room No.  " in i:
                room = i.split(":")
                roomNumber = room[-1]
            ...
            if "Bill  No." in i or "Invoice No. " in i or "Invoice No.  " in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "Bill  No." not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Rebate 9%" not in each["name"]:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Bill  No. ","")
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"HCJND-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HCJND-01","invoice_date":date_time_obj}
