...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "FSSAI No. :" in i:
                date_obj=i.split(":")
                timeobj=date_obj[-1].split(" ")
                date_timeobj=timeobj[-1]
                date_time_obj = datetime.datetime.strptime(date_timeobj,'%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            ...
            if "GST ID" in i or "GST ID  " in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace("Confirmation No","").strip()
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "Bed and Breakfast":
                        if ind+1 < len(items):
