...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HIEHH-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "Total Value" not in i and "INR" not in i and "charges." not in i:
                total_invoice = i.split(" ")
                total_invoice_amount = float(total_invoice[-2].replace(",", ""))
            if "Invoice Date" in i:
                date_time_obj = i.split("Invoice Date")[-1].strip()
            ...
            if "Invoice No." in i:
                ...
                if "l" in invoiceNumber:
                    invoiceNumber = invoiceNumber.replace("l","").strip()
                if "--" in invoiceNumber:
                    invoiceNumber = invoiceNumber.replace("--","-I-").strip()
        ...
        for index, i in enumerate(guestDeatils):
            ...
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Page 1 of 2","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"HIEHH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HIEHH-01","invoice_date":date_time_obj}
