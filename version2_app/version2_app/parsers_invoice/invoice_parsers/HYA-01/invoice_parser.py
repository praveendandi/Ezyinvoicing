...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("HYA-01")
		...
		for i in raw_data:
			...
			if "Total" in i and "INR" in i:
				...
				print(total_invoice,"totallllllllllllllllllllll")
			# if "Departure :" in i:
			...
			if "Guest Name" in i:
				guestDeatils.append(i)
			if "Membership:" in i:
				membership = i.split(":")[-1].strip().replace("Balance     INR 0.00","").replace(" ", "")
		...
		company_code = {"code":"HYA-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HYA-01","invoice_date":date_time_obj}
