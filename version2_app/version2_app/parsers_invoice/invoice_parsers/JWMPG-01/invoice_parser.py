...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("JWMPG-01")
		...
		for i in data:
			...
			if len(check_date) > 0 and "." in (st.split(" ")[-1]):
				item = dict()
				item_value = ""
				dt = i.strip()
				for index, j in enumerate(i.split(' ')):
					...
					if "~" in i:
						...
						if "CHECK#" in i:
							starting_ind = i.index("CHECK#")
							ref = i[starting_ind:]
							ending_ind = ref.index("[")
							item["check_number"] = ref[:ending_ind].replace("CHECK#","").strip()
							item["reference_check_number"] = item["date"] + '-' + item["check_number"]
						else:
							item["check_number"] = ""
							item["reference_check_number"] = ""
				itemsort+=1
				items.append(item)
		...
		for each in items:
			if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "UTGST" not in each["name"]:
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				guest['name'] = i.split(':')[1].strip().lstrip('.')
			if index == 1:
				guest['address1'] = i
			if index == 2:
				guest['address2'] = i
		...
		if check_invoice['success']==True:
			...
			if inv_data.docstatus==2:
				...
				if inv_data.invoice_type == "B2B":
					if inv_data.irn_generated=="Pending" or inv_data.irn_generated == "Error" or inv_data.irn_generated == "On Hold":
						reupload = True
				else:
					reupload = True

		company_code = {"code":"JWMPG-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JWMPG-01","invoice_date":date_time_obj}
