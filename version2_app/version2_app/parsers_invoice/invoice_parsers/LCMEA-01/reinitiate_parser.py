...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("LCMEA-01")
        company = frappe.get_doc("company","LCMEA-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  date" in i:
                guestname = raw_data.index(i)
                guestDeatils.append(raw_data[guestname+1])
            if "Membership" in i:
                Membership = i.split(":")
        ...
        company_code = {"code":"LCMEA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LCMEA-01","invoice_date":date_time_obj}
