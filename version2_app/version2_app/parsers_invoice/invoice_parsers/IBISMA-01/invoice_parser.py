...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISMA-01")
        ...
        for i in raw_data:
            ...
            #         i = i.strip()
            #         gst=i.split()
            #         gstNumber = gst[3].strip()
            #         gstNumber = gstNumber.replace("Confirmation","").replace("ConfirmationNo.","")
            if  "Bill No. " in i:
        ...
        company_code = {"code":"IBISMA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBISMA-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "IMA"+invoiceNumber
