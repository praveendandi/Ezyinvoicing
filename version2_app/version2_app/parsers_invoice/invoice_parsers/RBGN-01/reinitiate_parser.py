...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("RBGN-01")
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1:
                        ...
                        if "~" in i:
                            ...
                            item["name"] = ' '.join(remove_special_characters)
                    if 'SAC' in j:
                        item['sac_code'] = ''.join(filter(lambda j: j.isdigit(), j)).replace(" ","")
                    else:
        ...
        for ind, each in enumerate(items):
            print(each,"/////")
            # if "service Charge" not in each["name"] and "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Service Charge" not in each["name"]:
            ...
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charges" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Members Only Rates" or each["name"] == "Bed and Breakfast":
                        if ind+1 < len(items):
        ...
        company_code = {"code":"RBGN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RBGN-01","invoice_date":date_time_obj}
