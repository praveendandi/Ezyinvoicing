...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("STRG-01")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str:
                ...
                for index, j in enumerate(i.replace("  "," ").split(' ')):
                    ...
                    #     # check_number = re.search('CHECK#(.*)[', item["item_reference"])
                    #     if "CHECK#" in i:
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "vat" not in each["name"] and "IGST" not in each["name"]:
        ...
        company_code = {"code":"STRG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"STRG-01","invoice_date":date_time_obj}
