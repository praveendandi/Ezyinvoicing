...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("LAMAN-01")
        ...
        for i in raw_data:
            if "Bill Date  :" in i and "Original Bill  date" not in i and "Original" not in i:
            ...
            if "TAX INVOICE" in i or "Bill Date  :" in i and "CREDIT TAX INVOICE" not in i or "Unit GST ID  :" in i:
                ind = raw_data.index(i)
            ...
            if "Guest GST ID :" in i or "Guest GST ID " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("PrintedBy/On","").strip()
            # if "Guest GST ID :" in i:
            #     if "Printed By / On" not in i:
            #         gstNumber = i.split(':')[1].replace(' ', '')
            #         gstNumber = gstNumber.replace("TAXINVOICE","")
            #     else:
            #         gstNumber = i.split('Printed By / On')[0].split(':')[1].replace(' ', '')
            #         gstNumber = gstNumber.replace("TAXINVOICE","")
            if "Bill  No." in i:
            ...
            if "Printed By / On :" in i or "Printed By / On " in i:
                print_by = (i.split(" ")[-2]).strip()
                print_by = print_by.replace(" ","")
        ...
        for i in data:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        # val[-1]=val[-1].replace(',', '')
                        item_value = re.findall(r"[\d.,-]+",val[-1])[-1]
                        print(item_value,"???????????")
                        # item_value = re.compile(r"^\d*[.,]?\d*$",val[-1])
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ending_index = i.find("~")
                            if "-" in i[starting_index:ending_index].strip():
                                item["name"] = ((i[starting_index:ending_index-1]).strip()).replace("  "," ")
                            else:
                    ...
                    if "~" in i:
                        ...
                        item["item_reference"] = item["item_reference"].replace("[NA Pkg. Trx]","")
                    else:
                        item["item_reference"] = ""
                itemsort+=1
                if item !={}:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Rate","")
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"LAMAN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LAMAN-01","invoice_date":date_time_obj}
