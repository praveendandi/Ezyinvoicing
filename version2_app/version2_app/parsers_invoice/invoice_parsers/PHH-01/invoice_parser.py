...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("PHH-01")
        ...
        for i in raw_data:
            ...
            if "TAX INVOICE" in i:
                ...
                date_time_obj = datetime.datetime.strptime(str(date_time_obj),'%d.%m.%y').strftime('%d-%b-%y %H:%M:%S')
            if date_time_obj == "":
                if "Departure :" in i:
        ...
        company_code = {"code":"PHH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"PHH-01","invoice_date":date_time_obj}
