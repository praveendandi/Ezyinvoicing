...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("NCC-01")
        ...
        for i in raw_data:
            ...
            if "Confirmation No." in i or "Confirmation  " in i or "Conf. No. " in i:
            ...
            if ("GST ID" in i or "GST ID   " in i) and "Property GST ID  " not in i:
                ...
                gstNumber = gstNumber.replace("TAXINVOICE","").replace("CREDITBILL","").replace("PrintedBy/On","").strip() 
        ...
        company_code = {"code":"NCC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"NCC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "NCCR" + invoiceNumber
