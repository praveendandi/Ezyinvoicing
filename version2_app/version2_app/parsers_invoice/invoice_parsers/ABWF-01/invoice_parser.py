...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("ABWF-01")
        ...
        for i in data:
            ...
            item_val = i.split(" ")[-1]
            if len(check_date) > 0 and "." in item_val:
                item = dict()
                item_value = ""
                dt = i.strip()
                i = i.replace("  "," ")
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            if len(item_name) == 1:
                                item["name"] = (item["name"][:-1]).replace(" ","").strip()
                        else:
                            ending_index = i.find(item_value)
                            item["name"] = ((i[starting_index:ending_index]).strip()).replace("  "," ")
                            remove_special_characters = re.findall(r'\w.+\w',item["name"])
                    ...
                    if "~" in i:
                        ...
                        if "Room#" in i:
                            start_ind = i.index("Room#")
                            refer = i[start_ind:]
                            ending_ind = refer.index("CHECK#")
                            item["room_number"] = refer[:ending_ind].replace("Room#","").replace(":","").strip()
                        else:
                            item["room_number"] = ""
                    else:
                        item["item_reference"] = ""
        ...
        company_code = {"code":"ABWF-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"ABWF-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    ...
    invoiceNumber = "MI8001-"+invoiceNumber
