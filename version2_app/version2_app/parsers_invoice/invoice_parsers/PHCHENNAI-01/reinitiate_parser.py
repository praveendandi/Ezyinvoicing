...
def reinitiateInvoice(data):
    ...
    companyCheckResponse = check_company_exist("PHCHENNAI-01")
    ...
    content = []
    text_path = file_path.split('.pdf')[0]+".txt"
    ocr_pdf = file_path.split('.pdf')[0]+"_ocr.pdf"
    invoiceNumber = ''
    print_by = ''
    document_type = ""
    try:
        if cid_issue == True:
            cmd = "ocrmypdf --sidecar "+text_path+" "+file_path+" "+ocr_pdf+" --force-ocr" 
        ...
        for index,i in enumerate(raw_data):
            if "TAX INVOICE" in i or "CREDIT TAX INVOICE" in i or "CREDIT INVOICE" in i: 
                if date_time_obj == "":
                    if "GST ID" not in raw_data[index+1]:
                        date_ind = raw_data.index(i)+1
                        inv_date = raw_data[date_ind]
                        inv_date = datetime.datetime.strptime(inv_date, '%d.%m.%y')
                        date_time_obj = inv_date.strftime("%d-%b-%y %H:%M:%S")
                    else:
                        date_ind = raw_data.index(i)
                        inv_date = raw_data[date_ind]
                        inv_date = inv_date.replace("CREDIT INVOICE","").strip()
                        inv_date = datetime.datetime.strptime(inv_date.replace('TAX INVOICE',"").strip(), '%d.%m.%y')
                        date_time_obj = inv_date.strftime("%d-%b-%y %H:%M:%S")
            if date_time_obj =="":
            ...
            if "Room No." in i or "Room  No." in i:
                room = i.split("Room No.")
                roomNumber = room[-1]
                roomNumber = ''.join(filter(lambda j: j.isdigit(), roomNumber))
            if "GST ID" in i and "Property GST ID" not in i:
                gstNumber = ''
                if ':' in i and len(i)>23:
                    gstNumber = i.split(':')[1].replace(' ', '').replace('INVOICE','').replace('COPYOF','')
                else:
                    if len(i)>23:
                        gstNumber = i.split('>')[1].replace(' ', '')
                gstNumber = gstNumber.replace("COPYOFINVOICE","").replace('INVOICE','')
                gstNumber = gstNumber.replace("|","").strip()
            # if "Bill No." in i or 'Bill No.' in i:
            #     invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
            #     print("++++++++++",invoi)
            #     invoiceNumber = ''.join(filter(lambda j: j.isdigit(), invoiceNumber))
            if "Bill No." in i or 'Bill  No. ' in i:
                if ":" in i:
            ...
            if "Guest Name :" in i:
                guestDeatils.append(i)
            # if "Membership" in i and "No Membership" not in i:
            #     Membership = i.split(":")
            #     membership = Membership[1].strip().rstrip("\ufffd")
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if index == 1 and len(val)>1:
                        ...
                        if "~" in i:
                            ...
                            remove_special_characters = re.findall(r'\w.+\w',item["name"])
                            item["name"] = ' '.join(remove_special_characters)
                        item["name"] = item["name"].replace("#","").replace("*","").replace("^","").strip()
                        item["name"]=item["name"].replace(item["date"],"").strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                # guest['name'] = (i.split('Departure')[0]).replace("Guest Name:","").strip()
                gue_name=i.split(':')
        ...
        guest['company_code'] = "PHCHENNAI-01"
        ...
        company_code = {"code":"PHCHENNAI-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"PHCHENNAI-01","invoice_date":date_time_obj}
        ...
        if cid_issue == True and remove_file == True:
            os.remove(text_path)
            os.remove(ocr_pdf)
