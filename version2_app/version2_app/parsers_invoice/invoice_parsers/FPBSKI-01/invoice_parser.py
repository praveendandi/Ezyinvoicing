...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FPBSKI-01")
        ...
        for i in raw_data:
            ...
            #     date_time_obj = raw_data.index(i)-1
            #     print(date_time_obj,"daaaaaaaaaaaaa")
            #     date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
        ...
        company_code = {"code":"FPBSKI-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FPBSKI-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "COKFP-"+invoiceNumber 
