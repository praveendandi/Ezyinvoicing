...
def file_parsing(filepath):
    ...
    try:
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "Room#" not in each["name"] and "[NA Pkg. Trx]" not in each["name"] and "[Add: udf.]" not in each["name"] and "Adjust Trn." not in each["name"] and ("IGST" not in each["name"] or "Debit Note(IGST)" in each["name"]):
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                if "COPY OF INVOICE" in i:
                    ...
                    guest["name"] = guest["name"].replace("COPY OF INVOICE","").strip()
                if "TAX INVOICE" in i:
        ...
        print(json.dumps(guest, indent = 1),total_invoice_amount)
