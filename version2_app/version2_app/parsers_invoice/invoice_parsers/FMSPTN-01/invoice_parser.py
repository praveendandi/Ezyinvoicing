...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FMSPTN-01")
        ...
        for i in raw_data:
            print(i,"***********************")
            if "TAX INVOICE" in i and "Original Bill No." not in i:
            ...
            if ("Room  :" in i or "Room :" in i) and ("Room Service " not in i or "Room#" not in i):
                room = i.split(" ")
                roomNumber = room[-1]
            ...
            if "Invoice No" in i or "Invoice No  " in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "").replace("MAAFI-","")
        ...
        company_code = {"code":"FMSPTN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMSPTN-01","invoice_date":date_time_obj}
