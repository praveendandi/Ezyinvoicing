...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("LMG-01")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace('Rate','')
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"LMG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LMG-01","invoice_date":date_time_obj}
