...
def file_parsing(filepath):
    ...
    try:    
        ...
        companyCheckResponse = check_company_exist("NCO-01")
        ...
        for i in raw_data:
            ...
            #     break
            # if "PRO-FORMA FOLIO" in i or "INFORMATION INVOICE" in i:
            #     information_folio = "Yes"
            # if "Reservation No." in i:
            #     confirmation_number = raw_data[raw_data.index(i)+1]
            #     confirmation_number = confirmation_number.split(" ")
            #     conf_number = confirmation_number[-2]
            #     create_doc(filepath, 'Redg Card', conf_number)
            #     print("conf noo",conf_number)
            #     break
            if "CREDIT TAX INVOICE" in i:
                invoice_category = "Credit Invoice"
            if "Confirmation No." in i or "Confirmation No. " in i or "Confirmation No. " in i or "Conf. No. " in i:
            ...
            if "Original Bill  date :" in i or "Original Bill Date :" in i:
                ...
                try:
                    ...
                    try:
                        ...
                        print("Invalid date format:", date_time_obj)
                        date_time_obj = None
            # if "Departure :" in i:
            #     depatureDateIndex = i.index('Departure')
            #     date_time_obj = ':'.join(
            #         i[depatureDateIndex:].split(':')[1:])[1:]
            if "Room No. :" in i or "Room No.  :" in i or "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST ID " in i or "GST ID  " in i or "GST ID   " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.", "").replace("PrintedBy/On","").strip() 
            if "Bill  No." in i or "Bill  No. " in i or "Bill No. :" in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
                if "NCO" in invoiceNumber:
        ...
        if check_invoice['success'] == True:
            ...
            if inv_data.docstatus == 2:
                ...
                #                                 'message': 'Duplicate Invoice', 'type': "Duplicate Invoice", "invoice_number": inv_data.name, "company": inv_data.company})
                #         update_document_bin(
                #             print_by, document_type, "", "Duplicate", filepath)
        ...
        if gspApiDataResponse['success'] == True:
            if guest['invoice_type'] == 'B2B':
                ...
                if checkTokenIsValidResponse['success'] == True:
                    ...
                    if getTaxPayerDetailsResponse['success'] == True:
                        ...
                        calulateItemsApiResponse = calulate_items({'items': guest['items'], "invoice_number": guest['invoice_number'], "company_code": company_code[
                                                                  'code'], "invoice_item_date_format": companyCheckResponse['data'].invoice_item_date_format, "sez": sez})
                        if calulateItemsApiResponse['success'] == True:
                            guest['invoice_file'] = filepath
                            if reupload == False:
                                insertInvoiceApiResponse = insert_invoice({"guest_data": guest, "company_code": company_code['code'], "taxpayer": getTaxPayerDetailsResponse['data'].__dict__, "items_data": calulateItemsApiResponse[
                                                                          'data'], "total_invoice_amount": total_invoice_amount, "invoice_number": guest['invoice_number'], "amened": amened, "sez": sez})
                                ...
                                insertInvoiceApiResponse = Reinitiate_invoice({"guest_data": guest, "company_code": company_code['code'], "taxpayer": getTaxPayerDetailsResponse[
                                                                              'data'].__dict__, "items_data": calulateItemsApiResponse['data'], "total_invoice_amount": total_invoice_amount, "invoice_number": guest['invoice_number'], "amened": amened, "sez": sez})
                                if insertInvoiceApiResponse['success'] == True:
                                    print("Invoice Created",
                                          insertInvoiceApiResponse)
                                    return {"success": True, "message": "Invoice Created"}
                                    ...
                                    print("insertInvoiceApi fialed:  ",
                                          insertInvoiceApiResponse['message'])
                                    # update_document_bin(
                                    #     print_by, document_type, invoiceNumber, error_data['error_message'], filepath)
                                    return {"success": False, "message": insertInvoiceApiResponse['message']}
                ...
                calulateItemsApiResponse = calulate_items({'items': guest['items'], "invoice_number": guest['invoice_number'], "company_code": company_code[
                                                          'code'], "invoice_item_date_format": companyCheckResponse['data'].invoice_item_date_format, "sez": 0})
                if calulateItemsApiResponse['success'] == True:
                    guest['invoice_file'] = filepath
                    if reupload == False:
                        insertInvoiceApiResponse = insert_invoice({"guest_data": guest, "company_code": company_code['code'], "items_data": calulateItemsApiResponse[
                                                                  'data'], "total_invoice_amount": total_invoice_amount, "invoice_number": guest['invoice_number'], "amened": amened, "taxpayer": taxpayer, "sez": 0})
                        if insertInvoiceApiResponse['success'] == True:
        ...
        frappe.log_error("Ezy-invoicing invoice_parser",
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "NCO" + invoiceNumber
