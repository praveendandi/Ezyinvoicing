...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("PPNDA-01")
        ...
        for i in raw_data:
            ...
            if "Bill No." in i or "Bill  No. " in i:
                Dateindex = raw_data.index(i)
                date_time_obj = raw_data[Dateindex-1].split(" ")[-1].strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                if "Printed By / On" in guest['name']:
        ...
        company_code = {"code":"PPNDA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"PPNDA-01","invoice_date":date_time_obj}
