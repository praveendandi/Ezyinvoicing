...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("KRG-01")
        ...
        for i in raw_data:
            ...
            if ("Total" in i and "INR" in i ) and "CP //" not in i:
            ...
            if "Company GSTIN  :" in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace("/","").replace("TAXINVOICE","").strip()
        ...
        company_code = {"code":"KRG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"KRG-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "KHRG"+invoiceNumber
