...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("NBT-01")
        ...
        for i in raw_data:
            ...
            if "Room No." in i or "Room No. :" in i or "Room No.  :" in i:
                ...
                roomNumber =roomNumber.replace("Guests","")
        ...
        for each in items:
            if each["name"] != "":
                if ("CGST" not in each["name"] or "SGST/CGST" in each["name"]) and ("SGST" not in each["name"] or "SGST/CGST" in each["name"])and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"]:
        ...
        guest['gstNumber'] = gstNumber.replace("ConfirmationNo","")
        guest['room_number'] = int(roomNumber) if roomNumber != "" else 0
        guest['company_code'] = "NBT-01"
        ...
        company_code = {"code":"NBT-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"NBT-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "NBT"+invoiceNumber
