...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("LC-01")
        ...
        for i in raw_data:
            ...
            if "GST ID  " in i or "GST ID  :" in i:
                ...
                gstNumber = gstNumber.replace("TAXINVOICE","").replace("Membership","").strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Rate","").replace("Arrival","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"LC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LC-01","invoice_date":date_time_obj}
