    ...
    try:
        ...
        for i in raw_data:
            ...
            if "CREDIT TAX INVOICE" in i:
                ...
                date_time_obj = raw_data[dateind+1].split(" ")
                date_time_obj = datetime.datetime.strptime(date_time_obj[-1],'%d/%m/%y').strftime('%d-%b-%y %H:%M:%S')
            if "Confirmation No" in i:
                confirmation_number = i.split(":")
                conf_number = confirmation_number[-1].replace(" ", "")
            if "Total" in i and "Totala" not in i:
                total_invoice = i.split(" ")
                total_invoice_amount = float(total_invoice[-2].replace(",", ""))
                if invoice_category in ["Credit Invoice", "CREDIT TAX INVOICE"] and "-" not in str(total_invoice_amount):
                    invoice_category = "Tax Invoice"
            ...
            if "Invoice No" in i:
                invoiceNumber = ((i.split(':')[len(i.split(':')) - 1]).replace(" ", "")).replace("GOIFG-","")
        ...
        for i in data:
            ...
            if len(check_date) > 0  and "." in split_str and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "VAT" not in i and "FAIRFIELD BY MARRIOTT GOA ANJUNA" not in i:
                item = dict()
                item_value = ""
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("GOIFG", "").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
