...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            print(i,"++++++++++++++++++++++")
            if "Reservation #" in i or "Reservation No" in i or "R  e s  e  r v ation # :" in i:
                if "Reservation #" in i or "R  e s  e  r v ation # " in i:
            ...
            if "Bill Date" in i or "BillDate" in i or "Bill Date & Time" in i or "Bill Date & Time " in i:
                date_time_obj = (i.split(':')[-1]).strip()
                date_time_obj = date_time_obj.split(" ")[0].strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest["name"] = guest["name"].replace("BillNumber","").strip()
