...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("FMBW-01")
        ...
        for i in raw_data:
            ...
            if "Room  " in i or "Room " in i and ":" in i and "CHECK#" not in i and "Rooms" not in i and "GST" not in i and "Room No" not in i and "Room no" not in i and "Show" not in i:
                room = i.split(":")
                roomNumber = room[-1]
            ...
            if "Invoice No" in i:
                ...
                if "-" in invoiceNumber:
                    ...
                    invoiceNumber = "BLRFK-"+invoicenum
            if "Bill To" in i:
        ...
        if invoiceNumber != reupload_inv_number:
            invoice_number = reupload_inv_number.lstrip("AKQEJ-")
            other_invoice_number = invoiceNumber.lstrip("BLRFK-")
        ...
        company_code = {"code":"FMBW-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMBW-01","invoice_date":date_time_obj}
