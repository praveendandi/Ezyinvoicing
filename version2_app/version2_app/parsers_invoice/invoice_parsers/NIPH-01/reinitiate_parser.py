...
def reinitiateInvoice(data):
    try:
        ...
        for i in raw_data:
            ...
            if "Original Bill Date " in i or "Original Bill Date :" in i or "OriginalBillDate:" in i:
            ...
            if "GST NO" in i or "Guest Tax ID :" in i and "HOTEL" not in i:
                if "GST NO." not in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Guests","").replace("Departure","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
