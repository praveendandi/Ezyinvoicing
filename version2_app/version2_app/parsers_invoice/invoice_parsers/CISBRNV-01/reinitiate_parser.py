...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("CISBRNV-01")
        ...
        for i in raw_data:
            ...
            if "GST ID  :" in i or "GST ID  " in i and "Hotel GST ID  " not in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.replace("Membership","").strip(".").strip()
            if "Bill  No. :" in i or "Bill  No. " in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "").lstrip('0')
                invoice_Number = datetime.datetime.strptime(date_time_obj,'%d-%b-%y %H:%M:%S').strftime('%y%m')
            ...
            # if i in "Dae Description Reference Debit Credit":
            entered = True
            if 'CGST 6%=' in i:
                entered = False
            # if 'Billing' in i:
        ...
        for index, i in enumerate(guestDeatils):
            if "Guest Name :" in i:
                guest['name'] = i.split('Guest Name :')[-1].strip()
        ...
        company_code = {"code":"CISBRNV-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CISBRNV-01","invoice_date":date_time_obj}
