...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RLH-01")
        ...
        for i in raw_data:
            ...
            if "GST IN" in i:
                gstNumber = (i.split('Cashier')[0]).replace("GST IN","").strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.strip()
                guest['name'] =guest['name'].replace("Original Bill","").replace(":","").strip()
            if index == 1:
                guest['address1'] = ""
            if index == 2:
                guest['address2'] = ""
        ...
        company_code = {"code":"RLH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RLH-01","invoice_date":date_time_obj}
