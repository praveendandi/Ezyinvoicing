...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("HICP-01")
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    ...
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Accommodation" or each["name"] == "Room Charges" or each["name"] == "Cognizant CP" or each["name"] == "Accommodation - Package" or each["name"] == "Tpi 00p Dbb (1*":
                        if ind+1 < len(items):
        ...
        company_code = {"code":"HICP-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HICP-01","invoice_date":date_time_obj}
