...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("JWMMJ-01")
		...
		for i in raw_data:
			...
			if "Invoice No  :" in i or "Invoice No  :" in i and "Original Invoice No." not in i:
		...
		company_code = {"code":"JWMMJ-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JWMMJ-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "39021-"+invoiceNumber
