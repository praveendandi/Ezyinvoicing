...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TWRH-01")
        ...
        for i in raw_data:
            ...
            if "Bill  No. " in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "").replace("DEDWI000","")
        ...
        company_code = {"code":"TWRH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TWRH-01","invoice_date":date_time_obj}
