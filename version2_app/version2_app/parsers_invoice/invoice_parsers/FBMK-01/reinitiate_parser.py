...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("FBMK-01")
        ...
        for i in raw_data:
            ...
            if ("Total" in i and "INR" in i) and "Sub Total" not in i and "Sub Trotal" not in i:
            ...
            if "GST ID" in i and "GST ID  " in i:
            ...
            if i in "Date Description Reference Debit Credit" and i in "Date Description Reference Debit Credit":
                entered = True
            if 'CGST 6%=' in i:
                entered = False
            # if 'Billing' in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "Bill  No." not in i and "Bill No." not in i and "Sub Total" not in i and "Sub Trotal" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        company_code = {"code":"FBMK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FBMK-01","invoice_date":date_time_obj}
