...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISNashik-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  Date :" in i or "Original Bill Date" in i or "Original Bill Date:" in i:
            ...
            if "GST NO" in i or "GST ID" in i or "GST NO  :" in i or "Guest Tax ID " in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace('PrintedBy/On','').strip()
                if "ConfirmationNo." in gstNumber or "Confirmation No. " in gstNumber:
                    gstNumber = gstNumber.replace("ConfirmationNo.","")
                    gstNumber = gstNumber.replace("ConfirmationNo","")
                if "Membership" in gstNumber:
                    gstNumber = gstNumber.replace("Membership","")
            if "Bill  No." in i or "Bill  No. " in i or "Invoice No. " in i or "Invoice No." in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = (i.split(':')[1]).replace("Guests","").replace("Departure","").strip()
        ...
        guest['company_code'] = "IBISNashik-01"
        ...
        company_code = {"code":"IBISNashik-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBISNashik-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "IBN-"+invoiceNumber
