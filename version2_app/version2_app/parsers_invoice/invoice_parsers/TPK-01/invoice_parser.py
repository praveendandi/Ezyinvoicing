...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("TPK-01")
        ...
        for i in raw_data:
            ...
            if ("Original Bill  :" in i or "Original Bill  " in i) and "Original Bill  date " not in i:
                data_obj = (i.split(":")[-1]).strip()
                date_time_obj = datetime.datetime.strptime(data_obj,'%d.%m.%y').strftime('%d-%b-%y %H:%M:%S')
            if date_time_obj == "":
                if "Departure " in i:
            ...
            if entered_value == True:
                emailDetails.append(i)
            if "Guest Name :" in i:
                entered_value = False
        # To find email id from  list
        ...
        for i in data:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        item_value = val[-1]
                        remove_characters = re.sub('[A-Za-z]', '', item_value)
                        item['item_value'] = float(remove_characters.replace(',', ''))
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Rate","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"TPK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TPK-01","invoice_date":date_time_obj}
        ...
        if gspApiDataResponse['success'] == True:
            if guest['invoice_type'] == 'B2B':
                ...
                if checkTokenIsValidResponse['success'] == True:
                    getTaxPayerDetailsResponse = get_tax_payer_details({"email":email_id,"gstNumber":guest['gstNumber'],"code":company_code['code'],"invoice":guest['invoice_number'],"apidata":gspApiDataResponse['data']})
                    if getTaxPayerDetailsResponse['success'] == True:
                        ...
                        if calulateItemsApiResponse['success'] == True:
                            ...
                            if reupload == False:
                                ...
                                taxpayer["email"]=email_id
                                insertInvoiceApiResponse = insert_invoice({"guest_data":guest,"taxpayer":taxpayer,"company_code":company_code['code'],"taxpayer":getTaxPayerDetailsResponse['data'].__dict__,"items_data":calulateItemsApiResponse['data'],"total_invoice_amount":total_invoice_amount,"invoice_number":guest['invoice_number'],"amened":amened,"sez":sez})
                ...
                taxpayer= {"legal_name": "","address_1": "","address_2": "","email": email_id,"trade_name": "","phone_number": "","location": "","pincode": "","state_code": ""}
