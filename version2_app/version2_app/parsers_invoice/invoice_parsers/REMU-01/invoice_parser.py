...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("REMU-01")
        site_folder_path = companyCheckResponse['data'].site_name
        company = frappe.get_doc("company","REMU-01")
        ...
        for i in raw_data:
            ...
            if "GST ID" in i:
                if company.only_b2c == "No":
                    ...
                    gstNumber = gstNumber.replace("Depart/InvDate","")
                else:
                    gstNumber = ""
            ...
            if "Original Bill  date" in i:
                ...
                i="Guest Name :{}".format(raw_data[index_val])
                guestDeatils.append(i)
            if "Membership" in i:
                Membership = i.split(":")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Confirmation No","")
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        if companyCheckResponse['data'].pms_information_invoice_for_payment_qr == "Yes" and informationInvoice == "Yes":
            if informationInvoice == "Yes":
                ...
                socket = frappe.publish_realtime("custom_socket", {'message':'Information Invoice Created','data':information_data,"company":"REMU-01"})
            ...
            if company.only_b2c == "Yes":
                ...
                if reupload == False:
                    ...
                    insertInvoiceApiResponse = insert_invoice({"guest_data":guest,"company_code":"REMU-01","items_data":total_items,"total_invoice_amount":total_invoice_amount,"invoice_number":guest['invoice_number'],"amened":amened,"taxpayer":taxpayer,"sez":0})
                    ...
                    guest['invoice_file'] = filepath
                    insertInvoiceApiResponse = Reinitiate_invoice({"guest_data":guest,"company_code":"REMU-01","items_data":total_items,"total_invoice_amount":total_invoice_amount,"invoice_number":guest['invoice_number'],"amened":amened,"taxpayer":taxpayer,"sez":0})
                    if insertInvoiceApiResponse['success']== True:
                        print("B2C Invoice Created",insertInvoiceApiResponse)
                        return {"success":True,"message":"Invoice Created"}
                    else:
            ...
            if informationInvoice == "No":
                ...
                company_code = {"code":"REMU-01"}
                error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"REMU-01","invoice_date":date_time_obj}
