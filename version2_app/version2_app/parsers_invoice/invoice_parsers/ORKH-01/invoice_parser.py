...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("ORKH-01")
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "cess" not in each['name'] and "Vat" not in each["name"] and "IGST" not in each["name"] and "STATE GST" not in each["name"] and "CENTRAL GST" not in each["name"]:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0: 
                ...
                # guest['name'] = re.sub('[0-9]|[-:,]',' ',guest['name']).replace("Date","").strip()
                # # guest['name'] = i.replace("BILL No.","").strip(",")
                if "Cashier" in i:
                    guest['name'] = i.split('Cashier')[0]
        ...
        company_code = {"code":"ORKH-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"ORKH-01","invoice_date":date_time_obj}
