...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("JWMM-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "Pvt" not in i:
            ...
            #     total_ind = raw_data[ind-1]
            #     total_invoice_amount=total_ind.split()
            #     total_invoice_amount = float(total_invoice_amount[-2].replace(",", ""))
            if "TAX INVOICE" in i:
                ind = raw_data.index(i)
                if re.search("^([0-9]{2}\-[A-Z]{3}\-[0-9]{4})+",raw_data[ind+1]):
                    date_time_obj = raw_data[ind+1]
            ...
            if "Bill Generation Date" in i:
                ...
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if ("Room  :" in i or "Room :" in i) and "CHECK#" not in i and "Rooms" not in i and "GST" not in i and "Room No" not in i and "Room#" not in i:
                room = i.split(":")
                roomNumber = room[-1]
            ...
            if "Cashier" in i and "Cashier Signature" not in i:
                print_by =  i.split(":")[-1]
        ...
        for i in original_data:
            ...
            split_str = strip_str.split(" ")[-1]
            if len(check_date) > 0 and "." in split_str and "@" not in split_str and "Room# " not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and ("IGST" not in i and "Reimburasable Exp - IGST" not in i) and ("CGST/SGST" in i or ("CGST" not in i and "SGST" not in i)) and "NEFT" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
                # dt = i
        ...
        company_code = {"code":"JWMM-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JWMM-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "Q1X-"+invoiceNumber
