...
def reinitiateInvoice(data):
    try:
        ...
        for i in original_data:
            ...
            if (len(check_date) > 0 and "." in split_str and "@" not in split_str and "Room# " not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "IGST" not in i and ("CGST/SGST" in i or ("CGST" not in i and "SGST" not in i)) and "NEFT" not in i) or "Reimburasable Exp - IGST" in i:
                item = dict()
                item_value = ""
                dt = i.strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = (guest["name"].replace("Cashier","")).strip()
