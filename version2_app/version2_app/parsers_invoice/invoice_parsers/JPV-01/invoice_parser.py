...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("JPV-01")
        ...
        for i in raw_data:
            ...
            if "Confirmation No" in i:
                ...
                cn = i[Index:]
                confirmation_number = cn.split(":")
                conf_number = confirmation_number[1].replace("Guest Name", "").strip()
            if "Total" in i:
            ...
            if "Room No. :" in i or "Room No.  :" in i:
                room = i.split(":")
                roomNumber = room[1].replace("Room Type","").strip()
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST ID" in i or "GST ID  " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("Guests","").replace("Rate","").strip()
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if "~" in i:
                        ...
                        if "CHECK" in item["item_reference"]:
                            ...
                            edit_date = datetime.datetime.strptime(item['date'],'%d/%m/%y').strftime('%d%m%y')
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i
                guest['name'] = guest['name'].replace("Invoice","").replace("Arrival","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"JPV-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JPV-01","invoice_date":date_time_obj}
