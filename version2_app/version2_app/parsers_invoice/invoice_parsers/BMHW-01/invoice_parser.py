...
def file_parsing(filepath):
    ...
    try:
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] or "Rebate Transport IGST" in each['name']:
            ...
            if inv_data.docstatus==2:
                ...
                if inv_data.invoice_type == "B2B":
                    if inv_data.irn_generated=="Pending" or inv_data.irn_generated == "Error":
                        ...
                        get_inv = frappe.db.get_list('Invoices',filters={"name":["like",guest['invoice_number']+'CN']})
                        if len(get_inv)>0:
                            get_inv_tax = frappe.db.get_list('Invoices',filters={"name":["like",guest['invoice_number']+'-1']})
                            if len(get_inv_tax)>0:
                                guest['invoice_number'] = guest['invoice_number']+'-2'
                                reupload = False
                                amened='No'
                            else:
                                guest['invoice_number'] = guest['invoice_number']+'-1'
                                reupload = False
                                amened='No'
                        else:
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "39854-"+invoiceNumber
