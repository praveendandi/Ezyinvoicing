    ...
    try:
        ...
        for i in raw_data:
            ...
            if "CREDIT TAX INVOICE" in i:
                ...
                date_time_obj = datetime.datetime.strptime(date_time_obj[0],'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "TAX INVOICE" in i and "CREDIT" not in i:
                date_time_obj = i.split(" ")
                date_time_obj = datetime.datetime.strptime(date_time_obj[0],'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Confirmation No" in i or "onfirmation No " in i:
                confirmation_number = i.split(":")
                conf_number = confirmation_number[-1].replace(" ", "")
            if "Total" in i and "INR" in i and ("CENTRE" not in i):
            ...
            if "Room No." in i or "Room No" in i or "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
            if "GST ID" in i or "Client GST ID  " in i:
                ...
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace("MCoenmfibremrsahtiiopnNo","").strip()
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Departure","").replace("Membership","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
