...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CBMC-01")
        ...
        for i in raw_data:
            ...
            if "Confirmation No " in i or "Confirmation No :" in i:
                ...
                conf_number = confirmation_number[1].replace(" ", "").replace("OriginalBillDate","")
            if "Total : INR" in i or "Total  :  INR" in i and ("Tax:" not in i):
            ...
            if "Departure :" in i:
                date_time_obj = (i.split("Departure")[1]).replace(":","").strip()
            ...
            if "Bill No :" in i or "Bill No " in i:
                invoiceNumber = i.split(":")[-1]
                # invoiceNumber = ''.join(filter(lambda i: i.isdigit(), invoiceNumber)).lstrip("0")
                invoiceNumber = invoiceNumber.replace("Date","").strip()
            if "Charged To" in i:
        ...
        company_code = {"code":"CBMC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "MACCY-" + invoiceNumber.strip()
