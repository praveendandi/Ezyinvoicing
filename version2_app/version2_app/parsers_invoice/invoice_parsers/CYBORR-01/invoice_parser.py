...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CYBORR-01")
        ...
        for i in raw_data:
            ...
            # if "TAX INVOICE" in i and "Original Bill No." not in i and "CREDIT TAX INVOICE" not in i and "COPY OF INVOICE" not in i:
            #     ind = raw_data.index(i)
            #     date_obj = raw_data[ind+2]
            #     inv_date = datetime.datetime.strptime(date_obj, '%d-%m-%y')
            #     date_time_obj = inv_date.strftime("%d-%b-%y %H:%M:%S")
            # if date_time_obj == "":
            ...
            # if "Depart :" in i:
            #     # print(i.split(":")[-5])
            #     date_time_obj = i.split(":")[-4].replace(" Time ","").strip()
            #     date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room  :" in i or "Room :" in i:
                room = i.split(":")
                roomNumber = room[-1]
            if "GST No." in i and ":" in i or "GST No.               " in i:
                gstNumber = i.split(':')[-1].replace('Original Bill Date', '').strip()
            if "Invoice # :" in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")       
                invoiceNumber = invoiceNumber.lstrip('3')
            ...
            if 'Billing' in i:
                entered = False
            if 'Total' in i and "Raindrops" not in i:
                entered = False
            if entered == True:
                data.append(i)
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Confirmation No","").replace("Time printed","")
                guest['name'] = i.split(':')[1].replace("Confirmation No","")
                guest['name'] = guest['name'].replace("Time printed","").strip()   
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"CYBORR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CYBORR-01","invoice_date":date_time_obj}
