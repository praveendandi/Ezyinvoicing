...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("AOSRB-01")
		...
		for i in raw_data:
			i = i.strip()
			# print(i,"++++++++++++")
			if "Confirmation No." in i or "Confirmation No. " in i or "Conf. No. " in i or "Conf. No. " in i:
				confirmation_number = i.split(":")
				conf_number = confirmation_number[-1].replace(" ", "")
			if ("Total" in i and "INR" in i) and "Totale" not in i and "PAYMENT" not in i:
			...
			if "OriginalBillDate:" in i or "Original Bill Date " in i or "Original Bill Date  " in i or "Original Bill Date :" in i:
			...
			if "Guest Tax ID :" in i or "Guest GST ID   :" in i or "GST ID   " in i or "GST ID " in i:
		...
		for i in data:
			...
			if len(check_date) > 0 and "." in split_str and "Page" not in i:
				item = dict()
				item_value = ""
				for index, j in enumerate(i.split(' ')):
					...
					if len(val)>1:
						# item_value = val[-1]
						item_value = re.findall(r"([-0-9]+[,.\]+[0-9]+\d)",val[-1])[0]
						item['item_value'] = float(item_value.replace(',', '').replace("#443=",""))
					# else:
					# 	item_value = val[-2]
					...
					if "~" in i:
						...
						item['item_reference'] = i[start_index:end_index].replace("~","").strip()
						item["item_reference"] = item["item_reference"].replace("[NA Room]","").strip()
						if "CHECK" in item["item_reference"]:
							item["item_reference"]= item["item_reference"].split("CHECK")[1]
							item["item_reference"] = item["item_reference"].replace("#","").strip()
							item["item_reference"]= item["item_reference"].replace(item["item_reference"][:3], '')
							# item["item_reference"] = item["item_reference"].lstrip("0")
							if "[" in item["item_reference"]:
								item["item_reference"] = item["item_reference"].split("[")[0]
								item["item_reference"] = item["item_reference"].strip()
								edit_date = datetime.datetime.strptime(item['date'],'%d-%m-%y').strftime('%Y%m')
								item["reference_check_number"] = f"{edit_date}{item['item_reference']}"
							else:
								edit_date = datetime.datetime.strptime(item['date'],'%d-%m-%y').strftime('%Y%m')
								item["reference_check_number"] = f"{edit_date}{item['item_reference']}"
						else:
		...
		company_code = {"code":"AOSRB-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"AOSRB-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "NHCC" + invoiceNumber
