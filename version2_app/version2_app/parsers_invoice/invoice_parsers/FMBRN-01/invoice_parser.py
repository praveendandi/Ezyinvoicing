...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FMBRN-01")
        ...
        for i in raw_data:
            # print(i,"///////////")
            i = i.strip()
            if "TAX INVOICE" not in i and "printed on" not in i and "COPY OF TAX INVOICE" not in i:
                # ind = raw_data.index(i)
                # dateobj = raw_data[ind+1].split(" ")[0].strip()
                if "Invoice date :" in i  or "Group :" in i:
                    date_time_obj = (i.split(":")[-1]).strip()
                    date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            elif "TAX INVOICE" not in i and "printed on" not in i and "COPY OF TAX INVOICE" not in i:
                ind = raw_data.index(i)
                dateobj = raw_data[ind+1].split(" ")[0].strip()
                # match_obj=re.match('[0-9]{2}\-[0-9]{2}\-[0-9]{2}',dateobj)
                # if match_obj is not None:
                date_time_obj = datetime.datetime.strptime(dateobj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "CREDIT INVOICE" in i and "COPY" not in i and "printed on" not in i:
                ind = raw_data.index(i)
                dateobj = raw_data[ind+1].split(" ")[0].strip()
                date_time_obj = datetime.datetime.strptime(dateobj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            ...
            if "Invoice date :" in i:
                ...
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')            
            if ("Room No :" in i and ":" in i and "CHECK#" not in i and "Rooms" not in i and "GST" not in i and "Room No-" not in i and "Loyalty" not in i and "Roomi" not in i and "Room No." not in i and "Comp" not in i) or ("Room No" in i and "Room#" not in i and "ROOMS" not in i and "Room No-" not in i and "Roomi" not in i and "Room No." not in i and "Room Group" not in i):
                if "Room No :" in i or "Room No :" or "Room No " in i:
                    room = i.split(":")
                    roomNumber = room[-1]
                    roomNumber = roomNumber.replace("Room No","").strip()
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST No." in i or "GST ID   :" in i or "GST ID :" in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                # if "ConfirmationNo." in gstNumber:
                gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.replace("Membership","")
                gstNumber = gstNumber.replace("Page#","")
                gstNumber = gstNumber.replace("Page #","")
                gstNumber = gstNumber.strip()
            if "Invoice No" in i or "Invoice# :" in i:
        ...
        for i in original_data :
            ...
            if len(check_date) > 0 and "." in split_str and "IGST" not in i and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "VAT" not in i and "Loyalty No" not in i and "CHEQUE" not in i and "#" not in i : 
                item = dict()
                item_value = ""
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                # guest['name'] = i.split(':')[1].replace('Cashier', '')
                # guest['name'] = guest['name'].replace('BSRIN255','').replace("SUPERVISOR","").strip()
                if "Cashier" in i:
                    guest['name'] = i.split('Cashier')[0].replace('Guest Name :', '').strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"FMBRN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMBRN-01","invoice_date":date_time_obj}
