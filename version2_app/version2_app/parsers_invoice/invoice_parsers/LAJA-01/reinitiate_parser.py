...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("LAJA-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i:
                ...
                print(total_invoice_amount,"L:::::::::::L:")
            # if "Departure :" in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        # val[-1]=val[-1].replace(',', '')
                        item_value = re.findall(r"-?\d+,?\d+,?\.?\d*",val[-1])[0]
        ...
        company_code = {"code":"LAJA-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LAJA-01","invoice_date":date_time_obj}
