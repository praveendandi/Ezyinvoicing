...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("TWKR-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  :" in i:
                ...
                date_obj = re.findall("([0-9]{2}\/[0-9]{2}\/[0-9]{2})+", date_time_obj)
                if len(date_obj)>0:
                    date_obj = date_obj[0]
            ...
            if "Company GSTN" in i:
                ...
                gstNumber = gstNumber.replace("Depart/InvDate","").strip()
        ...
        company_code = {"code":"TWKR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TWKR-01","invoice_date":date_time_obj}
