...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CBMM-01")
        ...
        for i in raw_data:
            ...
            if "GST IN        " in i or "GST IN " in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("Page","")  
                # gstNumber = gstNumber.replace("CheckOutBy","")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[0].strip().replace("Room  :","").replace("Room","")
        ...
        company_code = {"code":"CBMM-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CBMM-01","invoice_date":date_time_obj}
