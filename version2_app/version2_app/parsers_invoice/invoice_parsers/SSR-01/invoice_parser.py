...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("SSR-01")
        ...
        for i in raw_data:
            ...
            if "Departure" in i:
                    ...
                    date_time_obj = datetime.datetime.strptime(date_time_obj,'%d/%m/%y %H:%M:%S').strftime('%d-%b-%y %H:%M:%S')
            if "Room No." in i or "Room No" in i or "Room" in i:
                room = i.split(":")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST ID " in i:
                gstNumber = i.split(':')[1]
                gstNumber = gstNumber.split(" ")[1].replace("Arrival","")
                gstNumber = gstNumber.replace("ConfirmationNo","")
                gstNumber = gstNumber.strip()
            if "Bill No." in i:
                ind = i.rindex("Bill No.")
                invoiceNumber = (i[ind:].replace("Bill No.","")).replace("/","-").strip()
        ...
        for i in data:
            ...
            print(check_date,"++++++++")
            if len(check_date) > 0 and "SHIV" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
                for index, j in enumerate(i.replace("  "," ").split(' ')):
                    val = dt.split(" ")
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "State GST" not in each['name'] and "Central GST" not in each['name']:
        ...
        if check_invoice['success']==True:
            ...
            print(inv_data.docstatus,inv_data.name)
            if inv_data.docstatus==2:
                ...
                if inv_data.invoice_type == "B2B":
                    ...
                    reupload = True
        print(invoiceNumber)
        company_code = {"code":"SSR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"SSR-01","invoice_date":date_time_obj}
