...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISMV-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill Date" in i or "Original Bill Date:" in i:
                ...
                # date_time_obj = datetime.datetime.strptime(date_obj,'%d-%b-%Y').strftime('%d-%b-%y %H:%M:%S')
                try:
            ...
            if "Total" in i and "INR" in i:
                match = re.search(r'Total\s+INR\s+(\d+(?:,\d+)*(?:\.\d+)?)', i)
                if match:
            ...
            if "Room No. " in i:
                ...
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
                    ...
                    room = i.split(" ")
                    roomNumber = room[-1]
            if "Guest Tax ID :" in i or "Guest Tax ID  " in i:
            ...
            if inv_data.docstatus==2:
                ...
                if inv_data.invoice_type == "B2B":
                    if inv_data.irn_generated=="Pending" or inv_data.irn_generated == "Error":
                        reupload = True
                    elif inv_data.credit_note_raised == "Yes":
                        amened='No'
                        guest['invoice_number'] = inv_data.name+"-1"
                    else:
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "IMVK"+invoiceNumber
