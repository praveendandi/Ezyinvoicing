...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RBRSK-01")
        ...
        for i in raw_data:
            ...
            if ("GST ID" in i or "GST ID  :" in i) and "Hotel GST ID" not in i:
                ...
                gstNumber = gstNumber.replace("-","").replace("/","").strip()
        ...
        company_code = {"code":"RBRSK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RBRSK-01","invoice_date":date_time_obj}
