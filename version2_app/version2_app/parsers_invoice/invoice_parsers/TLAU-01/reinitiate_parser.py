...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("TLAU-01")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i and "Billing" not in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        # val[-1]=val[-1].replace(',', '')
                        item_value = re.search(r"\d*\,?\d*\,?\d*\,?\d*\,?\.?\d+",val[-1]).group()
        ...
        company_code = {"code":"TLAU-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"TLAU-01","invoice_date":date_time_obj}
