...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HP-01")
        ...
        for i in raw_data:
            ...
            if "Property GST ID :" in i:
                ...
                dateobj = raw_data[ind-1].replace("TAX INVOICE","").replace("CREDIT INVOICE","").strip()
        ...
        for each in items:
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "sgst" not in each["name"] and "cgst" not in each["name"]:
        ...
        company_code = {"code":"HP-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HP-01","invoice_date":date_time_obj}
