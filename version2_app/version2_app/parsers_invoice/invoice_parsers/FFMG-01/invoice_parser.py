...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FFMG-01")
        ...
        for i in raw_data:
            ...
            if "Bill  No. :" in i or "Bill  No." in i or "Bill No. " in i or "Bill No." in i:
            ...
            if "GST ID  " in i or "GST ID  :" in i and "Property GST ID" not in i:
            ...
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                    if each["name"] not in payment_Types:
                        ...
                        if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Accommodation" or each["name"] == "Room Charges" or each["name"] == "Cognizant CP":
                            if ind+1 < len(items):
        ...
        company_code = {"code":"FFMG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FFMG-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = invoiceNumber.replace("0001","")
