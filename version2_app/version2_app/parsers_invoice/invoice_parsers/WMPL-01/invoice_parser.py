...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("WMPL-01")
        ...
        for i in raw_data:
            ...
            if date_time_obj == "":
                if "Original Bill  date :" in i:
                    dateinv = (i.split(":")[-1]).strip()
                    date_time_obj = datetime.datetime.strptime(dateinv,'%d-%b-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Departure :" in i:
                depatureDateIndex = i.index('Departure')
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    #     if "NA Pkg. Trx" in item["item_reference"]:
                    #         print(item["item_reference"],"-----------++++++++++++")
                    # else:
                    #     item["item_reference"] = ""
                itemsort+=1
                if item !={}:
        ...
        company_code = {"code":"WMPL-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"WMPL-01","invoice_date":date_time_obj,"checkout_date":checkout_date if checkout_date != '' else None}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "9784-"+invoiceNumber
