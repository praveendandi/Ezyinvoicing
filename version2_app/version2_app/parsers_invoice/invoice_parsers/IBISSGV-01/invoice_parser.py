...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("IBISSGV-01")
        ...
        for i in raw_data:
            # print(i,"++++++++++++++++++")
            i = i.strip()
            if "CREDIT INOVICE" in i:
                invoice_category = "Credit Invoice"
            if "Confirmation No." in i or "Confirmation No. " in i or "Conf. No. :" in i or "Confirmation No " in i:
                confirmation_number = i.split(":")
                conf_number = confirmation_number[-1].replace(" ", "")
            if "OriginalBillDate:" in i or "Original Bill Date " in i or "Original Bill Date" in i:
            ...
            if "Guest Tax ID " in i or "Guest Tax ID :" in i or "GST ID   :" in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("Membership", "").replace("PrintedBy/On","").strip()
                        ...
                        if calulateItemsApiResponse['success'] == True:
                            ...
                            if reupload == False:
                                ...
                                if insertInvoiceApiResponse['success'] == True:
                                    ...
                                    update_document_bin(
                                        print_by, document_type, "", error_data['error_message'], filepath)
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "IBIS-"+invoiceNumber
