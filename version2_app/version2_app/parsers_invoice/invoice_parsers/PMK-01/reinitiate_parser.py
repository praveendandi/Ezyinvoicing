...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("PMK-01")
        ...
        for i in raw_data:
            ...
            if "Total          INR" in i or "Total          INR" in i:
            ...
            if "GST ID  " in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace("Membership","").strip()
            if "Bill  No. " in i:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace("Loyalty No.","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
        ...
        company_code = {"code":"PMK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"PMK-01","invoice_date":date_time_obj}
