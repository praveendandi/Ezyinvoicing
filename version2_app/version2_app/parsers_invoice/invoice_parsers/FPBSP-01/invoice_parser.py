...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Total" in i and "INR" in i and ("Total Advance receivied" not in i and "Total Advance recyeivied" not in i):
            ...
            if "Room No." in i or "Room No" in i or "Room No. :" in i:
                room = i.split(":")
                roomNumber = room[-1]
            if ("GST ID  :" in i or "GST ID  :" in i) and "Property GST ID  :" not in i and "Property GST ID  :" not in i:
                gstNumber = i.split(':')[1].replace(' ', '').replace('CREDITINVOICE','')
                gstNumber = gstNumber.replace("ConfirmationNo.","").replace('TAXINVOICE','')
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] :
                if each["name"] not in payment_Types:
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Room Rate C" or each["name"] == "Members Only Rates" or each["name"] == "March Getaway Sale":
                        if ind+1 < len(items):
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Departure","").replace(".","").strip()
            if index == 1:
                guest['address1'] = i
            if index == 2:
                guest['address2'] = i
