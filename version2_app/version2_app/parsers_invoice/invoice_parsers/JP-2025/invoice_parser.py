...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("JP-2025")
        ...
        for i in raw_data:
            ...
            if "Total" in i and "in INR" in i and "Business" not in i:
                total_invoice = i.split(" ")
                total_invoice_amount = float(total_invoice[-2].replace(",", ""))
            if ("Departure" in i or "Departure " in i) and "Departure," not in i and "CP*" not in i:
                date_time_obj =i.split(":")[1].strip().split(" ")[0]
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%b-%y').strftime('%d-%b-%y %H:%M:%S')
            if ("Room No :" in i or "Room No" in i ) and "Room No 1" not in i and  "Room No.1" not in i and "12" not in i:
                room = i.split(":")
                roomNumber = room[-1]
            ...
            if "TAX INV No." in i or "TAX INV No. " in i:
            ...
            if 'CGST 6%=' in i:
                entered = False
            if 'Billing' in i or "Billing  " in i:
                entered = False
            if 'Total' in i:
                entered = False
            if entered == True:
        ...
        company_code = {"code":"JP-2025"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JP-2025","invoice_date":date_time_obj}
