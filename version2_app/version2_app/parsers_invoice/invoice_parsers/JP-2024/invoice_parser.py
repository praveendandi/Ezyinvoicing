...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("JP-2024")
        ...
        for i in raw_data:
            ...
            if "Checkout By" in i:
                index_value = i.index("Checkout By")
                p = i[index_value:].split(":")
                print_by = p[1].replace("Room Rate","")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "Confirmation No" not in i and "17/12/20,09/02/21,08/03/21,12/10/2 & SR no. 15022/2" not in i:
                item = dict()
                item_value = ""
                dt = i.strip()
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        ...
                        check_digits=re.findall("\d+", item['item_value'])
                        if check_digits:
                            if 'BHOOM' in item_value:
                                item['item_value']=float(item['item_value'].replace('BHOOM',''))
                            else:
        ...
        company_code = {"code":"JP-2024"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JP-2024","invoice_date":date_time_obj}
