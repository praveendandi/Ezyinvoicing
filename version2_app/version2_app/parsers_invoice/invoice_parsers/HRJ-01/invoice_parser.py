...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HRJ-01")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest['name'] = guest['name'].replace("Property GST ID ","").strip()
            if guest['name'] == "":
                if index == 1:
                    guest['name'] = i.split(':')[0]
        ...
        company_code = {"code":"HRJ-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HRJ-01","invoice_date":date_time_obj}
