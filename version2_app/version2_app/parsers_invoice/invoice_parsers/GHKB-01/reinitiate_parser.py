...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("GHKB-01")
        ...
        for i in raw_data:
            ...
            if "GST ID" in i or "GST ID  :" in i and "Pan No" not in i and "Property GST ID" not in i:
                gstNumber = i.split(':')[1].replace(' ', '')
                # gstNumber = gstNumber.replace("ConfirmationNo.","")
                gstNumber = gstNumber.replace("COPY OF INVOICE","").strip()
                gstNumber = gstNumber.replace("COPYOFINVOICE","").replace("PAN","").strip()
        ...
        company_code = {"code":"GHKB-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"GHKB-01","invoice_date":date_time_obj,"checkout_date":checkout_date if checkout_date != '' else None}
