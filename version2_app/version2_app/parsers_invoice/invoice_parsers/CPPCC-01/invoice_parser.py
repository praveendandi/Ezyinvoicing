...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CPPCC-01")
        ...
        for i in raw_data:
            ...
            if "GSTIN ID" in i:
                ...
                gstNumber = gstNumber.replace("TAXINVOICE","").replace("ConfirmationNo","")
            if "Bill  No." in i:
            ...
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] != "":
                    if each["name"] not in payment_Types:
                        if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Room Rate" or each["name"] == "Mosaic Breakfast Package" or each["name"] == "Bed and Breakfast" or each["name"] == "Accommodation Charges" or each["name"] == "Accommodation" or each["name"]== "Accommodation ++":
                            if ind+1 < len(items):
        ...
        company_code = {"code":"CPPCC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CPPCC-01","invoice_date":date_time_obj}
