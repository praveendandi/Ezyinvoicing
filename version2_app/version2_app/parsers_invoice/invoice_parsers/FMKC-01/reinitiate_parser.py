...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("FMKC-01")
        site_folder_path = companyCheckResponse['data'].site_name
        print(folder_path,site_folder_path,filepath,"********************")
        ...
        for index, i in enumerate(raw_data):
            # print(i)
            if index == 0:
                date_time_obj = datetime.datetime.strptime(i,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Confirmation No." in i or "Caonfirmation No " in i or "Confirmation No " in i:
            ...
            # if "Depalrture" in i:
            #     depatureDateIndex = i.index('Depalrture')
            #     date_time_obj = ((i[depatureDateIndex:]).replace("Depalrture :","")).strip()
            #     date_time_obj = datetime.datetime.strptime(date_time_obj, '%d-%b-%y %H:%M:%S').strftime("%d-%b-%y %H:%M:%S")
            if "Room No." in i:
                room = i.split(":")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "GST ID" in i and "CIN No." not in i:
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str and "Original Bill" not in i:
                item = dict()
                item_value = ""
        ...
        company_code = {"code":"FMKC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMKC-01","invoice_date":date_time_obj}
