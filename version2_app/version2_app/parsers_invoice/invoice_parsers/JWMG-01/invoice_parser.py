...
def file_parsing(filepath):
	...
	try:
		...
		companyCheckResponse = check_company_exist("JWMG-01")
		...
		for i in raw_data:
			...
			if "GST No.               :" in i or "GST No." in i and "Hotel GST No.  " not in i:
			...
			# if i in "Date Description Reference Debit Credit" or i in "DATE  REFERENCE CHARGES CREDITS":
			entered = True
			if 'CGST 6%=' in i:
				entered = False
			if 'Billing' in i:
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				...
				guest['name'] = guest['name'].replace("ConfirmationNo","").strip()
			if index == 1:
				guest['address1'] = i
			if index == 2:
				guest['address2'] = i
		...
		company_code = {"code":"JWMG-01"}
		error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"JWMG-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "GOIVM-"+invoiceNumber
