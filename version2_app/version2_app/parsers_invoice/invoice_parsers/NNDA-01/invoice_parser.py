...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            ...
            if "Invoice Date " in i or "Original Bill Date :" in i or "Original Bill Date  " in i:
            ...
            if "Company GSTIN  " in i or "GST ID   :" in i or "GST ID   " in i or "GST NO  " in i:
            ...
            if "Bill No. :" in i or "Bill No. :" in i or "Bill No. " in i or "Bill  No. " in i:
                inv = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
                if "N-" in inv:
                    invoiceNumber = inv.strip()
                else:
                    invoiceNumber = ""
            if "Bill To" in i:
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Service Tax" not in each['name']:
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                ...
                guest["name"] = guest["name"].replace("Arrival","").replace("Departure","")
        ...
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"NNDA-01","invoice_date":date_time_obj}
