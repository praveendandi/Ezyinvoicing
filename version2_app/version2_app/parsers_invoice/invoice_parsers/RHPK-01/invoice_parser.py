...
def file_parsing(filepath):
	...
	try:
		...
		for ind, each in enumerate(items):
			if ("CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]) or "Room Charges - IGST12" in each["name"] or "Laundry - Normal - IGST" in each["name"]:
