...
def file_parsing(filepath):
    ...
    try:
        ...
        date_time = today.strftime("%d-%b-%y %H:%M:%S")
        content = []
        ...
        for i in raw_data:
            ...
            # if ("Room No." in i or "Room No. " in i or "Room No. :") and ("Room No. : CE-" not in i and "com" not in i):
            if "Room No. :" in i and "Room No. : CE-" not in i and "com" not in i:
                room = i.split(" ")
                roomNumber = room[-1]
                # roomNumber = ''.join(filter(lambda j: j.isdigit(), i))
            if "Guest GST ID " in i or "Guest Tax ID " in i or "GST ID   :" in i:
        ...
        guest['invoice_date'] = date_time_obj if date_time_obj != "" else str(date_time)
        ...
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"IBISNDA-01","invoice_date":date_time_obj}
