...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RLCC-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  " in i and "Original Bill  date :" not in i:
                # print(i.split(":"),"dateeeeeeeeeeeeeeeee")
            ...
            if "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
            # if "GST ID  :" in i and "Hotel GST ID :" in i:
            #     gstNumber = i.split(':')[1].replace(' ', '').replace('/','')
            #     gstNumber = gstNumber.replace("TAXINVOICE", "").replace("CREDITINVOICE","")
            #     print(gstNumber,"+++++++++")
            if "GST No.  :" in i and "GSTIN No.:" not in i:
        ...
        company_code = {"code":"RLCC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"RLCC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "RM000"+invoiceNumber
