...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FMCOMR-01")
        ...
        for i in raw_data:
            ...
            if date_time_obj == "":
                if "Departure" in i or "Original Bill :" in i:
            ...
            if "Invoice No" in i or "Invoice No  " in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).split("-")[-1].replace(" ", "").replace("-","")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "." in split_str and "Arrival" not in i:
                item = dict()
                item_value = ""
        ...
        company_code = {"code":"FMCOMR-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FMCOMR-01","invoice_date":date_time_obj}
