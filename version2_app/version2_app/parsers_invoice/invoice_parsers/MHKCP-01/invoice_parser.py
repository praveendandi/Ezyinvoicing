...
def file_parsing(filepath):
    ...
    try:
        ...
        for each in items:
            if each["name"] != "":
                if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "Room No." not in each['name'] and "Allow " not in each['name']:
        ...
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"MHKCP-01","invoice_date":date_time_obj}
