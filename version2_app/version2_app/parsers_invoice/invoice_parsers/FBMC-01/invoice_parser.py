...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("FBMC-01")
        ...
        for i in raw_data:
            ...
            if "GST ID  " in i or "GST ID  :" in i:
                ...
                gstNumber = gstNumber.replace("TAX INVOICE","").replace("CREDIT INVOICES","")
                gstNumber=gstNumber.replace("COPY OF INVOICE","").replace("INVOICE","").strip()
            if "Bill  No. " in i or "Bill  No. :" in i:
            ...
            if "Date" in i and "Description" in i and  "Reference" in i:
                entered = True
            if 'CGST 6%=' in i:
                entered = False
            if 'Billing' in i:
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    specialchar_find=re.search("[@_!#$%^&*()<>?/\|}{~:]",each["name"])
                    # if specialchar_find!=None:
                    #     find_index=each["name"].find(specialchar_find.group())
                    #     each["name"]=each["name"][0:find_index-1]
                    if each["name"] == "Room Charge" or each["name"] == "Room charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Accommodation" or each["name"] == "Room Charges" or each["name"] == "Cognizant CP" or each["name"] == "Prepay Refundable With Breakfast and W" or each["name"]=="Superior Queen room pre-pay non refund" or each["name"] == "PRE-PAY REFUNDABLE RATE CODE" or each["name"] == "Package" :
                        if ind+1 < len(items):
        ...
        company_code = {"code":"FBMC-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"FBMC-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "2--"+ invoiceNumber.lstrip("0")
