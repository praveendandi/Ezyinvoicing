...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HCPGN-01")
        ...
        for i in data:
            ...
            if len(check_date) > 0 and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "IGST" not in i:
                ...
                if len(items_split)>4 and "." in items_split[-1]:
                    ...
                    #     item["name"] = item["name"][:name_index]
                    if "Paid Out-Food" in i:
                        item['name'] = "Paid Out-Food"	
        ...
        company_code = {"code":"HCPGN-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HCPGN-01","invoice_date":date_time_obj}
