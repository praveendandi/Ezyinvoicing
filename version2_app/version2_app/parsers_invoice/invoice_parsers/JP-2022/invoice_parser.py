...
def file_parsing(filepath):
    ...
    try:
        ...
        for i in raw_data:
            print(i,"++++++++++++++++")
            # if 'GUEST REGISTRATION CARD' in i:
            #     redg_card = 'Yes'
            #     ele_index = raw_data.index(i)
            #     conf_number = raw_data[ele_index+1].strip()
            ...
            if "INFORMATION INVOI" in i or "INFORMATION INVOICE" in i:
            ...
            if "GST ID" in i and "GST ID-" not in i:
                ...
                gstNumber = gstNumber.replace("RoomType","")
            if "Tax Invoice No." in i or "COPY OF INVOICE" in i or "INVOICE" in i:
                invoiceNumber = (i.split(':')[len(i.split(':')) - 1]).replace(" ", "")
            if "Page" in i and invoiceNumber=="":
                find_index=raw_data.index(i)-1
                val=raw_data[find_index]
                invoiceNumber = (val.split(':')[-1]).replace(" ", "")
            if "Bill To" in i:
...
def create_doc(file_path, docType, conf_number=""):
    print(file_path)
    company = frappe.get_doc("company","JP-2022")
