...
def reinitiateInvoice(data):
    try:
        ...
        companyCheckResponse = check_company_exist("LTVK-01")
        ...
        for i in raw_data:
            ...
            if "Original Bill  :" in i:
                ...
                # date_obj = (raw_data[ind+1].split(":"))
                date_obj = raw_data[ind-1].split(":")[-1]
                date_obj = date_obj.lstrip()
                # date_obj = re.sub(r"^\s+", "", date_obj, flags=re.UNICODE)  
                inv_date = datetime.datetime.strptime(date_obj, '%d-%m-%y')
                date_time_obj = inv_date.strftime("%d-%b-%y %H:%M:%S")
            # if "TAX INVOICE" in i or "Original Bill  :" in i or "Bill Date  :" in i and "CREDIT TAX INVOICE" not in i:
            #     ind = raw_data.index(i)
            ...
            if len(check_date) > 0:
                ...
                for index, j in enumerate(i.split(' ')):
                    ...
                    if len(val)>1:
                        ...
                        if re.search("[a-zA-Z=@]+",item_value):
                            split_value = re.split("[a-zA-Z=]+", item_value)[-1]
                            item["item_value"] = float(split_value.replace(',', '').replace("#",""))
                        else:
                            item_value = re.sub("[a-zA-Z]","",item_value)
                            item['item_value'] = float(item_value.replace(',', '').replace("#",""))
                    # else:
                    # 	item_value = val[-2]
                    # 	item['item_value'] = float(item_value.replace(',', ''))
        ...
        company_code = {"code":"LTVK-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"LTVK-01","invoice_date":date_time_obj}
            ...
            if guest['invoice_type'] == 'B2B':
                ...
                if checkTokenIsValidResponse['success'] == True:
                    ...
                    if getTaxPayerDetailsResponse['success'] == True:
                        ...
                        calulateItemsApiResponse = calulate_items({'items':guest['items'],"invoice_number":guest['invoice_number'],"company_code":company_code['code'],"invoice_item_date_format":companyCheckResponse['data'].invoice_item_date_format,"sez":sez,"place_of_supply":"23"})
