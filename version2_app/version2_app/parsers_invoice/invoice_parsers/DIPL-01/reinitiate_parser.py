...
def reinitiateInvoice(data):
    try:
        ...
        for i in raw_data:
            ...
            if ("Original Bill  :" in i or "Original Bill :" in i) and "TAX INVOICE" not in i:
                date_time_obj = (i.split(":")[-1]).strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if ("Original Bill  :" in i or "Original Bill :" in i) and "TAX INVOICE" in i:
                date_time_obj = (i.split(" ")[-1]).strip()
                date_time_obj = datetime.datetime.strptime(date_time_obj,'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No." in i or "Room No" in i and "Room" not in i:
                room = i.split(":")
                roomNumber = room[-1]
            if "GST ID" in i:				
        ...
        for ind, each in enumerate(items):
            if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"]:
                if each["name"] not in payment_Types:
                    ...
                    if each["name"] == "Room Charge" or each["name"] == "Package Room Charge" or each["name"] == "Accommodation & Breakfast" or each["name"] == "Accommodation" or each["name"] == "Bed and Breakfast":
                        if ind+1 < len(items):
