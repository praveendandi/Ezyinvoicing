...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("CMSHILLONG-01")
        ...
        for i in raw_data:
            # print(i)
            if "INFORMATION INVOICE" in i or "I NFORMATION INVOICE" in i or "INFORMATION INVOICE" in i:
            ...
            if "GST ID  " in i and "Hotel GST ID  :" not in i:
                ...
                gstNumber = gstNumber.replace("Membership","").replace("ConfirmationNo","")
        ...
        for index, i in enumerate(guestDeatils):
            if index == 0:
                guest['name'] = i.split(':')[1].replace('Arrival','')
        ...
        guest['company_code'] = "CMSHILLONG-01"
        ...
        company_code = {"code":"CMSHILLONG-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"CMSHILLONG-01","invoice_date":date_time_obj}
...
def invoiceNumberMethod(invoiceNumber):
    invoiceNumber = "SHLCY-"+invoiceNumber
