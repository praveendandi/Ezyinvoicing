        ...
        for i in raw_data:
            ...
            if redg_card == "Yes":
                if "Reservation No." in i:
                    visits_index = i.index("REGISTRATION CARD")
                    confirmation_number = i[:visits_index]
                    conf_number = confirmation_number.replace("REGISTRATION CARD Conf. No.:","").strip()
            ...
            if "Email ID" in i:
                Email_id = i.split(":")
                email_id = Email_id[1].replace(" ", "")
                email_id = Email_id[1].replace("Arrival", "")
            ...
            if gspApiDataResponse['success'] == True:
                if guest['invoice_type'] == 'B2B':
                    ...
                    if checkTokenIsValidResponse['success'] == True:
                        # gspApiDataResponse['data']['get_taxpayer_details']=email_id
...
def create_doc(file_path, docType, conf_number=""):
    print(file_path)
    company = frappe.get_doc("company","RDM-01")
