...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("RGUV-01")
        ...
        for i in raw_data:
            ...
            if redg_card == "Yes":
                if "Reservation No." in i:
                    visits_index = i.index("Visits")
                    confirmation_number = i[:visits_index]
                    conf_number = confirmation_number.replace("Reservation No.","").strip()
            ...
            if "Confirmation No. :" in i or "Confirmation No " in i:
                confirmation_number = i.split(":")
                conf_number = confirmation_number[-1].replace(" ", "")
            if "Total" in i and "INR" in i and "~Total" not in i:
            ...
            if ("GST  ID" in i or "GST ID  :" in i and "P. GST ID" not in i and "GST ID-" not in i) or ("GST ID" in i and "P. GST ID" not in i and "GST ID-" not in i):
                gstNumber = i.split(':')[1].replace(' ', '')
                gstNumber = gstNumber.replace("ConfirmationNo.", "")
            if "Bill  No. :" in i:
                invoiceNumber = (i.split(':')[-1]).replace(" ", "").lstrip("-")
            if "Bill To" in i:
...
def create_doc(file_path, docType, conf_number=""):
    print(file_path)
    company = frappe.get_doc("company","RGUV-01")
