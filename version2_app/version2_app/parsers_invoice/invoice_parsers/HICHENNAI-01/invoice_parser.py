...
def file_parsing(filepath):
    ...
    try:
        ...
        companyCheckResponse = check_company_exist("HICHENNAI-01")
        ...
        for i in raw_data:
            ...
            if "COPY OF INVOICE" in i or "INFORMATION INVOICE" in i:
                index_val = raw_data.index(i)-1
                date_time_obj=(raw_data[index_val]).replace("Original Bill  :","").replace("Bill  No. :","")
                date_time_obj = datetime.datetime.strptime(date_time_obj.strip(),'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if date_time_obj == "":
                if "TAX INVOICE" in i or "INFORMATION INVOICE" in i:
                    index_val = raw_data.index(i)+1
                    date_time_obj=(raw_data[index_val]).replace("Original Bill  :","").replace("GST ID  : 33AABCI2726B1Z9","")
                    date_time_obj = datetime.datetime.strptime(date_time_obj.strip(),'%d-%m-%y').strftime('%d-%b-%y %H:%M:%S')
            if "Room No. " in i:
                room = i.split(":")
                roomNumber = room[-1]
        ...
        guest['company_code'] = "HICHENNAI-01"
        ...
        company_code = {"code":"HICHENNAI-01"}
        error_data = {"invoice_type":'B2B' if gstNumber != '' else 'B2C',"invoice_number":invoiceNumber.replace(" ",""),"company_code":"HICHENNAI-01","invoice_date":date_time_obj}
