...
def Updateemitsocket(doc, method=None):
    if doc.status == "Success":
        ...
        frappe.publish_realtime(
            "custom_socket", {'message': 'bench  update started', 'type': "bench update"})
...
def DocumentBinSocket(doc, method=None):
    ...
    frappe.publish_realtime("custom_socket", {
                            'message': 'Document Bin Insert', 'type': "document_bin_insert", "data": doc.__dict__})
...
def updateManager(doc, method=None):
    try:
        if doc.status != "Ongoing":
            ...
            frappe.publish_realtime("custom_socket", {
                                    'message': 'bench update completed', 'type': "bench completed"})
