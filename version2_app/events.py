...
def invoice_created(doc, method=None):
    try:
        ...
        if not frappe.db.exists('Documents', doc.confirmation_number):
            data = {"doctype": "Documents", "tax_invoice_file": doc.invoice_file,
                    "confirmation_number": doc.confirmation_number, "module_name": "Sign Ezy", "user": doc.owner}
            get_doc = frappe.get_doc(data)
            get_doc.insert()
            frappe.db.commit()
        get_doc = frappe.db.set_value("Documents", doc.confirmation_number, {
            ...
            if doc.invoice_file.count("@-") == 2:
                ...
                if doc.confirmation_number != "":
                    if frappe.db.exists("Arrival Information", doc.confirmation_number):
                        get_values = frappe.db.get_value("Arrival Information", doc.confirmation_number, [
                                                         "guest_email_address", "guest_phone_no"], as_dict=1)
                ...
                if frappe.db.exists({"doctype": "Tablet Config", "work_station": workstation, "mode": "Active"}):
                    ...
                    tabletconfig = frappe.db.get_value("Tablet Config", {"work_station": workstation, "mode": "Active"}, [
                                                       "work_station", "tablet", "username", "work_station_socket_id", "tablet_socket_id", "mode", "device_name"], as_dict=1)
                    data = {'tablet_config': tabletconfig, 'doc_data': doc.__dict__,
                            'uuid': tabletconfig.tablet, "guest_details": get_values}
        ...
        frappe.log_error("Ezy-invoicing invoice created Event",
...
def company_created(doc, method=None):
    try:
        if frappe.db.exists('company', doc.name):
            pass
            doc = frappe.db.get_list('company', filters={"docstatus": 0}, fields=[
                                     "name", "company_name", "company_code", "phone_number", "gst_number", "provider", "licensing_host"])
            api = doc[0]["licensing_host"]+"/api/resource/Properties"
            adequare_doc = frappe.get_doc("GSP APIS", doc[0]["provider"])
            insert_dict = {"doctype": "Properties", "property_name": doc[0]["company_name"], "property_code": doc[0]["company_code"], "contact_number": doc[0]["phone_number"], "gst_number": doc[0]["gst_number"], "gsp_provider": doc[
                0]["provider"], "api_key": adequare_doc.gsp_prod_app_secret, "api_secret": adequare_doc.gsp_prod_app_id, "gsp_test_app_id": adequare_doc.gsp_test_app_id, "gsp_test_app_secret": adequare_doc.gsp_test_app_secret}
            headers = {'content-type': 'application/json'}
            r = requests.post(api, headers=headers,
                              json=insert_dict, verify=False)
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing company_created Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def invoice_deleted(doc, method=None):
    try:
        frappe.publish_realtime("custom_socket", {
                                'message': 'Invoice deleted', 'type': "Delete invoice", "invoice_number": doc.name, "company": doc.company})
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing invoice_deleted Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def precheckinsdocuments(doc, method=None):
    try:
        ...
        if not frappe.db.exists('Documents', confirmation_number):
            ...
            # frappe.db.commit()
            user_name = frappe.session.user
            data = {"doctype": "Documents", "guest_details": [{"image1": doc.image_1, "image2": doc.image_2}],
                    "confirmation_number": confirmation_number, "module_name": "Ezycheckins", "user": user_name}
            get_doc = frappe.get_doc(data)
            get_doc.insert()
            frappe.db.commit()
        else:
            update_doc = frappe.get_doc("Documents", confirmation_number)
            update_doc.append("guest_details", {
                              "image1": doc.image_1, "image2": doc.image_2})
            update_doc.save()
            frappe.db.commit()
    except Exception as e:
...
def invoiceCreated(doc):
    try:
        ...
        frappe.publish_realtime("custom_socket", {
                                'message': 'Invoices Created', 'data': doc.__dict__, "company": doc.company})
        soc_doc = frappe.new_doc("Socket Notification")
        soc_doc.invoice_number = doc.name
        soc_doc.guest_name = doc.guest_name
        soc_doc.document_type = doc.invoice_category
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing invoiceCreated Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        print(traceback.print_exc())
        return {"success": False, "message": str(e)}
...
def update_documentbin(filepath, error_log):
    try:
        bin_data = frappe.db.get_list('Document Bin', filters={
        ...
        print(traceback.print_exc())
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing update_documentbin Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def insert_folios(company, file_path):
    try:
        ...
        get_values = frappe.db.get_value("company", company.name, ["reg_card_identification_text", "paid_out_receipts",
                                         "advance_deposits_identification", "payment_receipts_identification", "encashment_certificates_identification"], as_dict=1)
...
def Updateemitsocket(doc, method=None):
    try:
        if doc.status == "Success":
            company = frappe.get_last_doc('company')
            frappe.log_error("trigger socket bench update",
                             " {'message':'bench  update started','type':'bench update'}")
            frappe.publish_realtime("custom_socket", {
                                    'message': 'bench  update started', 'type': "bench update", "company": company.name})
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing Updateemitsocket Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def DocumentBinSocket(doc, method=None):
    try:
        ...
        frappe.publish_realtime("custom_socket", {
                                'message': 'Document Bin Insert', 'type': "document_bin_insert", "data": doc.__dict__, "company": company.name})
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing DocumentBinSocket Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def updateManager(doc, method=None):
    try:
        if doc.status != "Ongoing":
            commands = ['git pull origin updates',
                        'systemctl nginx reload', 'systemctl nginx restart']
            console_dump = ''
            company = frappe.get_last_doc('company')
            print(company)
            ...
            frappe.publish_realtime("custom_socket", {
                                    'message': 'bench update completed', 'type': "bench completed", "company": company.name})
            # frappe.log_error("Angular project pull", console_dump)
            frappe.log_error("Angular project pull data", "update manager")
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing updateManager Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
...
def information_folio_created(doc, method=None):
    try:
        ...
        if not frappe.db.exists('Documents', doc.confirmation_number):
            data = {"doctype": "Documents", "invoice_file": doc.invoice_file,
                    "confirmation_number": doc.confirmation_number, "module_name": "Sign Ezy", "user": user_name}
            get_doc = frappe.get_doc(data)
            get_doc.insert()
            frappe.db.commit()
        get_doc = frappe.db.set_value("Documents", doc.confirmation_number, {
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-information folio created Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def update_workstations_status():
    try:
        ...
        update_workstation = frappe.db.set_value(
            'Active Work Stations', workstation, data)
        frappe.db.commit()
        table_config = frappe.db.get_value(
...
def create_redg_card(doc, method=None):
    try:
        ...
        if not frappe.db.exists('Documents', doc.confirmation_number):
            user_name = frappe.session.user
            data = {"doctype": "Documents", "redg_file": doc.redg_file,
                    "confirmation_number": doc.confirmation_number, "module_name": "Sign Ezy", "user": doc.owner}
            get_doc = frappe.get_doc(data)
            get_doc.insert()
            frappe.db.commit()
...
def create_advance_deposits(doc, method=None):
    try:
        ...
        if len(get_data) == 0:
            user_name = frappe.session.user
            data = {"doctype": "Documents", "advance_deposits_file": doc.file_path,
                    "confirmation_number": doc.conformation_no, "module_name": "Sign Ezy", "user": user_name}
            get_doc = frappe.get_doc(data)
            get_doc.insert()
            frappe.db.commit()
        user_name = frappe.session.user
        data = {"advance_deposits_file": doc.file_path, "confirmation_number":
                doc.conformation_no, "module_name": "Sign Ezy", "user": user_name}
...
def deleteemailfilesdaily():
    try:
        ...
        emaildata = frappe.db.get_list('Email Queue', filters={'creation': [
                                       '>', lastdate], 'status': "Sent"}, fields=['name', 'attachments'])
        filelist = []
        if len(emaildata) > 0:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing deletemailfilesdaily Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def gspmeteringhook(doc, method=None):
    try:
        company = frappe.get_last_doc('company')
        inputData = {"data": {"doctype": "Gsp Metering", "property_code": company.name, 'tax_payer_details': doc.tax_payer_details, 'login': doc.login, 'generate_irn': doc.generate_irn,
                              'get_irn_details_by_doc': doc.get_irn_details_by_doc, 'cancel_irn': doc.cancel_irn, 'invoice_by_irn': doc.invoice_by_irn, 'create_qr_image': doc.create_qr_image, 'status': doc.status}}
        print(inputData)
        headers = {'Content-Type': 'application/json'}
        if company.mode == "Production":
            if company.proxy == 1:
                ...
                json_response = requests.post(company.licensing_host+"/api/method/ezylicensing.ezylicensing.getcount.gspmetering_post",
                                              headers=headers, json=inputData, proxies=proxies, verify=False)
            else:
                if company.skip_ssl_verify == 1:
                    json_response = requests.post(
                        company.licensing_host+"/api/method/ezylicensing.ezylicensing.getcount.gspmetering_post", headers=headers, json=inputData, verify=False)
                else:
                    json_response = requests.post(
                        company.licensing_host+"/api/method/ezylicensing.ezylicensing.getcount.gspmetering_post", headers=headers, json=inputData, verify=False)
            print(json_response, "/////////")
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing gspmeteringhook Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        print(str(e))
...
def taxpayerhook(doc, method=None):
    try:
        ...
        inputData = {'doctype': 'TaxPayerDetail', 'gst_number': doc.gst_number, 'legal_name': doc.legal_name, 'email': doc.email, 'address_1': doc.address_1, 'address_2': doc.address_2, 'location': doc.location, 'pincode': doc.pincode, 'gst_status': doc.gst_status,
                     'tax_type': doc.tax_type, 'trade_name': doc.trade_name, 'phone_number': doc.phone_number, 'state_code': doc.state_code, 'address_floor_number': doc.address_floor_number, 'address_street': doc.address_street, 'status': doc.status, 'block_status': doc.block_status}
        if company.mode == "Production":
            if company.proxy == 1:
                proxyhost = company.proxy_url
                proxyhost = proxyhost.replace("http://", "@")
                ...
                insertTaxpayer = requests.post(company.licensing_host+"/api/resource/TaxPayerDetail",
                                               headers=headers, json=inputData, proxies=proxies, verify=False)
            else:
                if company.skip_ssl_verify == 1:
                    insertTaxpayer = requests.post(
                        company.licensing_host+"/api/resource/TaxPayerDetail", headers=headers, json=inputData, verify=False)
                else:
                    insertTaxpayer = requests.post(
                        company.licensing_host+"/api/resource/TaxPayerDetail", headers=headers, json=inputData, verify=False)
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing taxpayerhook Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        print(str(e))
...
def InvoiceDataTolicensing():
    try:
        ...
        if company.mode == "Production":
            ...
            Invoice_count = frappe.db.get_list('Invoices', filters={'creation': ["Between", [today, today]], 'mode': 'Testing'}, fields=[
                                               'count(name) as count', 'invoice_category', 'mode'], group_by='invoice_category')
            Invoice_count2 = frappe.db.get_list('Invoices', filters={'creation': ["Between", [today, today]], 'mode': 'Production'}, fields=[
                                                'count(name) as count', 'invoice_category', 'mode'], group_by='invoice_category')
            ...
            if len(Invoice_count) > 0:
                inputData = {"data": {"date": str(
                    today), "property_code": company.name, "mode": "Testing"}}
                ...
                json_response = requests.post(
                    company.licensing_host+"/api/method/ezylicensing.ezylicensing.getcount.invoice_post", headers=headers, json=inputData, verify=False)

            if len(Invoice_count2) > 0:
                inputData = {"data": {"date": str(
                    today), "property_code": company.name, "mode": "Production"}}
        ...
        print(traceback.print_exc())
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing InvoiceDataToLicensing Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
...
def dailyDeletedocumentBin():
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing dailyDeletedocumentBin Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def dailyIppprinterFiles():
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing dailyIppprinterFiles Event",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        print(str(e))
...
def updatepropertiesdetails():
    try:
        ...
        company = frappe.get_last_doc("company")
        inputData = {"data": {"doctype": "Properties", "company_code": company.name, "api_key": gspdetails.gsp_prod_app_id, 'api_secret': gspdetails.gsp_prod_app_secret, 'gst_prod_username': gspdetails.gst__prod_username, 'gst_prod_password': gspdetails.gst_prod_password,
                              "gsp_test_app_id": gspdetails.gsp_test_app_id, "gsp_test_app_secret": gspdetails.gsp_test_app_secret, "gst_test_username": gspdetails.gst__test_username, "gst_test_password": gspdetails.gst_test_password, "groups": company.property_group, "mode": company.mode}}
        headers = {'Content-Type': 'application/json'}
        if company.mode == "Production":
            if company.proxy == 1:
                proxyhost = company.proxy_url
                ...
                json_response = requests.post(company.licensing_host+"/api/method/ezylicensing.ezylicensing.doctype.properties.properties.update_property_status",
                                              headers=headers, json=inputData, proxies=proxies, verify=False)
            else:
                if company.skip_ssl_verify == 1:
                    json_response = requests.post(
                        company.licensing_host+"/api/method/ezylicensing.ezylicensing.doctype.properties.properties.update_property_status", headers=headers, json=inputData, verify=False)
                else:
                    json_response = requests.post(
                        company.licensing_host+"/api/method/ezylicensing.ezylicensing.doctype.properties.properties.update_property_status", headers=headers, json=inputData, verify=False)
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-updatepropertiesdetails",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def block_irn():
    try:
        ...
        if url_property.status_code == 200:
            ...
            company.pos_bills_module = "Enable" if json_property[
                "data"]["POS Bills Module"] == 1 else "Disable"
            company.save(ignore_permissions=True)
            frappe.db.commit()
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error(
            "Ezy-block-IRN", "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def backup_file_perticulerdoctypes(data):
    try:
        ...
        run_command = os.system(
            'bench --site {} backup --only "company,SAC HSN CODES,Payment Types,GSP APIS"'.format(site_name))
        cwd = cwd = os.getcwd()
        # site_name = cstr(frappe.local.site)
...
def arrival_information(doc, method=None):
    ...
    if len(get_data) == 0:
        ...
        frappe.db.set_value("Documents", get_data[0]["name"], {
...
def send_invoice_mail_scheduler():
    try:
        get_arrivals = frappe.db.get_list("Arrival Information", filters={'guest_eamil2': [
                                          '!=', ""], 'send_invoice_mail': ['=', 1], 'invoice_send_mail_send': ['=', 0]})
        if len(get_arrivals) > 0:
            for each in get_arrivals:
                ...
                if frappe.db.exists({"doctype": "Invoices", "confirmation_number": each["name"]}):
                    get_doc = frappe.db.get_value("Invoices", {"confirmation_number": each["name"]}, [
                                                  "invoice_file", "name"], as_dict=True)
                    b2csuccess = frappe.get_doc('Email Template', "Scan Ezy")
                    files = frappe.db.get_list(
                        'File', filters={'file_url': ['=', get_doc["invoice_file"]]}, fields=['name'])
                    attachments = [files[0]["name"]]
                    get_email_sender = frappe.db.get_list(
                        "Email Account", filters=[["default_outgoing", "=", 1]], fields=["email_id"])
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-send_invoice_mail",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def guest_attachments(doc, method=None):
    try:
        ...
        if len(get_data) == 0:
            user_name = frappe.session.user
            data = {"doctype": "Documents", "guest_details": [{"image1": doc.id_image1, "image2": doc.id_image2, "image3": doc.id_image3,
                                                               "face_image": doc.face_image}], "confirmation_number": doc.confirmation_number, "module_name": "Scan Ezy", "user": user_name}
            get_doc = frappe.get_doc(data)
            get_doc.insert()
            frappe.db.commit()
        else:
            user_name = frappe.session.user
            data = {"confirmation_number": doc.confirmation_number,
                    "module_name": "Scan Ezy", "user": user_name}
            get_doc = frappe.db.set_value(
                "Documents", get_data[0]["name"], data)
            frappe.db.commit()
            update_doc = frappe.get_doc("Documents", get_data[0]["name"])
            update_doc.append("guest_details", {
                              "image1": doc.id_image1, "image2": doc.id_image2, "image3": doc.id_image3, "face_image": doc.face_image})
            update_doc.save()
            frappe.db.commit()
        if frappe.db.exists('Arrival Information', doc.confirmation_number):
            ...
            doc.checkout_date = datetime.datetime.strptime(str(
                arrival_doc.departure_date), '%Y-%m-%d') if arrival_doc.departure_date else None
            doc.checkin_date = datetime.datetime.strptime(
                str(arrival_doc.arrival_date), '%Y-%m-%d') if arrival_doc.arrival_date else None
            if doc.main_guest == 1:
                doc.no_of_adults = 1
            else:
        ...
        if doc.id_type == "Foreigner":
            if frappe.db.exists({'doctype': 'Precheckins', 'confirmation_number': data["confirmation_number"]}):
                pre_checkins = frappe.db.get_value('Precheckins', {'confirmation_number': data["confirmation_number"]}, [
                                                   "address1", "guest_city", "guest_country"], as_dict=1)
...
def guest_update_attachment_logs(doc, method=None):
    try:
        given_name = doc.given_name if doc.given_name else ""
        surname = doc.surname if doc.surname else ""
        frappe.db.set_value('Guest Details', doc.name, {
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-invoicing guest update attachment logs",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def send_email(confirmation_number, company):
    ...
    url = "https://so.ezycheckins.com/v2/?hotelId={}&confirmation={}&source=email".format(
    ...
    activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number": arrival_doc.confirmation_number,
                     "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Email Sent successfully"}
    event_doc = frappe.get_doc(activity_data)
    event_doc.insert()
    frappe.db.commit()
    ...
    try:
        ...
        if company.mail_schedule == "True":
                ...
                if company.mail_frequency == "Once":
                    for x in get_arrival_data:
                        ...
                        if x['mail_sent'] == "No":
                            ...
                            activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number": conf_number,
                                             "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Email Sent successfully by System"}
                            event_doc = frappe.get_doc(activity_data)
                            event_doc.insert()
                            frappe.db.commit()
            ...
            if company.cancellation_email == 1:
                get_arrival_data = frappe.db.get_list("Arrival Information", filters={"booking_status": [
                                                      '=', "CANCELLED"]}, fields=["arrival_date", "name", "guest_email_address", "mail_sent", "mail_via"])
                for x in get_arrival_data:
                    dt_convert = str(x['arrival_date'])
                    name = str(x['name'])
                    cancel_email_address = str(x["guest_email_address"])
                    arrival_date = datetime.datetime.strptime(
                        dt_convert, '%Y-%m-%d').date()
                    folder_path = frappe.utils.get_bench_path()
                    site_folder_path = company.site_name
                    file_path = folder_path+'/sites/'+site_folder_path + \
                    ...
                    if x['mail_sent'] == "No":
                        ...
                        frappe.db.set_value(
                            'Arrival Information', x['name'], 'mail_via', 'Automatic')
                        activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number":
                                         x["confirmation_number"], "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Cancellation Mail Sent successfully"}
                        event_doc = frappe.get_doc(activity_data)
                        event_doc.insert()
                        frappe.db.commit()
                    else:
...
def manual_mail(data):
    ...
    if arrival_doc.booking_status == "RESERVED" or arrival_doc.booking_status == "DUE IN":
        ...
        data = f.read()
        data = data.replace(
            '{{name}}', arrival_doc.guest_first_name if arrival_doc.guest_first_name else "")
        data = data.replace(
            '{{lastName}}', arrival_doc.guest_last_name if arrival_doc.guest_last_name else "")
        ...
        mail_send = frappe.sendmail(recipients=email_address,
                                    subject=company.pre_checkin_mail_subject,
                                    message=data, now=True)
        frappe.db.set_value('Arrival Information', conf_number,
        ...
        activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number": conf_number,
                         "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Mail Sentout"}
        event_doc = frappe.get_doc(activity_data)
        event_doc.insert()
        frappe.db.commit()
        return {"success": True}
        ...
        frappe.db.set_value('Arrival Information',
                            conf_number, 'mail_count', count)
        activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number": conf_number,
                         "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Mail Sent successfully"}
        event_doc = frappe.get_doc(activity_data)
        event_doc.insert()
        frappe.db.commit()
        return {"success": True}
...
def check_hotelId(data):
    ...
    company_doc = frappe.db.get_value("company", company, ["name", "company_name", "trade_name", "gst_number",
                                      "b2c_qr_url", "company_logo", "terms_and_conditions", "ezycheckin_signature", "welcome_message"], as_dict=True)
    arrival_doc = frappe.db.get_value('Arrival Information', confirmation_number, ["guest_title", "guest_last_name", "guest_first_name", "room_type", "guest_email_address", "guest_phone_no", "travel_agent_name", "no_of_adults", "no_of_children", "no_of_nights", "billing_instructions", "no_of_rooms", "confirmation_number", "reservation_clerk", "room_rate",
                                      "print_rate", "csr_id", "membership_no", "membership_type", "checkin_date", "total_visits", "checkout_date", "cc_number", "booking_status", "company_name", "cc_exp_date", "company", "virtual_checkin_status", "arrival_date", "is_group_code", "departure_date", "number_of_guests", "mail_sent", "mail_via", "confirmation_number"], as_dict=True)
...
def check_hotelCode(data):
    company = data["company"]
    company_doc = frappe.db.get_value("company", company, ["name", "company_name", "trade_name", "gst_number",
                                      "b2c_qr_url", "company_logo", "terms_and_conditions", "welcome_message", "ezycheckin_signature"], as_dict=True)
    if company == company_doc["name"]:
...
def precheckins():
    ...
    for i in data["data"]:
        ...
        try:
            ...
            im1.save(cwd+"/"+site_name+"/private/files/image1" +
                     i["confirmation_number"]+".png", 'PNG')
            im2 = Image.open(BytesIO(image2_binary))
            im2.save(cwd+"/"+site_name+"/private/files/image2" +
                     i["confirmation_number"]+".png", 'PNG')
            im = Image.open(BytesIO(signature_binary))
            im.save(cwd+"/"+site_name+"/private/files/signature" +
                    i["confirmation_number"]+".png", 'PNG')
        ...
        frappe.db.set_value(
            'Arrival Information', i["confirmation_number"], 'virtual_checkin_status', 'Yes')
        activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number":
                         i["confirmation_number"], "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Precheckin Done"}
        event_doc = frappe.get_doc(activity_data)
        event_doc.insert()
        ...
        if company.thank_you_email == "1":
            ...
            if i['mail_sent'] == "No":
                ...
                mail_send = frappe.sendmail(recipients=cancel_email_address,
                                            ...
                                            message=data, now=True)
                frappe.db.set_value('Arrival Information',
                                    i['name'], 'mail_sent', 'Yes')
                frappe.db.set_value('Arrival Information',
                                    i['name'], 'mail_via', 'Automatic')
                activity_data = {"doctype": "Activity Logs", "datetime": date_time, "confirmation_number":
                                 i["confirmation_number"], "module": "Ezycheckins", "event": "PreArrivals", "user": user_name, "activity": "Thankyou Mail Sent-out"}
                event_doc = frappe.get_doc(activity_data)
                event_doc.insert()
                frappe.db.commit()
            else:
...
def upload_propery_logo_pms(data):
    try:
        ...
        if company.proxy == 1:
            proxyhost = company.proxy_url
            proxyhost = proxyhost.replace("http://", "@")
            proxies = {'https': 'https://'+company.proxy_username +
...
def update_company(doc, method=None):
    try:
        ...
        if doc.pms_property_logo:
            ...
            if status["success"] == False:
                return status
            frappe.db.set_value('company', doc.name, {
                                "pms_property_url": status["data"]})
            frappe.db.commit()
    except Exception as e:
...
def delete_arrival_activity():
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-Delete Arrival Activity",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def update_mail_send_confirmation(confirmation_number):
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-update_mail_send_confirmation",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def delete_error_logs():
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-delete_error_logs",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def delete_email_queue():
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("Ezy-delete_email_queue",
                         "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()))
        return {"success": False, "message": str(e)}
...
def ezy_suite_dashboard(from_date, to_date):
    try:
        ...
        reservations_pending = frappe.db.sql(
            """select count(name) as pending_reservations from `tabArrival Information` where DATE(arrival_date) between '{}' and '{}' and status='Pending'""".format(from_date, to_date), as_dict=1)
        ...
        return {"success": True, "data": {"total_reservations": total_reservations[0]["total_reservations"], "pre_checkins_count": pre_checkins_count[0]["pre_checkins"], "reservations_scanned": reservations_scanned[0]["scanned_reservations"],
                                          ...
                                          "total_foreign_ids_pathik": pathik_pending[0]["pending_pathik"]+uploaded_pathik[0]["uploaded_pathik"]}}
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
