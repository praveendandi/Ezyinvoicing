    ...
    try:
        ...
        if len(raw_data) > 5:
            ...
            if company_doc.enable_pos_extra_text == 1:
                text = add_extra_text_while_print(
                    data["check_no"], data["outlet"], company_doc
                )
            ...
            if outlet_doc.print == "Yes" and data["check_type"] == "Normal Check":
                port = int(
                    printer_doc.port) if printer_doc.port != "" else 9100
                            ...
                            if outlet_doc.print_count > 1:
                                if count == 0:
                                    ...
                                    added_text1 = "Merchant Copy\n".encode(
                                        "utf-8")
                                added_text = added_text1 + added_text.replace(
                                    "Guest Copy\n".encode(
                                        "utf-8"), "".encode("utf-8")
                                )
                            give_print(
                                data["payload"],
                                printer_doc.printer_ip,
                                logopath,
                                qrpath,
                                port,
                                company_doc,
        ...
        frappe.log_error(
            "Ezy-invoicing create pos bills",
...
def extract_data(payload, company_doc):
    try:
        ...
        for line in raw_data:
            if (
                company_doc.closed_check_reference in payload
                and company_doc.void_check_reference not in payload
            ):
                data["check_type"] = "Check Closed"
                if company_doc.void_total_amt_regex:
                    if re.match(company_doc.void_total_amt_regex, line.strip()):
                        total_amount_regex = re.findall(
                            company_doc.bill_amount_regex, line.replace(
                                " ", "")
                        )
                        total_amount = (
                            total_amount_regex[0] if len(
                                total_amount_regex) > 0 else ""
                        ).replace(",", "")
                ...
                if re.match(pattern, line):
                    if "GST" not in line:
                        ...
                        if len(final_list) > 3:
                            data["check_no"] = final_list[1]
                            data["table_number"] = final_list[0]
                            data["no_of_guests"] = final_list[-1].replace(
                                "GST", "")
            else:
                if (
                    company_doc.check_number_reference in line
                    and "GSTIN" not in line
                    and "GST IN" not in line
                ):
                    check_regex = re.findall(
                        company_doc.check_number_regex, line.strip()
                    )
                    check_string = check_regex[0] if len(
                        check_regex) > 0 else ""
                    check_no_regex = re.findall(r"\d+", check_string)
                    data["check_no"] = (
                        check_no_regex[0] if len(check_no_regex) > 0 else ""
                    )
                if (
                    company_doc.table_number_reference in line
                    and "GSTIN" not in line
                    and "GST IN" not in line
                ):
                    table_regex = re.findall(
                        company_doc.table_number_regex, line.strip()
                    )
                    table_string = table_regex[0] if len(
                        table_regex) > 0 else ""
                    table_no_regex = table_string.split(" ")
                    data["table_number"] = (
                        table_no_regex[-1] if len(table_no_regex) > 1 else ""
                    )
                    if data["table_number"] == "" and len(table_no_regex) == 1:
                        data["table_number"] = (
                            table_no_regex[0] if "/" in table_no_regex[0] else ""
                        )
                if (
                    company_doc.guest_number_reference in line
                    and "%" not in line
                    and "GST No." not in line
                ):
                    guestno_regex = re.findall(
                        company_doc.guest_number_regex, line.strip()
                    )
                    print(guestno_regex)
                    guest_string = guestno_regex[0] if len(
                        guestno_regex) > 0 else ""
                    guest_no_regex = re.findall(r"\d+", guest_string)
                    data["no_of_guests"] = (
                        guest_no_regex[0] if len(guest_no_regex) > 0 else ""
                    )
...
def razorPay(total_bill_amount, check_no, outlet, company_doc):
    try:
        ...
        data = {
            "customer": {"name": outlet + " " + "POS Bill", "email": "", "contact": ""},
            "type": "link",
        ...
        # notes = {
        #     "Shipping address": company_doc.address_1
        #     + ", "
        #     + company_doc.location
        #     + ", "
        #     + str(company_doc.pincode)
        # }
...
def add_extra_text_while_print(check_no, outlet, company_doc):
    try:
        ...
        if format:
            ...
            if countofy != 0:
                yearformat = x.strftime(
                    "%y") if countofy == 2 else x.strftime("%Y")
            countofm = format.count("M")
            if countofm != 0:
                monformat = x.strftime(
                    "%m") if countofy == 2 else x.strftime("%m")
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error(
            "Ezy-invoicing add_extra_text_while_print",
            "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()),
        )
        return {"success": False, "message": str(e)}
...
def print_pos_bill(data):
    try:
        ...
        if company_doc.enable_pos_extra_text == 1:
            text = add_extra_text_while_print(
                check_doc.check_no, check_doc.outlet, company_doc
            )
            if text["success"] is False:
                return text
            added_text = (text["string"]).encode("utf-8")
            invoice_number = (text["invoice_number"]).encode("utf-8")
        outlet_values = frappe.db.get_values(
            "Outlets",
            {"outlet_name": check_doc.outlet},
            ["static_payment_qr_code", "outlet_logo", "payment_mode", "name"],
            as_dict=1,
        )
        printer_settings = frappe.db.get_value(
            "POS Print Settings", {
                "outlet": outlet_values[0]["name"]}, ["printer"]
        )
        ...
        kitchen = Network(
            printer_doc.printer_ip,
            int(printer_doc.port) if printer_doc.port != "" else 9100,
        )  # Printer IP Address
        ...
        if check_doc.check_type == "Normal Check":
            if outlet_values[0]["payment_mode"] == "Dynamic":
                razor_pay = razorPay(
                    check_doc.total_amount,
                    check_doc.check_no,
                    check_doc.outlet,
                    company_doc,
                )
                if razor_pay["success"] is False:
...
def send_pos_bills_gcb(company, b2c_data):
    try:
        ...
        exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error(
            "Ezy-invoicing send pos bills gcb",
            "line No:{}\n{}".format(exc_tb.tb_lineno, traceback.format_exc()),
        )
        return {"success": False, "message": str(e)}
