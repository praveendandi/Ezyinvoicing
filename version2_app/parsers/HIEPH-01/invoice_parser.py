...
def file_parsing(filepath):
	...
	try:
		...
		for i in raw_data:
			...
			if entered == True:
				data.append(i)
			if "GUEST NAME:" in i or "GUESTNAME" in i:
				guestDeatils.append(i)
			if "Membership" in i:
				Membership = i.split(":")
		...
		for i in data:
			...
			if len(check_date) > 0 and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "IGST" not in i and "Service Charge" not in i and ("CGST/SGST" in i or ("CGST" not in i and "SGST" not in i)):
				item = dict()
				item_value = ""
				dt = i.strip()
				print(dt)
