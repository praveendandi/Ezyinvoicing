...
def file_parsing(filepath):
	try:
		...
		for each in items:
			if "CGST" not in each["name"] and "SGST" not in each["name"] and "CESS" not in each["name"] and "VAT" not in each["name"] and "Cess" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Service Charge" not in each['name'] and "Service charge" not in each['name'] and "Room No." not in each['name']:
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				...
				if "Membership No." in guest['name']:
					guest['name'] = guest['name'].replace("Membership No.","")
			if index == 1:
				guest['address1'] = i
			if index == 2:
				guest['address2'] = i
