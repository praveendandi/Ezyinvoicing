...
def reinitiateInvoice(data):
	try:
		...
		for i in raw_data:
			if "CREDIT TAX INVOICE" in i or "CREDIT TAX INV" in i:
		...
		for ind, each in enumerate(items):
			if "CGST" not in each["name"] and "SGST" not in each["name"] and "VAT" not in each["name"] and "Vat" not in each["name"] and "IGST" not in each["name"] and "Service Charge" not in each['name'] and "Service charge" not in each['name']:
