...
def reinitiateInvoice(data):
	try:
		...
		for i in original_data:
			...
			if len(check_date) > 0 and "." in split_str and "CGSt" not in i and "SGSt" not in i and "IGST" not in i and "CGST" not in i and "SGST" not in i and "CESS" not in i and "VAT" not in i and "Cess" not in i and "Vat" not in i and "VAT" not in i and "Cr-" not in i:
				item = dict()
				item_value = ""
				dt = i.strip()
		...
		for index, i in enumerate(guestDeatils):
			if index == 0:
				guest['name'] = (i.split(':')[1]).replace("Guests","")
