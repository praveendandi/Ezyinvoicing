        ...
        for each in data:
            ...
            if len(remove_data) == 26 and remove_data[-1] == "RESERVED":
                if not frappe.db.exists({'doctype': 'Reservations', "reservation_number": remove_data[16]}):
                    reservation_data = {"doctype": "Reservations", "guest_first_name": remove_data[2], "guest_last_name": remove_data[1], "email": remove_data[3], "contact_phone_no": remove_data[5].replace(
                        "+", ""), "no_of_nights": remove_data[9], "confirmation_number": remove_data[12], "reservation_number": remove_data[16], "status": "Pending", "booking_status": "Pending", "no_of_adults": remove_data[7], "no_of_children": remove_data[8]}
                    reservation_data["checkin_date"] = datetime.strptime(
                        remove_data[17], "%d-%b-%y").strftime('%Y-%m-%d %H:%M:%S')
                    reservation_data["checkout_date"] = datetime.strptime(
                        remove_data[18], "%d-%b-%y").strftime('%Y-%m-%d %H:%M:%S')
                    doc = frappe.get_doc(reservation_data)
                    doc.insert(ignore_permissions=True, ignore_links=True)
                    frappe.db.commit()
                else:
                    if remove_data[-1] == "RESERVED":
                        name = frappe.db.get_value('Reservations', filters={
...
CASCADE = os.path.join(
    abs_path, "apps/version2_app/version2_app/passport_scanner/doctype/reservations/", "Har_cascade.xml")
...
def aadhar_detect_text(image_file, doc_type):
    try:
        ...
        if doc_type == 'front':
            ...
            address = block[abc:]
            regex = re.compile(
                '([^a-zA-Z0-9-,./ ]|Address|govin|ligovin|help|No|www|o  |uidai)')
            ...
            ind = [final_address.index(
                x) for x in final_address if re.search('([0-9]{6})', x)]
            ...
            [final_address.remove(x)
             for x in final_address if re.search('([0-9]{6})', x)]
            ...
            address1 = ""
            address2 = ""
            person_address = person_address.replace("W|O", "").replace("W/O", "").replace("w/o", "").replace("w|o", "").replace("D/O", "").replace("d/o", "").replace("d|o", "").replace(
                "D|O", "").replace("d/o", "").replace("s/o", "").replace("s/o", "").replace("S/O", "").replace("s/o", "").replace("S|O", "").replace("s/o", "").replace("s|o", "")
            ...
            if person_address != "":
                if re.search("\d{6}", person_address):
                    postal_code_data = re.match(
                        '^.*(?P<zipcode>\d{6}).*$', person_address).groupdict()['zipcode']
                    postal_code = postal_code_data if len(
                        postal_code_data) == 6 else ''
        ...
        # exc_type, exc_obj, exc_tb = sys.exc_info()
        frappe.log_error("SignEzy aadhar_detect_text", str(e))
        return ({"success": False, "error": str(e), "message": "Unable to scan your id"})
...
def imgdeskew(image):
    try:
        ...
        if (median_angle > -5) and (median_angle < 5):
            ...
            cv2.imwrite(basedir+company.site_name +
                        "/private/files/aadhar_cropped_skewed_image.jpeg", img_rotated)
            with open(basedir+company.site_name+"/private/files/aadhar_cropped_skewed_image.jpeg", 'rb') as f:
                base64_string = base64.b64encode(f.read()).decode()
            os.remove(basedir+company.site_name +
                      "/private/files/aadhar_cropped_skewed_image.jpeg")
            return {"success": True, "data": base64_string}
        else:
            cv2.imwrite(basedir+company.site_name +
                        "/private/files/aadhar_cropped_skewed_image.jpeg", image)
...
def license_detect_text(image_file):
    try:
        ...
        dates = re.findall(
            r'(\d{2}\/\d{2}\/\d{4}|\d{2}\-\d{2}\-\d{4}|[0-9]{1,2}\-[A-Za-z]{3}\-[0-9]{2,4}|[0-9]{1,2} [A-Z0-9]{1,3} [0-9]{4})', text)
        ...
        if len(address) > 0:
            for x in address:
                if re.search(r'([0-9]{6}|[0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4}|[0-9]{1,2}\-[0-9]{1,2}\-[0-9]{4}|Holder|Issuing|Sign|licenced|Signature)', x):
                    ind = address.index(x)
        ...
        if person_address != '':
            ...
            for x in final_address:
                if re.search(r'([0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4}|[0-9]{1,2}\-[0-9]{1,2}\-[0-9]{4}|Holder|Issuing|Signature|licenced)', x):
            ...
            if len(address) > 0:
                for x in address:
                    if re.search(r'([0-9]{6}|[0-9]{1,2}\/[0-9]{1,2}\/[0-9]{2,4}|[0-9]{1,2}\-[0-9]{1,2}\-[0-9]{4}|Holder|Issued|Sign|Signature|licenced)', x):
                        ind = address.index(x)
                        final_address = address[:ind+1]
                        person_address = ' '.join(x for x in final_address)
                        person_address = re.sub(
                            r'[^A-Za-z0-9-,/ ]|[0-9]{2}\/[0-9]{2}\/[0-9]{4}|[0-9]{2}\-[0-9]{2}\-[0-9]{4}', '', person_address)
        ...
        for x in block:
            if 'Name' in x or 'NAME' in x or 'Nam ' in x:
                ...
                noun = block[abc]
                name = re.sub(r'[^A-Za-z]|Name|NAME|Nam|ATH ', '', noun)

                if name == '':

                    name = re.sub(r'DOB|D.O.B|-|/', '', block[abc+1])
        ...
        if person_address != '':
            ...
            person_address = re.sub(
                '/Address|/Addess|/Ad|/ Address|/ Addess|/ Ad|Add|/Add|Addess|Ad', 'Address', person_address)
            person_address = re.sub('PIN', '', person_address)
            final_address = person_address.split()
            final_address = list(OrderedDict((element, None)
                                             for element in final_address))
            if len(pin_get) > 0:
                for x in final_address:
        ...
        if person_address != '':
            person_address = re.sub(
                r'Issued on|Issued|Date of First Issue|ssued|DoB|[0-9]{2}\/[0-9]{2}\/[0-9]{4}|Addressress|Address|ADDRESS|address', '', person_address)
            if re.search(r"\d{6}", person_address):
                postal_code_data = re.match(
                    r'^.*(?P<zipcode>\d{6}).*$', person_address).groupdict()['zipcode']
                postal_code = postal_code_data if len(
                    postal_code_data) == 6 else ''
            if "," in person_address:
                split_address = person_address.split(",")
...
def scan_driving_license():
    try:
        ...
        if details_data["success"] is False:
            return details_data
        details = details_data["data"]
        drivingcropped = image_processing(filename)
        if drivingcropped["success"] is False:
...
def pan_detect_text(image_file):
    try:
        company = frappe.get_last_doc('company')
        unlike = ['OF INDIA', 'INCOME TAX DEPARTMENT',
                  'Permanent Account Number', 's Name', 'INDIA', 'OF INDIA', 'Birth']
        ...
        if company.proxy == 1:
            ...
            proxies = {'https': 'https://'+company.proxy_username +
                       ":"+company.proxy_password+proxyhost}
            response = requests.post(
                url, headers=header, json=body, proxies=proxies, verify=False).json()
        else:
            response = requests.post(url, headers=header, json=body).json()
        text = response['responses'][0]['textAnnotations'][0]['description'] if len(
            response['responses'][0]) > 0 else ''
        ...
        if len(names) > 1:
            ...
            name = ""
            father_name = ""
        parsed_birth = str(dateparser.parse(
            DOB[0], settings={'DATE_ORDER': 'DMY'}).date())
        details = {"name": name, "father_name": father_name,
                   "pan_no": bca[0], "date_of_birth": parsed_birth, "pan_face": image_string}
        return {"success": True, "data": details}
    except Exception as e:
        # exc_type, exc_obj, exc_tb = sys.exc_info()
...
def scan_pancard():
    try:
        ...
        if pan_details["success"] is False:
            return pan_details
    ...
    try:
        ...
        if doc_type == 'front':
            ...
            if sex == '':
                for x in block:
                    if "sex:" in x and "/" in x:
                        ...
                        sex = x.split("/")[-1]
            noun = re.compile(
                r'([a-zA-Z]+ [a-zA-Z]+ [a-zA-Z]+|[a-zA-Z]+ [a-zA-Z]+|[a-zA-Z]+ [a-zA-Z]+ [a-zA-Z]+ [a-zA-Z]+)')
            ...
            if abc != "":
                ...
                final = block[abc:]
                regex = re.compile(
                    r'([^a-zA-Z0-9-/ ]|Address|Addres|Addre|ADDRESS)')
                ...
                address = re.sub(
                    r'[0-9]{1,2}\/[0-9]{1,2}\/[0-9]{4}|[0-9]{1,2}\-[0-9]{1,2}\-[0-9]{4}|[a-zA-Z]{3}[0-9]{7}|/Locality|Village|Pin|Code|Date|Scanned|Resten|lector|Registration|Officer|Facsimile', '', address)
...
def scan_votercard():
    try:
        ...
        if votercropped["success"] is False:
...
def passportvisadetails():
    try:
        ...
        if pass_details["success"] is False:
            return pass_details
    ...
    try:
        ...
        if person_address != "":
            if re.search(r"\d{6}", person_address):
                postal_code_data = re.match(
                    r'^.*(?P<zipcode>\d{6}).*$', person_address).groupdict()['zipcode']
                postal_code = str(postal_code_data) if len(
                    postal_code_data) == 6 else ''
            if "," in person_address:
                split_address = person_address.split(",")
                address1 = ','.join(split_address[:len(split_address)//2])
                ...
                address1 = ' '.join(split_address[:len(split_address)//2])
                address2 = ' '.join(split_address[len(split_address)//2:])
        return {"success": True, "data": {"personaddress": person_address, "address1": address1, "address2": address2, "postal_code": postal_code}}
    except Exception as e:
        # exc_type, exc_obj, exc_tb = sys.exc_info()
...
def passport_address():
    try:
        ...
        if add_details["success"] is False:
            return add_details
...
country_with_codes = {'MDG': 'Madagascar', 'AND': 'Andorra', 'LIE': 'Liechtenstein', 'SGS': 'South Georgia and the South Sandwich Island', 'GEO': 'Georgia', 'GAB': 'Gabon', 'GNQ': 'Equatorial Guinea', 'IND': 'India', 'NIU': 'Niue', 'IRN': 'Iran, Islamic Republic of', 'UZB': 'Uzbekistan', 'JAM': 'Jamaica', 'TGO': 'Togo', 'SYR': 'Syrian Arab Republic', 'STP': 'Sao Tome and Principe', 'BWA': 'Botswana', 'BHR': 'Bahrain', 'AZE': 'Azerbaijan', 'TCD': 'Chad', 'LVA': 'Latvia', 'MHL': 'Marshall Islands', 'GBD': 'Dependent territories citizen', 'MAR': 'Morocco', 'D': 'Germany', 'PLW': 'Palau', 'SHN': 'Saint Helena', 'SUR': 'Suriname', 'ANT': 'Netherlands Antilles', 'CYM': 'Cayman Islands', 'LUX': 'Luxembourg', 'NIC': 'Nicaragua', 'ESP': 'Spain', 'WLF': 'Wallis and Futuna Islands', 'COG': 'Congo', 'MUS': 'Mauritius', 'CXR': 'Christmas Island', 'RUS': 'Russian Federation', 'KAZ': 'Kazakhstan', 'SYC': 'Seychelles', 'TKM': 'Turkmenistan', 'CHN': 'China', 'IDN': 'Indonesia', 'CIV': "Côte d'Ivoire", 'FLK': 'Falkland Islands (Malvinas)', 'ARM': 'Armenia', 'GRC': 'Greece', 'GIN': 'Guinea', 'COK': 'Cook Islands', 'BRN': 'Brunei Darussalam', 'ATA': 'Antarctica', 'YEM': 'Yemen', 'GBN': 'National (overseas)', 'AUT': 'Austria', 'EGY': 'Egypt', 'MOZ': 'Mozambique', 'VUT': 'Vanuatu', 'CAF': 'Central African Republic', 'KIR': 'Kiribati', 'CAN': 'Canada', 'SOM': 'Somalia', 'GUF': 'French Guiana', 'NTZ': 'Neutral Zone', 'PHL': 'Philippines', 'HTI': 'Haiti', 'BRB': 'Barbados', 'GUY': 'Guyana', 'AIA': 'Anguilla', 'TKL': 'Tokelau', 'NOR': 'Norway', 'MKD': 'The former Yugoslav Republic of Macedonia', 'SAU': 'Saudi Arabia', 'DOM': 'Dominican Republic', 'GIB': 'Gibraltar', 'PNG': 'Papua New Guinea', 'COL': 'Colombia', 'PAK': 'Pakistan', 'DJI': 'Djibouti', 'SGP': 'Singapore', 'TZA': 'United Republic of Tanzania', 'FIN': 'Finland', 'GMB': 'Gambia', 'ECU': 'Ecuador', 'ABW': 'Aruba', 'ERI': 'Eritrea', 'SMR': 'San Marino', 'PAN': 'Panama', 'NRU': 'Nauru', 'MSR': 'Montserrat', 'PRK': "Democratic People's Republic of Korea", 'CHE': 'Switzerland', 'TON': 'Tonga', 'COM': 'Comoros', 'BTN': 'Bhutan', 'REU': 'Réunion', 'GBS': 'Subject', 'LCA': 'Saint Lucia', 'MEX': 'Mexico', 'ARG': 'Argentina', 'TUR': 'Turkey', 'HUN': 'Hungary', 'HND': 'Honduras', 'NFK': 'Norfolk Island', 'BRA': 'Brazil', 'LAO': "Lao People's Democratic Republic", 'NZL': 'New Zealand', 'PRY': 'Paraguay', 'QAT': 'Qatar', 'GRD': 'Grenada', 'AGO': 'Angola', 'BLR': 'Belarus', 'BVT': 'Bouvet Island', 'GUM': 'Guam', 'MMR': 'Myanmar', 'POL': 'Poland', 'FSM': 'Micronesia, Federated States of', 'LBR': 'Liberia', 'HRV': 'Croatia', 'PCN': 'Pitcairn', 'GRL': 'Greenland', 'MNP': 'Northern Mariana Islands', 'SWZ': 'Swaziland', 'THA': 'Thailand', 'ASM': 'American Samoa', 'SLV': 'El Salvador',
                      'VNM': 'Viet Nam', 'PRI': 'Puerto Rico', 'LKA': 'Sri Lanka', 'CCK': 'Cocos (Keeling) Islands', 'GLP': 'Guadeloupe', 'DMA': 'Dominica', 'SWE': 'Sweden', 'CZE': 'Czech Republic', 'DZA': 'Algeria', 'TTO': 'Trinidad and Tobago', 'BHS': 'Bahamas', 'SLE': 'Sierra Leone', 'VIR': 'Virgin Islands (United States)', 'SVN': 'Slovenia', 'MTQ': 'Martinique', 'DNK': 'Denmark', 'ATG': 'Antigua and Barbuda', 'AFG': 'Afghanistan', 'MYT': 'Mayotte', 'UKR': 'Ukraine', 'KGZ': 'Kyrgyzstan', 'BIH': 'Bosnia and Herzegovina', 'LBN': 'Lebanon', 'COD': 'Democratic Republic of the Congo', 'MYS': 'Malaysia', 'GBP': 'Protected Person', 'PER': 'Peru', 'LBY': 'Libyan Arab Jamahiriya', 'ZAR': 'Zaire', 'FJI': 'Fiji', 'VEN': 'Venezuela', 'VCT': 'Saint Vincent and the Grenadines', 'MDA': 'Republic of Moldova', 'IOT': 'British Indian Ocean Territory', 'TUN': 'Tunisia', 'ARE': 'United Arab Emirates', 'HMD': 'Heard and McDonald Islands', 'SLB': 'Solomon Islands', 'GTM': 'Guatemala', 'PYF': 'French Polynesia', 'MRT': 'Mauritania', 'SEN': 'Senegal', 'MNG': 'Mongolia', 'BDI': 'Burundi', 'EST': 'Estonia', 'KNA': 'Saint Kitts and Nevis', 'JPN': 'Japan', 'BEN': 'Benin', 'GNB': 'Guinea-Bissau', 'KOR': 'Republic of Korea', 'NPL': 'Nepal', 'TUV': 'Tuvalu', 'TWN': 'Taiwan Province of China', 'MLT': 'Malta', 'BGD': 'Bangladesh', 'SPM': 'Saint Pierre and Miquelon', 'UGA': 'Uganda', 'BFA': 'Burkina Faso', 'TJK': 'Tajikistan', 'MWI': 'Malawi', 'TMP': 'East Timor', 'KWT': 'Kuwait', 'ESH': 'Western Sahara', 'BOL': 'Bolivia', 'OMN': 'Oman', 'CRI': 'Costa Rica', 'BLZ': 'Belize', 'NAM': 'Namibia', 'ZAF': 'South Africa', 'SVK': 'Slovakia', 'ZMB': 'Zambia', 'PRT': 'Portugal', 'LSO': 'Lesotho', 'URY': 'Uruguay', 'USA': 'United States of America', 'MDV': 'Maldives', 'UMI': 'United States of America Minor Outlying Islands', 'ISR': 'Israel', 'GBR': 'United Kingdom of Great Britain and Northern Ireland', 'GBO': 'Overseas citizen', 'NLD': 'Netherlands, Kingdom of the', 'SDN': 'Sudan', 'ISL': 'Iceland', 'VGB': 'Virgin Islands (Great Britian)', 'ALB': 'Albania', 'WSM': 'Samoa', 'ETH': 'Ethiopia', 'TCA': 'Turks and Caicos Islands', 'NCL': 'New Caledonia', 'BMU': 'Bermuda', 'JOR': 'Jordan', 'AUS': 'Australia', 'KHM': 'Cambodia', 'CHL': 'Chile', 'VAT': 'Holy See (Vatican City State)', 'SJM': 'Svalbard and Jan Mayen Islands', 'CUB': 'Cuba', 'FXX': 'France, Metropolitan', 'CMR': 'Cameroon', 'MCO': 'Monaco', 'GHA': 'Ghana', 'ITA': 'Italy', 'CPV': 'Cape Verde', 'BEL': 'Belgium', 'BGR': 'Bulgaria', 'HKG': 'Hong Kong', 'KEN': 'Kenya', 'FRA': 'France', 'MLI': 'Mali', 'ZWE': 'Zimbabwe', 'RWA': 'Rwanda', 'CYP': 'Cyprus', 'IRL': 'Ireland', 'NER': 'Niger', 'IRQ': 'Iraq', 'FRO': 'Faeroe Islands', 'LTU': 'Lithuania', 'ROM': 'Romania', 'NGA': 'Nigeria'}


def qr_detect_text(image_file):
    try:
        service = ['eTOURIST VISA', 'eBUSSINESS VISA', 'eMEDICAL VISA',
                   'eCONFERENCE Visa', 'eMEDICAL ATTENDANT VISA']
        text_data = text_getter(image_file)
        if text_data["success"] is False:
            return {"success": False, "message": text_data["message"]}
        text = text_data["data"]
...
def other_images():
    try:
        ...
        if croppedother["success"] is False:
